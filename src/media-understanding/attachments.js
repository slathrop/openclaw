...
function orderAttachments(attachments, prefer) {
  ...
  if (prefer === 'last') {
    return [..._attachments].toReversed();
  }
  if (prefer === 'path') {
...
class MediaAttachmentCache {
  ...
  constructor(attachments) {
    ...
    for (const attachment of attachments) {
      this._entries.set(attachment.index, { attachment });
    }
  }
  ...
  async ensureEntry(attachmentIndex) {
    const existing = this._entries.get(attachmentIndex);
    if (existing) {
      if (!existing.resolvedPath) {
        existing.resolvedPath = this._resolveLocalPath(existing.attachment);
      }
      return existing;
    }
    const attachment = this._attachments.find((item) => item.index === attachmentIndex) ?? {
      index: attachmentIndex
    };
    const entry = {
      attachment,
      resolvedPath: this._resolveLocalPath(attachment)
    };
    this._entries.set(attachmentIndex, entry);
    return entry;
  }
  _resolveLocalPath(attachment) {
