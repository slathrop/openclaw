...
describe('describeGeminiVideo', () => {
  ...
  beforeEach(() => {
    resolvePinnedHostnameSpy = vi.spyOn(ssrf, 'resolvePinnedHostnameWithPolicy').mockImplementation(async (hostname) => {
  ...
  it('builds the expected request payload', async () => {
    ...
    const bodyText = typeof seenInit?.body === 'string' ? seenInit.body : Buffer.isBuffer(seenInit?.body) ? seenInit.body.toString('utf8') : '';
