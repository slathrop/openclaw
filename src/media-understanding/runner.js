...
async function resolveKeyEntry(params) {
  ...
  if (capability === 'image') {
    const activeProvider2 = params.activeModel?.provider?.trim();
    if (activeProvider2) {
      const activeEntry = await checkProvider(activeProvider2, params.activeModel?.model);
      if (activeEntry) {
        return activeEntry;
      }
    }
...
function buildModelDecision(params) {
  ...
  const providerId = providerIdRaw ? normalizeMediaProviderId(providerIdRaw) : void 0;
  return {
    type: 'provider',
...
async function runProviderEntry(params) {
  ...
  if (capability === 'image') {
    ...
    const media2 = await params.cache.getBuffer({
      attachmentIndex: params.attachmentIndex,
      maxBytes,
      timeoutMs
    });
    const provider2 = getMediaUnderstandingProvider(providerId, params.providerRegistry);
    const result2 = provider2?.describeImage ? await provider2.describeImage({
      buffer: media2.buffer,
    ...
    return {
      ...
      attachmentIndex: params.attachmentIndex,
      text: trimOutput(result2.text, maxChars),
      provider: providerId,
      model: result2.model ?? modelId
    };
  }
  const provider = getMediaUnderstandingProvider(providerId, params.providerRegistry);
  if (!provider) {
  ...
  if (capability === 'audio') {
    ...
    const auth2 = await resolveApiKeyForProvider({
      provider: providerId,
      cfg,
    ...
    const apiKey2 = requireApiKey(auth2, providerId);
    const providerConfig2 = cfg.models?.providers?.[providerId];
    const baseUrl = entry.baseUrl ?? params.config?.baseUrl ?? providerConfig2?.baseUrl;
    const mergedHeaders = {
      ...providerConfig2?.headers,
    ...
    const result2 = await provider.transcribeAudio({
      buffer: media2.buffer,
      fileName: media2.fileName,
      mime: media2.mime,
      apiKey: apiKey2,
