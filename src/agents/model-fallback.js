 ...
 * @typedef {object} FallbackChain - Ordered list of fallback models.
 * @property
 */
...
function resolveFallbackCandidates(params) {
  ...
  const modelFallbacks = (() => {
    ...
    const model2 = params.cfg?.agents?.defaults?.model;
    if (model2 && typeof model2 === 'object') {
      return model2.fallbacks ?? [];
    }
    return [];
  })();
...
async function runWithModelFallback(params) {
  ...
  const summary = attempts.length > 0 ? attempts.map(
    (attempt) => `${attempt.provider}/${attempt.model}: ${attempt.error}${attempt.reason ? ` (${attempt.reason})` : ''}`
  ).join(' | ') : 'unknown';
