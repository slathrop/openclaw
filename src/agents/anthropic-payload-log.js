/**
 * @module anthropic-payload-log
 * Anthropic API payload logging for debugging and diagnostics.
 * @typedef {object} AnthropicPayloadLogger - Callback for logging Anthropic API payloads.
 * @property
 */
...
function resolvePayloadLogConfig(env) {
  ...
  const filePath = fileOverride ? resolveUserPath(fileOverride) : path.join(resolveStateDir(env), 'logs', 'anthropic-payload.jsonl');
