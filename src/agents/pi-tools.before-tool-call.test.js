...
describe('before_tool_call hook integration', () => {
  ...
  it('executes tool normally when no hook is registered', async () => {
    ...
    await tool.execute('call-1', { path: '/tmp/file' }, void 0, void 0);
    expect(hookRunner.runBeforeToolCall).not.toHaveBeenCalled();
    expect(execute).toHaveBeenCalledWith('call-1', { path: '/tmp/file' }, void 0, void 0);
  });
  it('allows hook to modify parameters', async () => {
    ...
    await tool.execute('call-2', { cmd: 'ls' }, void 0, void 0);
  ...
  it('blocks tool execution when hook returns block=true', async () => {
    ...
    const execute = vi.fn().mockResolvedValue({ content: [], details: { ok: true } });
    const tool = wrapToolWithBeforeToolCallHook({ name: 'exec', execute });
    await expect(tool.execute('call-3', { cmd: 'rm -rf /' }, void 0, void 0)).rejects.toThrow(
      'blocked'
    );
  ...
  it('continues execution when hook throws', async () => {
    ...
    const execute = vi.fn().mockResolvedValue({ content: [], details: { ok: true } });
    const tool = wrapToolWithBeforeToolCallHook({ name: 'read', execute });
    await tool.execute('call-4', { path: '/tmp/file' }, void 0, void 0);
    expect(execute).toHaveBeenCalledWith('call-4', { path: '/tmp/file' }, void 0, void 0);
  });
  it('normalizes non-object params for hook contract', async () => {
    hookRunner.hasHooks.mockReturnValue(true);
    hookRunner.runBeforeToolCall.mockResolvedValue(void 0);
    const execute = vi.fn().mockResolvedValue({ content: [], details: { ok: true } });
    const tool = wrapToolWithBeforeToolCallHook({ name: 'ReAd', execute }, {
      agentId: 'main',
      sessionKey: 'main'
    });
    await tool.execute('call-5', 'not-an-object', void 0, void 0);
...
describe('before_tool_call hook integration for client tools', () => {
  ...
  it('passes modified params to client tool callbacks', async () => {
    ...
    await tool.execute('client-call-1', { value: 'ok' }, void 0, void 0, void 0);
