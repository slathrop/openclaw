/**
 * Sandbox configuration loading and resolution.
 * @module agents/sandbox/config
 */
...
function resolveSandboxDockerConfig(params) {
  ...
  const env = agentDocker?.env ? { ...globalDocker?.env ?? { LANG: 'C.UTF-8' }, ...agentDocker.env } : globalDocker?.env ?? { LANG: 'C.UTF-8' };
  const ulimits = agentDocker?.ulimits ? { ...globalDocker?.ulimits, ...agentDocker.ulimits } : globalDocker?.ulimits;
