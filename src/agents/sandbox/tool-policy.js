/**
 * Tool execution policy enforcement for sandbox environments.
 * @module agents/sandbox/tool-policy
 */
...
function resolveSandboxToolPolicyForAgent(cfg, agentId) {
  ...
  if (!expandedDeny.map((v) => v.toLowerCase()).includes('image') && !expandedAllow.map((v) => v.toLowerCase()).includes('image')) {
