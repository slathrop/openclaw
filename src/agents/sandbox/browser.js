/**
 * Browser sandbox management for agent tool execution.
 * @module agents/sandbox/browser
 */
...
async function ensureSandboxBrowser(params) {
  ...
  const ensureBridge = async () => {
    ...
    const onEnsureAttachTarget = params.cfg.browser.autoStart ? async () => {
      const state2 = await dockerContainerState(containerName);
      if (state2.exists && !state2.running) {
        await execDocker(['start', containerName]);
      }
