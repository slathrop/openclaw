...
function listSetupTokenProfiles(store) {
  return Object.entries(store.profiles).filter(([, cred]) => {
    if (cred.type !== 'token') {
      return false;
    }
...
async function resolveTokenSource() {
  const explicitToken = (SETUP_TOKEN_RAW && isSetupToken(SETUP_TOKEN_RAW) ? SETUP_TOKEN_RAW : '') || SETUP_TOKEN_VALUE;
  if (explicitToken) {
    ...
    const store2 = ensureAuthProfileStore(tempDir, {
    ...
    saveAuthProfileStore(store2, tempDir);
    return {
      agentDir: tempDir,
      profileId,
      cleanup: async () => {
...
function pickModel(models, raw) {
  ...
  if (normalized) {
    ...
    return models.find(
      (model) => normalizeProviderId(model.provider) === parsed.provider && model.id === parsed.model
    ) ?? null;
  }
  const preferred = [
    'claude-opus-4-5',
    ...
    async () => {
      ...
      try {
        ...
        const candidates = all.filter(
          (model2) => normalizeProviderId(model2.provider) === 'anthropic'
        );
        expect(candidates.length).toBeGreaterThan(0);
        const model = pickModel(candidates, SETUP_TOKEN_MODEL);
        if (!model) {
          throw new Error(
            SETUP_TOKEN_MODEL ? `Model not found: ${SETUP_TOKEN_MODEL}` : 'No Anthropic models available.'
          );
        }
