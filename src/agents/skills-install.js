/**
 * @module skills-install
 * Skill package installation from npm, git, and local sources.
 */
...
async function extractArchive(params) {
  ...
  if (archiveType === 'zip') {
    ...
    const argv2 = ['unzip', '-q', archivePath, '-d', targetDir];
    return await runCommandWithTimeout(argv2, { timeoutMs });
  }
  if (!hasBinary('tar')) {
