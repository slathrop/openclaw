...
describe('sanitizeToolCallIdsForCloudCodeAssist', () => {
  describe('strict mode (default)', () => {
    ...
    it('avoids collisions when sanitization would produce duplicate IDs', () => {
      ...
      expect(isValidCloudCodeAssistToolId(a.id, 'strict')).toBe(true);
      expect(isValidCloudCodeAssistToolId(b.id, 'strict')).toBe(true);
      const r1 = out[1];
      const r2 = out[2];
  ...
  describe('strict9 mode (Mistral tool call IDs)', () => {
    it('enforces alphanumeric IDs with length 9', () => {
      ...
      expect(isValidCloudCodeAssistToolId(a.id, 'strict9')).toBe(true);
      expect(isValidCloudCodeAssistToolId(b.id, 'strict9')).toBe(true);
      const r1 = out[1];
      const r2 = out[2];
      expect(r1.toolCallId).toBe(a.id);
      expect(r2.toolCallId).toBe(b.id);
