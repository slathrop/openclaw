/**
 * @module model-auth
 * Model provider API key resolution and authentication dispatch.
 *
 * SECURITY: Resolves API keys for model providers from config, env vars, and auth profiles.
 *
 * SECURITY: API keys passed in-memory to provider SDKs -- never logged or persisted beyond config.
 *
 * SECURITY: AWS credential resolution supports bearer tokens, access keys, and IAM profiles.
 * @typedef {object} ModelAuthResult - Resolved authentication for a model provider.
 * @property
 */
...
function resolveEnvApiKey(provider) {
  ...
  const applied = new Set(getShellEnvAppliedKeys());
  const pick = (envVar2) => {
    const value = process.env[envVar2]?.trim();
    if (!value) {
      return null;
    }
    const source = applied.has(envVar2) ? `shell env: ${envVar2}` : `env: ${envVar2}`;
...
function resolveModelAuthMode(provider, cfg, store) {
  ...
  if (profiles.length > 0) {
    ...
    const distinct = ['oauth', 'token', 'api_key'].filter(
      (k) => modes.has(k)
    );
