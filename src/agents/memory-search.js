 ...
 * Agent memory search across conversation history and stored context.
 * @typedef {object} MemorySearchResult - A memory search match.
 * @property
 */
...
function mergeConfig(defaults, overrides, agentId) {
  ...
  const hasRemoteConfig = Boolean(
    overrideRemote?.baseUrl || overrideRemote?.apiKey || overrideRemote?.headers || defaultRemote?.baseUrl || defaultRemote?.apiKey || defaultRemote?.headers
  );
