/**
 * @module model-scan
 * Remote model scanning -- OpenRouter, Ollama, and provider discovery.
 * @typedef {object} ModelScanResult - Discovered model from remote scan.
 * @property
 */
...
function ensureImageInput(model) {
  ...
  return {
    ...model,
    input: Array.from(/* @__PURE__ */ new Set([...model.input, 'image']))
  };
}
...
async function scanOpenRouterModels(options = {}) {
  ...
  return mapWithConcurrency(
    ...
    async (entry) => {
      ...
      const imageResult = model.input.includes('image') ? await probeImage(ensureImageInput(model), apiKey, timeoutMs) : { ok: false, latencyMs: null, skipped: true };
      return {
        id: entry.id,
        name: entry.name,
        provider: 'openrouter',
