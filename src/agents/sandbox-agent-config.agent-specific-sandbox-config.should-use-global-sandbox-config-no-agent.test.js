...
describe('Agent-specific sandbox config', () => {
  ...
  it('should allow agent-specific docker setupCommand overrides', async () => {
    ...
    expect(
      spawnCalls.some(
        (call) => call.command === 'docker' && call.args[0] === 'exec' && call.args.includes('-lc') && call.args.includes('echo work')
      )
    ).toBe(true);
  });
  it('should ignore agent-specific docker overrides when scope is shared', async () => {
    ...
    expect(
      spawnCalls.some(
        (call) => call.command === 'docker' && call.args[0] === 'exec' && call.args.includes('-lc') && call.args.includes('echo global')
      )
    ).toBe(true);
  });
});
