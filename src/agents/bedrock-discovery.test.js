...
describe('bedrock discovery', () => {
  ...
  it('filters to active streaming text models and maps modalities', async () => {
    const { discoverBedrockModels, resetBedrockDiscoveryCacheForTest } = await import('./bedrock-discovery.js');
  ...
  it('uses configured defaults for context and max tokens', async () => {
    ...
    const models = await discoverBedrockModels({
      region: 'us-east-1',
      config: { defaultContextWindow: 64e3, defaultMaxTokens: 8192 },
      clientFactory
    });
    expect(models[0]).toMatchObject({ contextWindow: 64e3, maxTokens: 8192 });
  });
