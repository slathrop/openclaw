/**
 * Main run orchestration for Pi embedded agent execution.
  ...
  return enqueueSession(
    () => enqueueGlobal(async () => {
      ...
      if (lockedProfileId) {
        ...
        if (!lockedProfile || normalizeProviderId(lockedProfile.provider) !== normalizeProviderId(provider)) {
      ...
      const throwAuthProfileFailover = (params2) => {
        ...
        const message = params2.message?.trim() || (params2.error ? describeUnknownError(params2.error).trim() : '') || fallbackMessage;
      ...
      try {
        while (true) {
          ...
          if (promptError && !aborted) {
            ...
            const fallbackThinking2 = pickFallbackThinkingLevel({
              message: errorText,
              attempted: attemptedThinking
            });
            if (fallbackThinking2) {
              log.warn(
                `unsupported thinking level for ${provider}/${modelId}; retrying with ${fallbackThinking2}`
              );
              thinkLevel = fallbackThinking2;
              continue;
            }
          ...
          if (shouldRotate) {
            if (lastProfileId) {
              const reason = timedOut || assistantFailoverReason === 'timeout' ? 'timeout' : assistantFailoverReason ?? 'unknown';
              await markAuthProfileFailure({
                store: authStore,
                profileId: lastProfileId,
                reason,
