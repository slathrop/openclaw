...
vi.mock('../context-window-guard.js', () => ({
  CONTEXT_WINDOW_HARD_MIN_TOKENS: 1e3,
  CONTEXT_WINDOW_WARN_BELOW_TOKENS: 5e3,
...
describe('overflow compaction in run loop', () => {
  ...
  it('retries after successful compaction on context overflow promptError', async () => {
    ...
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: overflowError })).mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
  ...
  it('retries compaction up to 3 times before giving up', async () => {
    ...
    mockedCompactDirect
      .mockResolvedValueOnce({
        ok: true,
        compacted: true,
        result: { summary: 'Compacted 1', firstKeptEntryId: 'entry-3', tokensBefore: 18e4 }
      })
      .mockResolvedValueOnce({
        ok: true,
        compacted: true,
        result: { summary: 'Compacted 2', firstKeptEntryId: 'entry-5', tokensBefore: 16e4 }
      })
      .mockResolvedValueOnce({
        ok: true,
        compacted: true,
        result: { summary: 'Compacted 3', firstKeptEntryId: 'entry-7', tokensBefore: 14e4 }
      });
    const result = await runEmbeddedPiAgent(baseParams);
