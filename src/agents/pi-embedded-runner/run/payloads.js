/**
 * Payload building for Pi embedded runner requests.
 * @module agents/pi-embedded-runner/run/payloads
 */
...
function buildEmbeddedRunPayloads(params) {
  ...
  if (params.lastToolError) {
    const lastAssistantHasToolCalls = Array.isArray(params.lastAssistant?.content) && params.lastAssistant?.content.some(
      (block) => block && typeof block === 'object' ? block.type === 'toolCall' : false
    );
    ...
    if (!hasUserFacingReply && !isRecoverableError) {
      const toolSummary = formatToolAggregate(
        ...
        params.lastToolError.meta ? [params.lastToolError.meta] : void 0,
        { markdown: useMarkdown }
      );
