 ...
 * Skill install status resolution and spec selection.
 */
...
function buildSkillStatus(entry, config, prefs, eligibility, bundledNames) {
  ...
  const missing = always ? { bins: [], anyBins: [], env: [], config: [], os: [] } : {
    bins: missingBins,
...
function buildWorkspaceSkillStatus(workspaceDir, opts) {
  ...
  return {
    ...
    skills: skillEntries.map(
      (entry) => buildSkillStatus(entry, opts?.config, prefs, opts?.eligibility, bundledContext.names)
    )
  };
}
export {
