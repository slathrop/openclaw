/**
 * @module cli-runner
 * Generic CLI subprocess runner for external tool invocation.
 */
...
async function runCliAgent(params) {
  ...
  const resolvedArgs = useResume ? baseArgs.map((entry) => entry.replaceAll('{sessionId}', cliSessionIdToSend ?? '')) : baseArgs;
...
${stderr}`);
      ...
      if (outputMode === 'text') {
        return { text: stdout, sessionId: void 0 };
      }
      if (outputMode === 'jsonl') {
        const parsed2 = parseCliJsonl(stdout, backend);
        return parsed2 ?? { text: stdout };
      }
      const parsed = parseCliJson(stdout, backend);
      return parsed ?? { text: stdout };
    });
    const text = output.text?.trim();
    const payloads = text ? [{ text }] : void 0;
    return {
      payloads,
      meta: {
