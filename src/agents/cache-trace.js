/**
 * @module cache-trace
 * Cache trace instrumentation for prompt caching diagnostics.
 * @typedef {'request'|'response'|'delta'|'summary'|'error'} CacheTraceStage
 * @typedef {object} CacheTraceEvent - A cache trace instrumentation event.
 * @typedef {object} CacheTrace - Full cache trace for a request.
 * @property
 */
...
function createCacheTrace(params) {
  ...
  const recordStage = (stage, payload = {}) => {
    ...
    if (payload.prompt !== void 0 && cfg.includePrompt) {
