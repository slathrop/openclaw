 ...
 * CLI credential management -- reading and caching provider credentials.
 *
 * SECURITY: Reads and caches CLI credentials (OAuth tokens, API keys) from filesystem and macOS Keychain.
 *
 * SECURITY: Credential cache invalidation relies on file mtime -- stale caches may serve expired tokens.
 *
 * SECURITY: Keychain access via security(1) CLI -- requires user approval on first access.
 * @typedef {object} ClaudeCliCredential - Claude CLI OAuth or token credential.
 * @typedef {object} CodexCliCredential - Codex CLI OAuth credential.
 * @typedef {object} QwenCliCredential - Qwen CLI OAuth credential.
 * @typedef {object} MiniMaxCliCredential - MiniMax CLI OAuth credential.
 * @property
 */
...
function readCodexKeychainCredentials(options) {
  ...
  try {
    ...
    const expires = Number.isFinite(lastRefresh) ? lastRefresh + 60 * 60 * 1e3 : Date.now() + 60 * 60 * 1e3;
...
function readClaudeCliCredentialsCached(options) {
  ...
  if (ttlMs > 0 && claudeCliCache && claudeCliCache.cacheKey === cacheKey && now - claudeCliCache.readAt < ttlMs) {
...
function writeClaudeCliCredentials(newCredentials, options) {
  ...
  const writeFile = options?.writeFile ?? ((credentials, fileOptions) => writeClaudeCliFileCredentials(credentials, fileOptions));
  if (platform === 'darwin') {
...
function readCodexCliCredentialsCached(options) {
  ...
  if (ttlMs > 0 && codexCliCache && codexCliCache.cacheKey === cacheKey && now - codexCliCache.readAt < ttlMs) {
...
function readQwenCliCredentialsCached(options) {
  ...
  if (ttlMs > 0 && qwenCliCache && qwenCliCache.cacheKey === cacheKey && now - qwenCliCache.readAt < ttlMs) {
...
function readMiniMaxCliCredentialsCached(options) {
  ...
  if (ttlMs > 0 && minimaxCliCache && minimaxCliCache.cacheKey === cacheKey && now - minimaxCliCache.readAt < ttlMs) {
