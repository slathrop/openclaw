/**
 * Auth profile ordering and priority resolution for provider credential selection.
 * @module agents/auth-profiles/order
 */
...
function resolveAuthProfileOrder(params) {
  ...
  const filtered = baseOrder.filter((profileId) => {
    ...
    if (cred.type === 'token') {
      ...
      if (typeof cred.expires === 'number' && Number.isFinite(cred.expires) && cred.expires > 0 && now >= cred.expires) {
        return false;
      }
      return true;
    }
  ...
  if (explicitOrder && explicitOrder.length > 0) {
    ...
    for (const profileId of deduped) {
      ...
      if (typeof cooldownUntil === 'number' && Number.isFinite(cooldownUntil) && cooldownUntil > 0 && now < cooldownUntil) {
