/**
 * SECURITY: Auth credential storage - manages API key and OAuth token persistence.
 * Handles credential lifecycle: load, save, merge, and lock-based concurrent updates.
 * Credentials are stored as JSON on the local filesystem with file-level locking.
...
// eslint-disable-next-line no-unused-vars -- kept for potential future use
...
function coerceAuthStore(raw) {
  ...
  const order = record.order && typeof record.order === 'object' ? Object.entries(record.order).reduce(
    (acc, [provider, value]) => {
...
function loadAuthProfileStore() {
  ...
  if (legacy) {
    ...
    syncExternalCliCredentials(store2);
    return store2;
  }
...
// eslint-disable-next-line no-unused-vars -- signature preserved for future use
...
function ensureAuthProfileStore(agentDir, options) {
  ...
  const mainStore = loadAuthProfileStoreForAgent(void 0, options);
