/**
 * SECURITY: OAuth token refresh and exchange - manages credential forwarding and renewal.
 * Handles token expiry checks, lock-based refresh, and fallback resolution for OAuth profiles.
 * Tokens are refreshed in-place under file lock to prevent concurrent refresh races.
 * @module agents/auth-profiles/oauth
 */
import {
  getOAuthApiKey,
  getOAuthProviders
...
function buildOAuthApiKey(provider, credentials) {
  ...
  return needsProjectId ? JSON.stringify({
...
async function resolveApiKeyForProfile(params) {
  ...
  if (cred.type === 'token') {
    ...
    if (typeof cred.expires === 'number' && Number.isFinite(cred.expires) && cred.expires > 0 && Date.now() >= cred.expires) {
      return null;
    }
  ...
  try {
    ...
    if (params.agentDir) {
      try {
        const mainStore = ensureAuthProfileStore(void 0);
    ...
    throw new Error(
      `OAuth token refresh failed for ${cred.provider}: ${message}. Please try again or re-authenticate.${  hint ? `
