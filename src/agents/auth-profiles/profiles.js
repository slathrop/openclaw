/**
 * SECURITY: Profile credential access - manages per-profile credential scoping and ordering.
 * Controls which credentials are exposed per provider profile and persists profile preferences.
 * @module agents/auth-profiles/profiles
 */
...
async function markAuthProfileGood(params) {
  ...
  const updated = await updateAuthProfileStoreWithLock({
    agentDir,
    updater: (freshStore) => {
      const profile2 = freshStore.profiles[profileId];
      if (!profile2 || profile2.provider !== provider) {
        return false;
      }
