/**
 * @module subagent-announce
 * Subagent completion announcement formatting and delivery.
 */
...
async function sendAnnounce(item) {
  ...
  const threadId = origin?.threadId !== null && origin?.threadId !== undefined && origin.threadId !== '' ? String(origin.threadId) : void 0;
  await callGateway({
    method: 'agent',
    params: {
  ...
  try {
    ...
    await callGateway({
      ...
      params: {
        ...
        to: directOrigin?.to,
        threadId: directOrigin?.threadId !== null && directOrigin?.threadId !== undefined && directOrigin.threadId !== '' ? String(directOrigin.threadId) : void 0,
        idempotencyKey: crypto.randomUUID()
      },
      expectFinal: true,
      timeoutMs: 6e4
