...
const writeAuthStore = async (agentDir, opts) => {
  ...
  const payload = {
    ...
    profiles: {
      ...
      ...opts?.includeAnthropic ? { 'anthropic:default': { type: 'api_key', provider: 'anthropic', key: 'sk-anth' } } : {}
    },
    ...
    try {
      ...
      try {
        ...
        const payload = {
          ...
          usageStats: {
            'openai:p1': { lastUsed: 1, cooldownUntil: now + 60 * 60 * 1e3 },
            'openai:p2': { lastUsed: 2 }
          }
        };
        await fs.writeFile(authPath, JSON.stringify(payload));
  ...
  it('skips profiles in cooldown during initial selection', async () => {
    ...
    try {
      ...
      try {
        ...
        expect(stored.usageStats?.['openai:p1']?.cooldownUntil).toBe(now + 60 * 60 * 1e3);
        expect(typeof stored.usageStats?.['openai:p2']?.lastUsed).toBe('number');
      } finally {
        await fs.rm(agentDir, { recursive: true, force: true });
        await fs.rm(workspaceDir, { recursive: true, force: true });
    ...
    try {
      ...
      try {
        await writeAuthStore(agentDir, {
          usageStats: {
            'openai:p1': { lastUsed: 1, cooldownUntil: now + 60 * 60 * 1e3 },
            'openai:p2': { lastUsed: 2, cooldownUntil: now + 60 * 60 * 1e3 }
          }
        });
        await expect(
          runEmbeddedPiAgent({
    ...
    try {
      ...
      try {
        ...
        const payload = {
          ...
          usageStats: {
            'openai:p1': { lastUsed: 1 },
            'openai:p2': { cooldownUntil: now + 60 * 60 * 1e3 },
            // p2 in cooldown
        ...
        expect(stored.usageStats?.['openai:p2']?.cooldownUntil).toBe(now + 60 * 60 * 1e3);
      } finally {
        await fs.rm(agentDir, { recursive: true, force: true });
        await fs.rm(workspaceDir, { recursive: true, force: true });
      }
