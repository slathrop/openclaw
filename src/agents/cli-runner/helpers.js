/**
 * CLI runner helper utilities for agent command execution.
 * @module agents/cli-runner/helpers
 */
...
async function cleanupResumeProcesses(backend, sessionId) {
  ...
  const pattern = [commandToken, ...resumeTokens].filter(Boolean).map((token) => escapeRegex(token)).join('.*');
  if (!pattern) {
    return;
  }
  try {
...
function enqueueCliRun(key, task) {
  ...
  const chained = prior.catch(() => void 0).then(task);
...
function parseCliJson(raw, backend) {
  ...
  const text = collectText(parsed.message) || collectText(parsed.content) || collectText(parsed.result) || collectText(parsed);
...
function resolveSessionIdToSend(params) {
  ...
  if (mode === 'none') {
    return { sessionId: void 0, isNew: !existing };
  }
  if (mode === 'existing') {
