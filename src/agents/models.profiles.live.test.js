...
async function completeOkWithRetry(params) {
  const runOnce = async () => {
    ...
    const text = res.content.filter((block) => block.type === 'text').map((block) => block.text.trim()).join(' ');
    return { res, text };
  };
...
describeLive('live models (profile keys)', () => {
  it(
    ...
    async () => {
      ...
      const perModelTimeoutMs = toInt(process.env.OPENCLAW_LIVE_MODEL_TIMEOUT_MS, 3e4);
      const failures = [];
      const skipped = [];
      const candidates = [];
      ...
      for (const [index, entry] of candidates.entries()) {
        ...
        for (let attempt = 0; attempt < attemptMax; attempt += 1) {
          ...
          const apiKey = model.provider === 'anthropic' && anthropicKeys.length > 0 ? anthropicKeys[attempt] : requireApiKey(apiKeyInfo, model.provider);
          try {
            if (model.provider === 'openai' && model.api === 'openai-responses' && model.id === 'gpt-5.2') {
              ...
              let firstText = first.content.filter((b) => b.type === 'text').map((b) => b.text.trim()).join(' ').trim();
              for (let i = 0; i < 2 && (!toolCall || firstText.length > 0); i += 1) {
                firstUserContent = 'Call the tool `noop` with {}. IMPORTANT: respond ONLY with the tool call; no other text.';
                firstUser = {
                  role: 'user',
                  content: firstUserContent,
                  timestamp: Date.now()
                ...
                firstText = first.content.filter((b) => b.type === 'text').map((b) => b.text.trim()).join(' ').trim();
              }
              ...
              const secondText = second.content.filter((b) => b.type === 'text').map((b) => b.text.trim()).join(' ');
            ...
            if (ok.text.length === 0 && (model.provider === 'openrouter' || model.provider === 'opencode')) {
              skipped.push({
                model: id,
            ...
            if (ok.text.length === 0 && allowNotFoundSkip && (model.provider === 'google-antigravity' || model.provider === 'openai-codex')) {
              skipped.push({
                model: id,
                reason: 'no text returned (provider returned empty content)'
              });
            ...
            if (allowNotFoundSkip && model.provider === 'minimax' && message.includes('request ended without sending any chunks')) {
              skipped.push({ model: id, reason: message });
              logProgress(`${progressLabel}: skip (minimax empty response)`);
              break;
            }
            if (allowNotFoundSkip && model.provider === 'opencode' && isRateLimitErrorMessage(message)) {
              skipped.push({ model: id, reason: message });
              logProgress(`${progressLabel}: skip (rate limit)`);
              break;
            }
            if (allowNotFoundSkip && model.provider === 'openai-codex' && isChatGPTUsageLimitErrorMessage(message)) {
              skipped.push({ model: id, reason: message });
              logProgress(`${progressLabel}: skip (chatgpt usage limit)`);
              break;
            }
            if (allowNotFoundSkip && model.provider === 'openai-codex' && isInstructionsRequiredError(message)) {
              skipped.push({ model: id, reason: message });
      ...
      if (failures.length > 0) {
        const preview = failures.slice(0, 10).map((f) => `- ${f.model}: ${f.error}`).join('\n');
