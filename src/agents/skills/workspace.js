/**
 * Workspace skill resolution and loading.
 * @module agents/skills/workspace
 */
import {
...
function loadSkillEntries(workspaceDir, opts) {
  const loadSkills = (params) => {
    ...
    if (loaded && typeof loaded === 'object' && 'skills' in loaded && Array.isArray(loaded.skills)) {
...
function buildWorkspaceSkillsPrompt(workspaceDir, opts) {
  ...
  const remoteNote = opts?.eligibility?.remote?.note?.trim();
  return [remoteNote, formatSkillsForPrompt(promptEntries.map((entry) => entry.skill))].filter(Boolean).join('\n');
}
