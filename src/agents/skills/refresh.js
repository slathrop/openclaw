/**
 * Skill refresh and reload logic.
 * @module agents/skills/refresh
 */
...
function resolveWatchPaths(workspaceDir, config) {
  ...
  const extraDirs = extraDirsRaw.map((d) => typeof d === 'string' ? d.trim() : '').filter(Boolean).map((dir) => resolveUserPath(dir));
