...
describe('createOpenClawCodingTools', () => {
  describe('Claude/Gemini alias support', () => {
    it('adds Claude-style aliases to schemas without dropping metadata', () => {
      ...
      const params = patched.parameters;
  ...
  it('keeps browser tool schema OpenAI-compatible without normalization', () => {
    ...
    const schema = browser.parameters;
  ...
  it('keeps browser tool schema properties after normalization', () => {
    ...
    const parameters = browser?.parameters;
  ...
  it('exposes raw for gateway config.apply tool calls', () => {
    ...
    const parameters = gateway?.parameters;
  ...
  it('flattens anyOf-of-literals to enum for provider compatibility', () => {
    ...
    const snapshotFormat = parameters.properties?.snapshotFormat;
  ...
  it('cleans tuple items schemas', () => {
    ...
    const tuples = cleaned.properties?.tuples;
  ...
  it('drops null-only union variants without flattening other unions', () => {
    ...
    const parentId = cleaned.properties?.parentId;
    const count = cleaned.properties?.count;
  ...
  it('avoids anyOf/oneOf/allOf in tool schemas', () => {
    const offenders = [];
    const keywords = /* @__PURE__ */ new Set(['anyOf', 'oneOf', 'allOf']);
    const walk = (value, path2, name) => {
      if (!value) {
        return;
      }
      if (Array.isArray(value)) {
        for (const [index, entry] of value.entries()) {
          walk(entry, `${path2}[${index}]`, name);
        }
        return;
      }
      ...
      for (const [key, entry] of Object.entries(record)) {
        const nextPath = path2 ? `${path2}.${key}` : key;
        if (keywords.has(key)) {
  ...
  it('keeps raw core tool schemas union-free', () => {
    ...
    const coreTools = /* @__PURE__ */ new Set([
      'browser',
      'canvas',
      'nodes',
      'cron',
  ...
  it('removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility', () => {
    const unsupportedKeywords = /* @__PURE__ */ new Set([
      'patternProperties',
    ...
    const findUnsupportedKeywords = (schema, path2) => {
      const found = [];
      ...
      if (Array.isArray(schema)) {
        schema.forEach((item, i) => {
          found.push(...findUnsupportedKeywords(item, `${path2}[${i}]`));
        });
        return found;
      }
      const record = schema;
      ...
      if (properties) {
        for (const [key, value] of Object.entries(properties)) {
          found.push(...findUnsupportedKeywords(value, `${path2}.properties.${key}`));
        }
      }
      for (const [key, value] of Object.entries(record)) {
        ...
        if (unsupportedKeywords.has(key)) {
          found.push(`${path2}.${key}`);
        }
        if (value && typeof value === 'object') {
          found.push(...findUnsupportedKeywords(value, `${path2}.${key}`));
        }
      }
      return found;
    };
