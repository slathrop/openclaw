/**
 * Browser interaction tool for web page navigation and content extraction.
 * @module agents/tools/browser-tool
 */
...
async function callBrowserProxy(params) {
  const gatewayTimeoutMs = typeof params.timeoutMs === 'number' && Number.isFinite(params.timeoutMs) ? Math.max(1, Math.floor(params.timeoutMs)) : DEFAULT_BROWSER_PROXY_TIMEOUT_MS;
...
function createBrowserTool(opts) {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const resolvedTarget = target === 'node' ? void 0 : target;
      const baseUrl = nodeTarget ? void 0 : resolveBrowserBaseUrl({
        target: resolvedTarget,
        sandboxBridgeUrl: opts?.sandboxBridgeUrl,
        allowHostControl: opts?.allowHostControl
      });
      const proxyRequest = nodeTarget ? async (opts2) => {
          ...
          try {
            ...
            if (msg.includes('404:') && msg.includes('tab not found') && profile === 'chrome') {
              const tabs = proxyRequest ? (await proxyRequest({
                ...
                path: '/tabs',
                profile
              })).tabs ?? [] : await browserTabs(baseUrl, { profile }).catch(() => []);
              if (!tabs.length) {
                throw new Error(
