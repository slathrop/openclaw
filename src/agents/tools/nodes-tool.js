/**
 * Nodes tool for managing distributed gateway nodes.
 * @module agents/tools/nodes-tool
 */
      ...
      try {
        switch (action) {
          ...
          case 'camera_clip': {
            ...
            const durationMs = typeof params.durationMs === 'number' && Number.isFinite(params.durationMs) ? params.durationMs : typeof params.duration === 'string' ? parseDurationMs(params.duration) : 3e3;
          ...
          case 'screen_record': {
            ...
            const screenIndex = typeof params.screenIndex === 'number' && Number.isFinite(params.screenIndex) ? params.screenIndex : 0;
            const includeAudio = typeof params.includeAudio === 'boolean' ? params.includeAudio : true;
            const raw = await callGatewayTool('node.invoke', gatewayOpts, {
              nodeId,
            ...
            const filePath = typeof params.outPath === 'string' && params.outPath.trim() ? params.outPath.trim() : screenRecordTempPath({ ext: payload.format || 'mp4' });
          ...
          case 'location_get': {
            const node = readStringParam(params, 'node', { required: true });
            const nodeId = await resolveNodeId(gatewayOpts, node);
            const maxAgeMs = typeof params.maxAgeMs === 'number' && Number.isFinite(params.maxAgeMs) ? params.maxAgeMs : void 0;
            const desiredAccuracy = params.desiredAccuracy === 'coarse' || params.desiredAccuracy === 'balanced' || params.desiredAccuracy === 'precise' ? params.desiredAccuracy : void 0;
            const locationTimeoutMs = typeof params.locationTimeoutMs === 'number' && Number.isFinite(params.locationTimeoutMs) ? params.locationTimeoutMs : void 0;
            const raw = await callGatewayTool('node.invoke', gatewayOpts, {
              nodeId,
          ...
          case 'run': {
            ...
            const supportsSystemRun = Array.isArray(nodeInfo?.commands) ? nodeInfo?.commands?.includes('system.run') : false;
            ...
            const needsScreenRecording = typeof params.needsScreenRecording === 'boolean' ? params.needsScreenRecording : void 0;
            const raw = await callGatewayTool('node.invoke', gatewayOpts, {
              nodeId,
              command: 'system.run',
              params: {
