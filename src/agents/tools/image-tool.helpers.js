/**
 * Helper utilities for image tool operations.
 * @module agents/tools/image-tool.helpers
 */
...
function coerceImageAssistantText(params) {
  ...
  if (stop === 'error' || stop === 'aborted') {
    throw new Error(
      errorMessage ? `Image model failed (${params.provider}/${params.model}): ${errorMessage}` : `Image model failed (${params.provider}/${params.model})`
    );
  }
  if (errorMessage) {
...
function resolveProviderVisionModelFromConfig(params) {
  ...
  const preferMinimaxVl = params.provider === 'minimax' ? models.find(
    (m) => (m?.id ?? '').trim() === 'MiniMax-VL-01' && Array.isArray(m?.input) && m.input.includes('image')
  ) : null;
  const picked = preferMinimaxVl ?? models.find((m) => Boolean((m?.id ?? '').trim()) && m.input?.includes('image'));
