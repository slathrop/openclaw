/**
 * Slack action tool for workspace interactions and messaging.
 * @module agents/tools/slack-actions
 */
...
const messagingActions = /* @__PURE__ */ new Set(['sendMessage', 'editMessage', 'deleteMessage', 'readMessages']);
...
async function handleSlackAction(params, cfg, context) {
  const resolveChannelId = () => resolveSlackChannelId(
  ...
  if (reactionsActions.has(action)) {
    ...
    if (action === 'react') {
      ...
      if (isEmpty) {
        const removed = writeOpts ? await removeOwnSlackReactions(channelId, messageId, writeOpts) : await removeOwnSlackReactions(channelId, messageId);
    ...
    const reactions = readOpts ? await listSlackReactions(channelId, messageId, readOpts) : await listSlackReactions(channelId, messageId);
    ...
    switch (action) {
      ...
      case 'readMessages': {
        ...
        const messages = result.messages.map(
          (message) => withNormalizedTimestamp(
            message,
            message.ts
          )
        );
  ...
  if (pinActions.has(action)) {
    ...
    const normalizedPins = pins.map((pin) => {
      const message = pin.message ? withNormalizedTimestamp(
        pin.message,
        pin.message.ts
      ) : pin.message;
