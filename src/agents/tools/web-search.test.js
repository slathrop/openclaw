...
const { inferPerplexityBaseUrlFromApiKey, resolvePerplexityBaseUrl, normalizeFreshness } = __testing;
describe('web_search perplexity baseUrl defaults', () => {
  ...
  it('defaults to direct when using PERPLEXITY_API_KEY', () => {
    expect(resolvePerplexityBaseUrl(void 0, 'perplexity_env')).toBe('https://api.perplexity.ai');
  });
  it('defaults to OpenRouter when using OPENROUTER_API_KEY', () => {
    expect(resolvePerplexityBaseUrl(void 0, 'openrouter_env')).toBe(
      'https://openrouter.ai/api/v1'
    );
  });
  it('defaults to direct when config key looks like Perplexity', () => {
    expect(resolvePerplexityBaseUrl(void 0, 'config', 'pplx-123')).toBe(
      'https://api.perplexity.ai'
    );
  });
  it('defaults to OpenRouter when config key looks like OpenRouter', () => {
    expect(resolvePerplexityBaseUrl(void 0, 'config', 'sk-or-v1-123')).toBe(
      'https://openrouter.ai/api/v1'
    );
  });
  it('defaults to OpenRouter for unknown config key formats', () => {
    expect(resolvePerplexityBaseUrl(void 0, 'config', 'weird-key')).toBe(
      'https://openrouter.ai/api/v1'
    );
  });
});
