/**
 * Discord messaging actions (send, edit, delete messages).
 * @module agents/tools/discord-actions-messaging
 */
import {
  createThreadDiscord,
...
async function handleDiscordMessagingAction(action, params, isActionEnabled) {
  ...
  switch (action) {
    case 'react': {
      ...
      if (isEmpty) {
        const removed = accountId ? await removeOwnReactionsDiscord(channelId, messageId, { accountId }) : await removeOwnReactionsDiscord(channelId, messageId);
    ...
    case 'permissions': {
      ...
      const channelId = resolveChannelId();
      const permissions = accountId ? await fetchChannelPermissionsDiscord(channelId, { accountId }) : await fetchChannelPermissionsDiscord(channelId);
    ...
    case 'fetchMessage': {
      ...
      const message = accountId ? await fetchMessageDiscord(channelId, messageId, { accountId }) : await fetchMessageDiscord(channelId, messageId);
      return jsonResult({
        ok: true,
    ...
    case 'readMessages': {
      ...
      const messages = accountId ? await readMessagesDiscord(channelId, query, { accountId }) : await readMessagesDiscord(channelId, query);
      return jsonResult({
        ok: true,
    ...
    case 'editMessage': {
      ...
      const message = accountId ? await editMessageDiscord(channelId, messageId, { content }, { accountId }) : await editMessageDiscord(channelId, messageId, { content });
    ...
    case 'threadList': {
      ...
      const threads = accountId ? await listThreadsDiscord(
        {
          guildId,
          channelId,
          includeArchived,
    ...
    case 'searchMessages': {
      ...
      const normalizedMessages = Array.isArray(messages) ? messages.map(
        (group) => Array.isArray(group) ? group.map((msg) => normalizeMessage(msg)) : group
      ) : messages;
      return jsonResult({
        ok: true,
        results: {
