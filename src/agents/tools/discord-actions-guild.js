/**
 * Discord guild-level actions (server management, roles, channels).
 * @module agents/tools/discord-actions-guild
 */
...
async function handleDiscordGuildAction(action, params, isActionEnabled) {
  ...
  switch (action) {
    ...
    case 'roleInfo': {
      ...
      const roles = accountId ? await fetchRoleInfoDiscord(guildId, { accountId }) : await fetchRoleInfoDiscord(guildId);
    ...
    case 'emojiList': {
      ...
      const emojis = accountId ? await listGuildEmojisDiscord(guildId, { accountId }) : await listGuildEmojisDiscord(guildId);
    ...
    case 'emojiUpload': {
      ...
      const emoji = accountId ? await uploadEmojiDiscord(
        {
          guildId,
          name,
          mediaUrl,
    ...
    case 'stickerUpload': {
      ...
      const sticker = accountId ? await uploadStickerDiscord(
        {
          guildId,
          name,
          description,
    ...
    case 'channelInfo': {
      ...
      const channel = accountId ? await fetchChannelInfoDiscord(channelId, { accountId }) : await fetchChannelInfoDiscord(channelId);
    ...
    case 'channelList': {
      ...
      const channels = accountId ? await listGuildChannelsDiscord(guildId, { accountId }) : await listGuildChannelsDiscord(guildId);
    ...
    case 'voiceStatus': {
      ...
      const voice = accountId ? await fetchVoiceStatusDiscord(guildId, userId, { accountId }) : await fetchVoiceStatusDiscord(guildId, userId);
    ...
    case 'eventList': {
      ...
      const events = accountId ? await listScheduledEventsDiscord(guildId, { accountId }) : await listScheduledEventsDiscord(guildId);
    ...
    case 'eventCreate': {
      ...
      const event = accountId ? await createScheduledEventDiscord(guildId, payload, { accountId }) : await createScheduledEventDiscord(guildId, payload);
    ...
    case 'channelCreate': {
      ...
      const channel = accountId ? await createChannelDiscord(
        {
          guildId,
          name,
          type: type ?? void 0,
    ...
    case 'channelEdit': {
      ...
      const channel = accountId ? await editChannelDiscord(
        {
          channelId,
          name: name ?? void 0,
          topic: topic ?? void 0,
    ...
    case 'channelDelete': {
      ...
      const result = accountId ? await deleteChannelDiscord(channelId, { accountId }) : await deleteChannelDiscord(channelId);
    ...
    case 'categoryDelete': {
      ...
      const result = accountId ? await deleteChannelDiscord(categoryId, { accountId }) : await deleteChannelDiscord(categoryId);
      return jsonResult(result);
    }
