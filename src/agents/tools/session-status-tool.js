/**
 * Session status tool for inspecting active session state.
 * @module agents/tools/session-status-tool
 */
...
function resolveSessionEntry(params) {
  ...
  const candidates = /* @__PURE__ */ new Set([keyRaw, internal]);
...
function createSessionStatusTool(opts) {
  return {
    ...
    name: 'session_status',
    description: 'Show a /status-equivalent session status card (usage + time + cost when available). Use for model-use questions (\u{1F4CA} session_status). Optional: set per-session model override (model=default resets overrides).',
    ...
    execute: async (_toolCallId, args) => {
        ...
        try {
          ...
          if (snapshot) {
            ...
            if (formatted && !formatted.startsWith('error:')) {
              usageLine = `\u{1F4CA} Usage: ${formatted}`;
            }
          }
        } catch {
      ...
      const isGroup = resolved.entry.chatType === 'group' || resolved.entry.chatType === 'channel' || resolved.key.includes(':group:') || resolved.key.includes(':channel:');
      const groupActivation = isGroup ? normalizeGroupActivation(resolved.entry.groupActivation) ?? 'mention' : void 0;
      ...
      const timeLine = userTime ? `\u{1F552} Time: ${userTime} (${userTimezone})` : `\u{1F552} Time zone: ${userTimezone}`;
      const agentDefaults = cfg.agents?.defaults ?? {};
      const defaultLabel = `${configured.provider}/${configured.model}`;
      const agentModel = typeof agentDefaults.model === 'object' && agentDefaults.model ? { ...agentDefaults.model, primary: defaultLabel } : { primary: defaultLabel };
