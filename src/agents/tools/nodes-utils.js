 ...
 * @module agents/tools/nodes-utils
 */
...
function pickDefaultNode(nodes) {
  const withCanvas = nodes.filter(
    (n) => Array.isArray(n.caps) ? n.caps.includes('canvas') : true
  );
  ...
  const local = candidates.filter(
    (n) => n.platform?.toLowerCase().startsWith('mac') && typeof n.nodeId === 'string' && n.nodeId.startsWith('mac-')
  );
  if (local.length === 1) {
    return local[0];
  }
