/**
 * Message sending tool for cross-channel communication.
 * @module agents/tools/message-tool
 */
...
const EXPLICIT_TARGET_ACTIONS = /* @__PURE__ */ new Set([
  'send',
  'sendWithEffect',
  'sendAttachment',
  'reply',
...
function filterActionsForContext(params) {
  ...
  const isGroupTarget = lowered.startsWith('chat_guid:') || lowered.startsWith('chat_id:') || lowered.startsWith('chat_identifier:') || lowered.startsWith('group:');
  if (isGroupTarget) {
    return params.actions;
  }
...
function buildMessageToolDescription(options) {
  ...
  if (options?.currentChannel) {
    ...
    if (channelActions.length > 0) {
      const allActions = /* @__PURE__ */ new Set(['send', ...channelActions]);
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal) => {
      ...
      if (requireExplicitTarget && actionNeedsExplicitTarget(action)) {
        const explicitTarget = typeof params.target === 'string' && params.target.trim().length > 0 || typeof params.to === 'string' && params.to.trim().length > 0 || typeof params.channelId === 'string' && params.channelId.trim().length > 0 || Array.isArray(params.targets) && params.targets.some((value) => typeof value === 'string' && value.trim().length > 0);
        if (!explicitTarget) {
          throw new Error(
