 ...
 * @module agents/tools/web-search
 */
...
// eslint-disable-next-line no-unused-vars -- documents valid provider values (was used for TS typeof)
...
const BRAVE_FRESHNESS_SHORTCUTS = /* @__PURE__ */ new Set(['pd', 'pw', 'pm', 'py']);
...
function resolvePerplexityApiKey(perplexity) {
  ...
  return { apiKey: void 0, source: 'none' };
}
...
function resolvePerplexityModel(perplexity) {
  const fromConfig = perplexity && 'model' in perplexity && typeof perplexity.model === 'string' ? perplexity.model.trim() : '';
...
async function runWebSearch(params) {
  const cacheKey = normalizeCacheKey(
    params.provider === 'brave' ? `${params.provider}:${params.query}:${params.count}:${params.country || 'default'}:${params.search_lang || 'default'}:${params.ui_lang || 'default'}:${params.freshness || 'default'}` : `${params.provider}:${params.query}:${params.count}:${params.country || 'default'}:${params.search_lang || 'default'}:${params.ui_lang || 'default'}`
  );
  ...
  if (params.provider === 'perplexity') {
    ...
    writeCache(SEARCH_CACHE, cacheKey, payload2, params.cacheTtlMs);
    return payload2;
  }
  ...
  const mapped = results.map((entry) => {
    ...
    const rawSiteName = resolveSiteName(url2);
    return {
