...
describe('markAuthProfileFailure', () => {
  it('disables billing failures for ~5 hours by default', async () => {
    ...
    try {
      ...
      expect(remainingMs).toBeGreaterThan(4.5 * 60 * 60 * 1e3);
      expect(remainingMs).toBeLessThan(5.5 * 60 * 60 * 1e3);
    } finally {
  ...
  it('honors per-provider billing backoff overrides', async () => {
    ...
    try {
      ...
      expect(typeof disabledUntil).toBe('number');
      const remainingMs = disabledUntil - startedAt;
      expect(remainingMs).toBeGreaterThan(0.8 * 60 * 60 * 1e3);
      expect(remainingMs).toBeLessThan(1.2 * 60 * 60 * 1e3);
    } finally {
      fs.rmSync(agentDir, { recursive: true, force: true });
    }
  });
        ...
        JSON.stringify({
          ...
          usageStats: {
            'anthropic:default': {
              ...
              lastFailureAt: now - 48 * 60 * 60 * 1e3
            }
          }
        })
      );
