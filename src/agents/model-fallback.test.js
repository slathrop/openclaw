...
describe('runWithModelFallback', () => {
  ...
  it('falls back on auth errors', async () => {
    const cfg = makeCfg();
    const run = vi.fn().mockRejectedValueOnce(Object.assign(new Error('nope'), { status: 401 })).mockResolvedValueOnce('ok');
  ...
  it('falls back on 402 payment required', async () => {
    const cfg = makeCfg();
    const run = vi.fn().mockRejectedValueOnce(Object.assign(new Error('payment required'), { status: 402 })).mockResolvedValueOnce('ok');
    const result = await runWithModelFallback({
      cfg,
      provider: 'openai',
      model: 'gpt-4.1-mini',
  ...
  it('falls back on credential validation errors', async () => {
    const cfg = makeCfg();
    const run = vi.fn().mockRejectedValueOnce(new Error('No credentials found for profile "anthropic:default".')).mockResolvedValueOnce('ok');
    const result = await runWithModelFallback({
      cfg,
      provider: 'anthropic',
      model: 'claude-opus-4',
  ...
  it('does not append configured primary when fallbacksOverride is set', async () => {
    ...
    const run = vi.fn().mockImplementation(() => Promise.reject(Object.assign(new Error('nope'), { status: 401 })));
    await expect(
      runWithModelFallback({
        cfg,
        provider: 'anthropic',
  ...
  it('falls back on missing API key errors', async () => {
    const cfg = makeCfg();
    const run = vi.fn().mockRejectedValueOnce(new Error('No API key found for profile openai.')).mockResolvedValueOnce('ok');
    const result = await runWithModelFallback({
      cfg,
      provider: 'openai',
      model: 'gpt-4.1-mini',
  ...
  it('falls back when message says aborted but error is a timeout', async () => {
    const cfg = makeCfg();
    const run = vi.fn().mockRejectedValueOnce(Object.assign(new Error('request aborted'), { code: 'ETIMEDOUT' })).mockResolvedValueOnce('ok');
    const result = await runWithModelFallback({
      cfg,
      provider: 'openai',
      model: 'gpt-4.1-mini',
  ...
  it('does not fall back on user aborts', async () => {
    const cfg = makeCfg();
    const run = vi.fn().mockRejectedValueOnce(Object.assign(new Error('aborted'), { name: 'AbortError' })).mockResolvedValueOnce('ok');
    await expect(
      runWithModelFallback({
        cfg,
        provider: 'openai',
  ...
  it('appends the configured primary as a last fallback', async () => {
    ...
    const run = vi.fn().mockRejectedValueOnce(Object.assign(new Error('timeout'), { code: 'ETIMEDOUT' })).mockResolvedValueOnce('ok');
    const result = await runWithModelFallback({
      cfg,
      provider: 'openrouter',
