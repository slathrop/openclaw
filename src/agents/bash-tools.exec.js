/**
 * @module bash-tools.exec
 * Bash exec tool -- single-shot command execution for agents.
 * @typedef {object} ExecToolDefaults - Default options for exec tool.
 * @typedef {object} ExecElevatedDefaults - Elevated exec tool defaults.
 * @typedef {object} ExecToolDetails - Details returned from exec tool.
 * @property
 */
...
const DANGEROUS_HOST_ENV_VARS = /* @__PURE__ */ new Set([
  'LD_PRELOAD',
  'LD_LIBRARY_PATH',
  'LD_AUDIT',
...
` : '';
  ...
  const promise = new Promise((resolve) => {
    ...
    const handleExit = (code, exitSignal) => {
      ...
      const wasSignal = exitSignal !== null && exitSignal !== undefined;
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (host === 'node') {
        ...
        const invokeTimeoutMs = Math.max(
          1e4,
          (typeof params.timeout === 'number' ? params.timeout : defaultTimeoutSec) * 1e3 + 5e3
        );
...
` : '';
          void (async () => {
            ...
            if (decision === 'deny') {
              ...
              if (askFallback === 'full') {
                ...
                /* allowlist fallback -- no additional action */
              } else {
      ...
      if (host === 'gateway' && !bypassApprovals) {
        ...
        if (requiresAsk) {
          ...
          const noticeSeconds = Math.max(1, Math.round(approvalRunningNoticeMs / 1e3));
          const commandText = params.command;
          const effectiveTimeout2 = typeof params.timeout === 'number' ? params.timeout : defaultTimeoutSec;
          const warningText = warnings.length ? `${warnings.join('\n')}
...
` : '';
          void (async () => {
            ...
            markBackgrounded(run2.session);
            let runningTimer = null;
            if (approvalRunningNoticeMs > 0) {
              runningTimer = setTimeout(() => {
                emitExecSystemEvent(
                  `Exec running (gateway id=${approvalId}, session=${run2?.session.id}, >${noticeSeconds}s): ${commandText}`,
                  { sessionKey: notifySessionKey, contextKey }
                );
              }, approvalRunningNoticeMs);
            }
            const outcome = await run2.promise;
            if (runningTimer) {
              clearTimeout(runningTimer);
            }
            const output = normalizeNotifyOutput(
            ...
            const summary = output ? `Exec finished (gateway id=${approvalId}, session=${run2.session.id}, ${exitLabel})
${output}` : `Exec finished (gateway id=${approvalId}, session=${run2.session.id}, ${exitLabel})`;
