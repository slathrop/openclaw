...
const waitForCalls = async (getCount, count, timeoutMs = 2e3) => {
...
describe('sessions tools', () => {
  ...
  it('sessions_send supports fire-and-forget and wait', async () => {
    ...
    let sendCallCount = 0;
    let lastWaitedRunId;
    const replyByRunId = /* @__PURE__ */ new Map();
    ...
    expect(
      agentCalls.some(
        (call) => typeof call.params?.extraSystemPrompt === 'string' && call.params?.extraSystemPrompt?.includes(
  ...
  it('sessions_send runs ping-pong then announces', async () => {
    ...
    const replySteps = calls.filter(
      (call) => call.method === 'agent' && typeof call.params?.extraSystemPrompt === 'string' && call.params?.extraSystemPrompt?.includes(
        'Agent-to-agent reply step'
      )
    );
