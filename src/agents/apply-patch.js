/**
 * @module apply-patch
 * Patch application engine for file modifications via agent tools.
 * @typedef {{filesCreated: number, filesModified: number, filesDeleted: number}} ApplyPatchSummary
 * @typedef {{success: boolean, summary: ApplyPatchSummary, error?: string}} ApplyPatchResult
 * @typedef {{filePath: string, action: string, preview?: string}} ApplyPatchToolDetails
 */
...
async function applyPatch(input, options) {
  ...
  for (const hunk of parsed.hunks) {
    ...
    if (hunk.kind === 'add') {
      const target2 = await resolvePatchPath(hunk.path, options);
      await ensureDir(target2.resolved);
      await fs.writeFile(target2.resolved, hunk.contents, 'utf8');
      recordSummary(summary, seen, 'added', target2.display);
      continue;
    }
    if (hunk.kind === 'delete') {
      const target2 = await resolvePatchPath(hunk.path, options);
      await fs.rm(target2.resolved);
      recordSummary(summary, seen, 'deleted', target2.display);
      continue;
    }
...
async function resolvePatchPath(filePath, options) {
  if (options.sandboxRoot) {
    const resolved2 = await assertSandboxPath({
      filePath,
      cwd: options.cwd,
    ...
    return {
      ...
      display: resolved2.relative || resolved2.resolved
    };
  }
