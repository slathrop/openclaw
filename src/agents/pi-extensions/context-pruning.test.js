...
describe('context-pruning', () => {
  ...
  it('does not touch tool results after the last N assistants', () => {
    ...
    const ctx = {
      model: { contextWindow: 1e3 }
    };
    const next = pruneContextMessages({ messages, settings, ctx });
    expect(toolText(findToolResult(next, 't2'))).toContain('y'.repeat(2e4));
    expect(toolText(findToolResult(next, 't3'))).toContain('z'.repeat(2e4));
    expect(toolText(findToolResult(next, 't4'))).toContain('w'.repeat(2e4));
  ...
  it('never prunes tool results before the first user message', () => {
    ...
    const next = pruneContextMessages({
      messages,
      settings,
      ctx: { model: { contextWindow: 1e3 } },
      isToolPrunable: () => true,
      contextWindowTokensOverride: 1e3
    });
    expect(toolText(findToolResult(next, 't0'))).toBe('x'.repeat(2e4));
    expect(toolText(findToolResult(next, 't1'))).toBe('[cleared]');
  });
  ...
  it('reads per-session settings from registry', async () => {
    ...
    setContextPruningRuntime(sessionManager, {
      ...
      contextWindowTokens: 1e3,
      isToolPrunable: () => true,
      lastCacheTouchAt: Date.now() - DEFAULT_CONTEXT_PRUNING_SETTINGS.ttlMs - 1e3
    });
    const messages = [
      makeUser('u1'),
      makeAssistant('a1'),
    ...
    const api = {
      ...
      appendEntry: () => {
        // intentionally empty -- stub for interface compliance
      }
    };
  ...
  it('cache-ttl prunes once and resets the ttl window', () => {
    const sessionManager = {};
    const lastTouch = Date.now() - DEFAULT_CONTEXT_PRUNING_SETTINGS.ttlMs - 1e3;
    setContextPruningRuntime(sessionManager, {
      settings: {
        ...DEFAULT_CONTEXT_PRUNING_SETTINGS,
        keepLastAssistants: 0,
  ...
  it('respects tools allow/deny (deny wins; wildcards supported)', () => {
    ...
    const next = pruneContextMessages({ messages, settings, ctx });
    expect(toolText(findToolResult(next, 't1'))).toContain('x'.repeat(2e4));
    expect(toolText(findToolResult(next, 't2'))).toContain('y'.repeat(2e4));
  });
  it('skips tool results that contain images (no soft trim, no hard clear)', () => {
    ...
    expect(toolText(tool)).toContain('x'.repeat(2e4));
  });
