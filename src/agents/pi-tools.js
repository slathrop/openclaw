/**
 * @module pi-tools
 * Pi agent tool registration, provider dispatch, and model compatibility.
 */
import {
  codingTools,
  createEditTool,
...
function createOpenClawCodingTools(options) {
  ...
  const mergeAlsoAllow = (policy, alsoAllow) => {
    ...
    return { ...policy, allow: Array.from(/* @__PURE__ */ new Set([...policy.allow, ...alsoAllow])) };
  };
  ...
  const subagentFiltered = subagentPolicyExpanded ? filterToolsByPolicy(sandboxed, subagentPolicyExpanded) : sandboxed;
  const normalized = subagentFiltered.map(normalizeToolParameters);
  const withHooks = normalized.map(
    (tool) => wrapToolWithBeforeToolCallHook(tool, {
      agentId,
      sessionKey: options?.sessionKey
    })
  );
