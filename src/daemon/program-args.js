...
async function resolveCliProgramArguments(params) {
  ...
  if (runtime === 'bun') {
    if (params.dev) {
      const repoRoot2 = resolveRepoRootForDev();
      const devCliPath2 = path.join(repoRoot2, 'src', 'index.ts');
      await fs.access(devCliPath2);
      const bunPath3 = isBunRuntime(execPath) ? execPath : await resolveBunPath();
      return {
        programArguments: [bunPath3, devCliPath2, ...params.args],
        workingDirectory: repoRoot2
      };
    }
    const bunPath2 = isBunRuntime(execPath) ? execPath : await resolveBunPath();
    const cliEntrypointPath = await resolveCliEntrypointPathForService();
    return {
      programArguments: [bunPath2, cliEntrypointPath, ...params.args]
    };
  }
  if (!params.dev) {
    try {
