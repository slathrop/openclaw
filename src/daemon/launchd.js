...
async function repairLaunchAgentBootstrap(args) {
  ...
  if (boot.code !== 0) {
    return { ok: false, detail: (boot.stderr || boot.stdout).trim() || void 0 };
  }
  const kick = await execLaunchctl(['kickstart', '-k', `${domain}/${label}`]);
  if (kick.code !== 0) {
    return { ok: false, detail: (kick.stderr || kick.stdout).trim() || void 0 };
  }
  return { ok: true };
}
async function findLegacyLaunchAgents(env) {
  const domain = resolveGuiDomain();
  const results = [];
