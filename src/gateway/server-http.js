/**
 * @module gateway/server-http
 * SECURITY: HTTP/HTTPS server with TLS enforcement and origin checking.
 * Manages TLS certificate loading, origin-based request validation,
 * and route-level access control. All external HTTP endpoints must
 * pass origin validation before processing.
 */
import {
...
function createHooksRequestHandler(opts) {
  ...
  return async (req, res) => {
    ...
    if (fromQuery) {
      logHooks.warn(
        'Hook token provided via query parameter is deprecated for security reasons. Tokens in URLs appear in logs, browser history, and referrer headers. Use Authorization: Bearer <token> or X-OpenClaw-Token header instead.'
      );
    }
    if (req.method !== 'POST') {
      res.statusCode = 405;
...
function createGatewayHttpServer(opts) {
  ...
  const httpServer = opts.tlsOptions ? createHttpsServer(opts.tlsOptions, (req, res) => {
