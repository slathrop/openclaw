...
describe('OpenResponses HTTP API (e2e)', () => {
  it('rejects when disabled (default + config)', { timeout: 12e4 }, async () => {
    ...
    // eslint-disable-next-line no-unused-vars -- server started for side effect (must stay alive)
  ...
  it('handles OpenResponses request parsing and validation', async () => {
    ...
    try {
      ...
      expect(missingModelJson.error?.type).toBe(
      ...
      expect(optsHeader?.sessionKey ?? '').toMatch(
        /^agent:beta:/
      );
      ...
      expect(optsModel?.sessionKey ?? '').toMatch(
        /^agent:beta:/
      );
      ...
      expect(optsUser?.sessionKey ?? '').toContain(
      ...
      expect(
        optsMaxTokens?.streamParams?.maxTokens
      ).toBe(123);
      ...
      expect(noUserJson.error?.type).toBe(
        'invalid_request_error'
      );
  ...
  it('streams OpenResponses SSE events', async () => {
    ...
    try {
      ...
      const deltas = deltaEvents.filter((e) => e.event === 'response.output_text.delta').map((e) => {
