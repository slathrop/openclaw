/** @module gateway/server-methods/chat -- Chat session RPC method handlers. */
...
const chatHandlers = {
  ...
  'chat.abort': ({ params, respond, context }) => {
    ...
    if (!runId) {
      const res2 = abortChatRunsForSessionKey(ops, {
        sessionKey,
        stopReason: 'rpc'
      });
      respond(true, { ok: true, aborted: res2.aborted, runIds: res2.runIds });
      return;
    }
  ...
  'chat.send': async ({ params, respond, context, client }) => {
    ...
    if (activeExisting) {
      respond(true, { runId: clientRunId, status: 'in_flight' }, void 0, {
        cached: true,
        runId: clientRunId
      });
      return;
