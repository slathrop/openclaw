/** @module gateway/server-methods/channels -- Channel management RPC method handlers (list, status, start, stop). */
...
const channelsHandlers = {
  'channels.status': async ({ params, respond, context }) => {
    ...
    const resolveRuntimeSnapshot = (channelId, accountId, defaultAccountId) => {
      ...
      const defaultRuntime2 = runtime.channels[channelId];
      const raw = accounts?.[accountId] ?? (accountId === defaultAccountId ? defaultRuntime2 : void 0);
      if (!raw) {
        return void 0;
      }
      return raw;
    ...
    const buildChannelAccounts = async (channelId) => {
      ...
      for (const accountId of accountIds) {
        ...
        if (snapshot.lastInboundAt === null || snapshot.lastInboundAt === undefined) {
          snapshot.lastInboundAt = activity.inboundAt;
        }
        if (snapshot.lastOutboundAt === null || snapshot.lastOutboundAt === undefined) {
