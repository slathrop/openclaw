/** @module gateway/server-methods/sessions -- Session management RPC method handlers. */
...
const sessionsHandlers = {
  ...
  'sessions.preview': ({ params, respond }) => {
    ...
    const keys = keysRaw.map((key) => String(key ?? '').trim()).filter(Boolean).slice(0, 64);
    ...
    if (keys.length === 0) {
      respond(true, { ts: Date.now(), previews: [] }, void 0);
      return;
    }
    const cfg = loadConfig();
  ...
  'sessions.resolve': ({ params, respond }) => {
    ...
    respond(true, { ok: true, key: resolved.key }, void 0);
  },
  ...
  'sessions.reset': async ({ params, respond }) => {
    ...
    respond(true, { ok: true, key: target.canonicalKey, entry: next }, void 0);
  },
  'sessions.delete': async ({ params, respond }) => {
    ...
    if (sessionId) {
      ...
      const ended = await waitForEmbeddedPiRunEnd(sessionId, 15e3);
      if (!ended) {
        respond(
          false,
          void 0,
    ...
    respond(true, { ok: true, key: target.canonicalKey, deleted: existed, archived }, void 0);
  },
