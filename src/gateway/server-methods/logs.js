/** @module gateway/server-methods/logs -- Log retrieval RPC method handler. */
...
async function resolveLogFile(file) {
  ...
  const candidates = await Promise.all(
    entries.filter((entry) => entry.isFile() && ROLLING_LOG_RE.test(entry.name)).map(async (entry) => {
      const fullPath = path.join(dir, entry.name);
      const fileStat = await fs.stat(fullPath).catch(() => null);
      return fileStat ? { path: fullPath, mtimeMs: fileStat.mtimeMs } : null;
    })
  );
  const sorted = candidates.filter((entry) => Boolean(entry)).toSorted((a, b) => b.mtimeMs - a.mtimeMs);
...
const logsHandlers = {
  'logs.tail': async ({ params, respond }) => {
    ...
    try {
      ...
      respond(true, { file, ...result }, void 0);
    } catch (err) {
      respond(
        false,
        void 0,
