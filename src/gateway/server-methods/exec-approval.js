/** @module gateway/server-methods/exec-approval -- Execution approval RPC method handler. */
import {
  ErrorCodes,
  errorShape,
...
function createExecApprovalHandlers(manager, opts) {
  return {
    'exec.approval.request': async ({ params, respond, context }) => {
      ...
      void opts?.forwarder?.handleRequested({
        id: record.id,
        request: record.request,
        createdAtMs: record.createdAtMs,
        expiresAtMs: record.expiresAtMs
    ...
    'exec.approval.resolve': async ({ params, respond, client, context }) => {
      ...
      void opts?.forwarder?.handleResolved({ id: p.id, decision, resolvedBy, ts: Date.now() }).catch((err) => {
