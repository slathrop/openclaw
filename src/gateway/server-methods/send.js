/** @module gateway/server-methods/send -- Message send RPC method handler. */
  ...
  send: async ({ params, respond, context }) => {
    ...
    const work = (async () => {
      try {
        ...
        const mirrorText = mirrorPayloads.map((payload2) => payload2.text).filter(Boolean).join('\n');
        const mirrorMediaUrls = mirrorPayloads.flatMap(
          (payload2) => payload2.mediaUrls ?? (payload2.mediaUrl ? [payload2.mediaUrl] : [])
        );
