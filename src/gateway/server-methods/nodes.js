/** @module gateway/server-methods/nodes -- Node management RPC method handlers. */
...
const nodeHandlers = {
  ...
  'node.rename': async ({ params, respond }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      respond(true, { nodeId: updated.nodeId, displayName: updated.displayName }, void 0);
    });
  },
  'node.list': async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const pairedById = new Map(
        list.paired.filter((entry) => isNodeEntry(entry)).map((entry) => [
          entry.deviceId,
          {
      ...
      const nodeIds = /* @__PURE__ */ new Set([...pairedById.keys(), ...connectedById.keys()]);
  ...
  'node.invoke.result': async ({ params, respond, context, client }) => {
    ...
    if (!ok) {
      ...
      respond(true, { ok: true, ignored: true }, void 0);
      return;
    }
