/** @module gateway/config-reload -- Hot-reload handler for gateway configuration changes. */
...
function resolveGatewayReloadSettings(cfg) {
  ...
  const debounceMs = typeof debounceRaw === 'number' && Number.isFinite(debounceRaw) ? Math.max(0, Math.floor(debounceRaw)) : DEFAULT_RELOAD_SETTINGS.debounceMs;
