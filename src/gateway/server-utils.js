/** @module gateway/server-utils -- Server utility functions (error formatting, request parsing). */
import { defaultVoiceWakeTriggers } from '../infra/voicewake.js';
function normalizeVoiceWakeTriggers(input) {
  const raw = Array.isArray(input) ? input : [];
  const cleaned = raw.map((v) => typeof v === 'string' ? v.trim() : '').filter((v) => v.length > 0).slice(0, 32).map((v) => v.slice(0, 64));
...
function formatError(err) {
  ...
  const hasStatus = statusValue !== void 0;
  const hasCode = codeValue !== void 0;
