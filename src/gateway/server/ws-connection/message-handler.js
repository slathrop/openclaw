/**
 * @module gateway/server/ws-connection/message-handler
 * SECURITY: Per-message authentication validation for WebSocket connections.
 * Every incoming message is validated against the connection's auth state
 * before dispatching to method handlers. Unauthenticated messages are rejected.
 */
...
const DEVICE_SIGNATURE_SKEW_MS = 10 * 60 * 1e3;
    ...
    try {
      ...
      if (!client) {
        ...
        if (role === 'node') {
          ...
          const instanceId2 = typeof instanceIdRaw === 'string' ? instanceIdRaw.trim() : '';
          const nodeIdsForPairing = /* @__PURE__ */ new Set([nodeSession.nodeId]);
          if (instanceId2) {
            nodeIdsForPairing.add(instanceId2);
          }
          for (const nodeId of nodeIdsForPairing) {
            void updatePairedNodeMetadata(nodeId, {
              lastConnectedAtMs: nodeSession.connectedAtMs
            }).catch(
              (err) => logGateway.warn(`failed to record last connect for ${nodeId}: ${formatForLog(err)}`)
            );
          }
          ...
          void loadVoiceWakeConfig().then((cfg) => {
        ...
        void refreshGatewayHealthSnapshot({ probe: true }).catch(
          (err) => logHealth.error(`post-connect health refresh failed: ${formatError(err)}`)
        );
        return;
      }
