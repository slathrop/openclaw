...
describe('sessions_send label lookup', () => {
  it('finds session by label and sends message', { timeout: 6e4 }, async () => {
    const spy = vi.mocked(agentCommand);
    spy.mockImplementation(async (opts) => {
      const params = opts;
...
`, 'utf8');
  ...
  it('returns error when label not found', { timeout: 6e4 }, async () => {
    const tool = createOpenClawTools().find((candidate) => candidate.name === 'sessions_send');
    if (!tool) {
      throw new Error('missing sessions_send tool');
    }
  ...
  it('returns error when neither sessionKey nor label provided', { timeout: 6e4 }, async () => {
    const tool = createOpenClawTools().find((candidate) => candidate.name === 'sessions_send');
    if (!tool) {
      throw new Error('missing sessions_send tool');
    }
