...
describe('gateway server sessions', () => {
  ...
  test('lists and patches session store via sessions.* RPC', async () => {
    ...
    const now = Date.now();
    const recent = now - 3e4;
    const stale = now - 15 * 6e4;
...
`,
    ...
    const list1 = await rpcReq(ws, 'sessions.list', { includeGlobal: false, includeUnknown: false });
    ...
    const sendPolicyPatched = await rpcReq(ws, 'sessions.patch', { key: 'agent:main:main', sendPolicy: 'deny' });
    ...
    const compactedLines = (await fs.readFile(path.join(dir, 'sess-main.jsonl'), 'utf-8')).split(/\r?\n/).filter((l) => l.trim().length > 0);
    ...
    expect(badThinking.error?.message ?? '').toMatch(
