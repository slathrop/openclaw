/** @module gateway/openresponses-http -- Open Responses HTTP API endpoint handler. */
...
function extractUsageFromResult(result) {
  ...
  const usage = meta && typeof meta === 'object' ? meta.agentMeta?.usage : void 0;
  return toUsage(
    usage
  );
}
  ...
  if (!stream) {
    try {
      ...
      if (stopReason === 'tool_calls' && pendingToolCalls && pendingToolCalls.length > 0) {
        ...
        const response2 = createResponseResource({
          id: responseId,
          model,
          status: 'incomplete',
          output: [
