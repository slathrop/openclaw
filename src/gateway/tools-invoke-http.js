/** @module gateway/tools-invoke-http -- HTTP-based tool invocation for remote tool execution. */
...
async function handleToolsInvokeHttpRequest(req, res, opts) {
  ...
  const coreToolNames = new Set(
    allTools.filter((tool2) => !getPluginToolMeta(tool2)).map((tool2) => normalizeToolName(tool2.name)).filter(Boolean)
  );
  const pluginGroups = buildPluginToolGroups({
    tools: allTools,
    // oxlint-disable-next-line typescript/no-explicit-any
    toolMeta: (tool2) => getPluginToolMeta(tool2)
  });
