...
describe('gateway server chat', () => {
  test('handles chat send and history flows', async () => {
    ...
    try {
      ...
      const timeoutCall = spy.mock.calls.at(-1)?.[1];
      ...
      const sessionCall = spy.mock.calls.at(-1)?.[0];
      ...
      expect(blockedRes.error?.message ?? '').toMatch(
        /send blocked/i
      );
      ...
      expect(agentBlockedRes.error?.message ?? '').toMatch(
        /send blocked/i
      );
      testState.sessionStorePath = void 0;
      testState.sessionConfig = void 0;
      ...
      const imgRes = await onceMessage(ws, (o) => o.type === 'res' && o.id === reqId, 8e3);
      expect(imgRes.ok).toBe(true);
      expect(imgRes.payload?.runId).toBeDefined();
      await waitFor(() => spy.mock.calls.length > callsBeforeImage, 8e3);
      const imgOpts = spy.mock.calls.at(-1)?.[1];
      ...
      const imgOnlyRes = await onceMessage(ws, (o) => o.type === 'res' && o.id === reqIdOnly, 8e3);
      expect(imgOnlyRes.ok).toBe(true);
      expect(imgOnlyRes.payload?.runId).toBeDefined();
      await waitFor(() => spy.mock.calls.length > callsBeforeImageOnly, 8e3);
      const imgOnlyOpts = spy.mock.calls.at(-1)?.[1];
  ...
  test('routes chat.send slash commands without agent runs', async () => {
    ...
    try {
      ...
      const eventPromise = onceMessage(
        ws,
        (o) => o.type === 'event' && o.event === 'chat' && o.payload?.state === 'final' && o.payload?.runId === 'idem-command-1',
        8e3
      );
