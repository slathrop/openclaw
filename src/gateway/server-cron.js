/** @module gateway/server-cron -- Scheduled task (cron) management for the gateway. */
...
function buildGatewayCronService(params) {
  ...
  const resolveCronAgent = (requested) => {
    ...
    const hasAgent = normalized !== void 0 && Array.isArray(runtimeConfig.agents?.list) && runtimeConfig.agents.list.some(
