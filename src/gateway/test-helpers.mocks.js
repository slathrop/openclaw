...
const hoisted = vi.hoisted(() => ({
  ...
  agentCommand: vi.fn().mockResolvedValue(void 0),
  ...
  embeddedRunMock: {
    ...
    abortCalls: [],
    waitCalls: [],
    waitResults: /* @__PURE__ */ new Map()
  },
  testTailscaleWhois: { value: null },
  getReplyFromConfig: vi.fn().mockResolvedValue(void 0),
...
const setTestPluginRegistry = (registry) => {
  pluginRegistryState.registry = registry;
  setActivePluginRegistry(registry);
};
const resetTestPluginRegistry = () => {
...
const setTestConfigRoot = (root) => {
  testConfigRoot.value = root;
  process.env.OPENCLAW_CONFIG_PATH = path.join(root, 'openclaw.json');
};
const testTailnetIPv4 = hoisted.testTailnetIPv4;
const testTailscaleWhois = hoisted.testTailscaleWhois;
const piSdkMock = hoisted.piSdkMock;
const cronIsolatedRun = hoisted.cronIsolatedRun;
const agentCommand = hoisted.agentCommand;
const getReplyFromConfig = hoisted.getReplyFromConfig;
const testState = {
  agentConfig: void 0,
  agentsConfig: void 0,
  bindingsConfig: void 0,
  channelsConfig: void 0,
  sessionStorePath: void 0,
  sessionConfig: void 0,
  allowFrom: void 0,
  cronStorePath: void 0,
  cronEnabled: false,
  gatewayBind: void 0,
  gatewayAuth: void 0,
  gatewayControlUi: void 0,
  hooksConfig: void 0,
  canvasHostPort: void 0,
  legacyIssues: [],
  legacyParsed: {},
  migrationConfig: null,
  migrationChanges: []
};
const testIsNixMode = hoisted.testIsNixMode;
const sessionStoreSaveDelayMs = hoisted.sessionStoreSaveDelayMs;
const embeddedRunMock = hoisted.embeddedRunMock;
...
vi.mock('../config/config.js', async () => {
  ...
  const hashConfigRaw = (raw) => crypto.createHash('sha256').update(raw ?? '').digest('hex');
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const agents = testState.agentsConfig ? { ...fileAgents, ...testState.agentsConfig, defaults } : { ...fileAgents, defaults };
      const fileBindings = Array.isArray(fileConfig.bindings) ? fileConfig.bindings : void 0;
      const fileChannels = fileConfig.channels && typeof fileConfig.channels === 'object' && !Array.isArray(fileConfig.channels) ? { ...fileConfig.channels } : {};
      const overrideChannels = testState.channelsConfig && typeof testState.channelsConfig === 'object' ? { ...testState.channelsConfig } : {};
      const mergedChannels = { ...fileChannels, ...overrideChannels };
      if (testState.allowFrom !== void 0) {
        const existing = mergedChannels.whatsapp && typeof mergedChannels.whatsapp === 'object' && !Array.isArray(mergedChannels.whatsapp) ? mergedChannels.whatsapp : {};
        mergedChannels.whatsapp = {
          ...existing,
          allowFrom: testState.allowFrom
        };
      }
      const channels = Object.keys(mergedChannels).length > 0 ? mergedChannels : void 0;
      const fileSession = fileConfig.session && typeof fileConfig.session === 'object' && !Array.isArray(fileConfig.session) ? fileConfig.session : {};
      const session = {
        ...fileSession,
      ...
      const fileGateway = fileConfig.gateway && typeof fileConfig.gateway === 'object' && !Array.isArray(fileConfig.gateway) ? { ...fileConfig.gateway } : {};
      ...
      const gateway = Object.keys(fileGateway).length > 0 ? fileGateway : void 0;
      const fileCanvasHost = fileConfig.canvasHost && typeof fileConfig.canvasHost === 'object' && !Array.isArray(fileConfig.canvasHost) ? { ...fileConfig.canvasHost } : {};
      if (typeof testState.canvasHostPort === 'number') {
        fileCanvasHost.port = testState.canvasHostPort;
      }
      const canvasHost = Object.keys(fileCanvasHost).length > 0 ? fileCanvasHost : void 0;
      const hooks = testState.hooksConfig ?? fileConfig.hooks;
      const fileCron = fileConfig.cron && typeof fileConfig.cron === 'object' && !Array.isArray(fileConfig.cron) ? { ...fileConfig.cron } : {};
      ...
      const cron = Object.keys(fileCron).length > 0 ? fileCron : void 0;
      const config = {
        ...fileConfig,
        agents,
