...
describeLive('gateway live (cli backend)', () => {
  it('runs the agent pipeline against the local CLI backend', async () => {
    ...
    const providerDefaults = providerId === 'claude-cli' ? { command: 'claude', args: DEFAULT_CLAUDE_ARGS } : providerId === 'codex-cli' ? { command: 'codex', args: DEFAULT_CODEX_ARGS } : null;
    ...
    const baseCliArgs = parseJsonStringArray(
    ...
    const cliClearEnv = parseJsonStringArray(
    ...
    const cliImageArg = process.env.OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || void 0;
        ...
        defaults: {
          ...
          cliBackends: {
            ...
            [providerId]: {
              command: cliCommand,
              args: cliArgs,
              clearEnv: cliClearEnv.length > 0 ? cliClearEnv : void 0,
...
`);
    ...
    try {
      ...
      const message = providerId === 'codex-cli' ? `Please include the token CLI-BACKEND-${nonce} in your reply.` : `Reply with exactly: CLI backend OK ${nonce}.`;
      ...
      if (CLI_RESUME) {
        ...
        const resumeMessage = providerId === 'codex-cli' ? `Please include the token CLI-RESUME-${resumeNonce} in your reply.` : `Reply with exactly: CLI backend RESUME OK ${resumeNonce}.`;
      ...
      if (previous.skipChannels === void 0) {
        delete process.env.OPENCLAW_SKIP_CHANNELS;
      } else {
        process.env.OPENCLAW_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === void 0) {
        delete process.env.OPENCLAW_SKIP_GMAIL_WATCHER;
      } else {
        process.env.OPENCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
      }
      if (previous.skipCron === void 0) {
        delete process.env.OPENCLAW_SKIP_CRON;
      } else {
        process.env.OPENCLAW_SKIP_CRON = previous.skipCron;
      }
      if (previous.skipCanvas === void 0) {
        delete process.env.OPENCLAW_SKIP_CANVAS_HOST;
      } else {
        process.env.OPENCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
      }
      if (previous.anthropicApiKey === void 0) {
        delete process.env.ANTHROPIC_API_KEY;
      } else {
        process.env.ANTHROPIC_API_KEY = previous.anthropicApiKey;
      }
      if (previous.anthropicApiKeyOld === void 0) {
        delete process.env.ANTHROPIC_API_KEY_OLD;
      } else {
