/** @module gateway/ws-log -- WebSocket connection logging utilities. */
...
function logWsOptimized(direction, kind, meta) {
  ...
  if (direction === 'in' && kind === 'req' && inflightKey) {
    ...
    if (wsInflightOptimized.size > 2e3) {
...
function logWsCompact(direction, kind, meta) {
  ...
  const arrowColor = kind === 'req' || kind === 'res' ? chalk.yellowBright : direction === 'in' ? chalk.greenBright : chalk.cyanBright;
