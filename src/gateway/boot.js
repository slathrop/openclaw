/** @module gateway/boot -- Gateway boot sequence and initialization orchestration. */
...
async function runBootOnce(params) {
  const bootRuntime = {
    ...
    error: (message2) => log.error(String(message2)),
  ...
  try {
    ...
    log.error(`boot: failed to read ${BOOT_FILENAME}: ${message2}`);
    return { status: 'failed', reason: message2 };
  }
