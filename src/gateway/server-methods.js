/** @module gateway/server-methods -- RPC method dispatcher for incoming gateway requests. */
...
const APPROVAL_METHODS = /* @__PURE__ */ new Set(['exec.approval.request', 'exec.approval.resolve']);
const NODE_ROLE_METHODS = /* @__PURE__ */ new Set(['node.invoke.result', 'node.event', 'skills.bins']);
const PAIRING_METHODS = /* @__PURE__ */ new Set([
  'node.pair.request',
  'node.pair.list',
  'node.pair.approve',
  'node.pair.reject',
...
function authorizeGatewayMethod(method, client) {
  ...
  if (method.startsWith('config.') || method.startsWith('wizard.') || method.startsWith('update.') || method === 'channels.logout' || method === 'skills.install' || method === 'skills.update' || method === 'cron.add' || method === 'cron.update' || method === 'cron.remove' || method === 'cron.run' || method === 'sessions.patch' || method === 'sessions.reset' || method === 'sessions.delete' || method === 'sessions.compact') {
    return errorShape(ErrorCodes.INVALID_REQUEST, 'missing scope: operator.admin');
  }
  return errorShape(ErrorCodes.INVALID_REQUEST, 'missing scope: operator.admin');
}
