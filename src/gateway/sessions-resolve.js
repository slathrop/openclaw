/** @module gateway/sessions-resolve -- Session resolution and lookup helpers. */
...
function resolveSessionKeyFromResolveParams(params) {
  ...
  if (hasKey) {
    ...
    const store2 = loadSessionStore(target.storePath);
    const existingKey = target.storeKeys.find((candidate) => store2[candidate]);
    if (!existingKey) {
      return {
        ok: false,
  ...
  if (hasSessionId) {
    const { storePath: storePath2, store: store2 } = loadCombinedSessionStoreForGateway(cfg);
    const list2 = listSessionsFromStore({
      cfg,
    ...
    const matches = list2.sessions.filter(
