...
describe('gateway server auth/connect', () => {
  describe('default auth (token)', () => {
    ...
    test('closes silent handshakes after timeout', { timeout: 6e4 }, async () => {
      vi.useRealTimers();
      ...
      try {
        ...
        if (prevHandshakeTimeout === void 0) {
    ...
    test('rejects protocol mismatch', async () => {
      ...
      try {
        ...
        // Expected: protocol mismatch should reject
      }
      ws.close();
    });
  ...
  test('allows control ui with device identity when insecure auth is enabled', async () => {
    ...
    expect(typeof nonce).toBe('string');
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } = await import('../infra/device-identity.js');
  ...
  test('allows control ui with stale device identity when device auth is disabled', async () => {
    ...
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } = await import('../infra/device-identity.js');
    const identity = loadOrCreateDeviceIdentity();
    const signedAtMs = Date.now() - 60 * 60 * 1e3;
    const payload = buildDeviceAuthPayload({
      deviceId: identity.deviceId,
      clientId: GATEWAY_CLIENT_NAMES.CONTROL_UI,
      clientMode: GATEWAY_CLIENT_MODES.WEBCHAT,
  ...
  test('requires pairing for scope upgrades', async () => {
    ...
    const { buildDeviceAuthPayload: buildDeviceAuthPayload2 } = await import('./device-auth.js');
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } = await import('../infra/device-identity.js');
    const { approveDevicePairing, getPairedDevice, listDevicePairing } = await import('../infra/device-pairing.js');
    const { GATEWAY_CLIENT_MODES: GATEWAY_CLIENT_MODES2, GATEWAY_CLIENT_NAMES: GATEWAY_CLIENT_NAMES2 } = await import('../utils/message-channel.js');
    const { server, ws, port, prevToken } = await startServerWithClient('secret');
    const identityDir = await mkdtemp(join(tmpdir(), 'openclaw-device-scope-'));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, 'device.json'));
    const client = {
      id: GATEWAY_CLIENT_NAMES2.TEST,
      version: '1.0.0',
      platform: 'test',
      mode: GATEWAY_CLIENT_MODES2.TEST
    };
    const buildDevice = (scopes) => {
      const signedAtMs = Date.now();
      const payload = buildDeviceAuthPayload2({
        deviceId: identity.deviceId,
        clientId: client.id,
  ...
  test('rejects revoked device token', async () => {
    const { loadOrCreateDeviceIdentity } = await import('../infra/device-identity.js');
    const { approveDevicePairing, getPairedDevice, listDevicePairing, revokeDeviceToken } = await import('../infra/device-pairing.js');
    const { server, ws, port, prevToken } = await startServerWithClient('secret');
    const res = await connectReq(ws, { token: 'secret' });
    if (!res.ok) {
      const list = await listDevicePairing();
