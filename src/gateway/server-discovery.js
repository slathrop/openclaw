/** @module gateway/server-discovery -- Gateway discovery protocol (mDNS/Bonjour) setup. */
...
async function resolveTailnetDnsHint(opts) {
  ...
  const exec = opts?.exec ?? ((command, args) => runExec(command, args, { timeoutMs: 1500, maxBuffer: 2e5 }));
  try {
