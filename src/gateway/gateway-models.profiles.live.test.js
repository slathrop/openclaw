...
function extractPayloadText(result) {
  ...
  const texts = payloads.map((p) => p && typeof p === 'object' ? p.text : void 0).filter((t) => typeof t === 'string' && t.trim().length > 0);
  return texts.join('\n').trim();
}
...
function isToolNonceRefusal(error) {
  ...
  return msg.includes('token') || msg.includes('secret') || msg.includes('local file') || msg.includes('disclose') || msg.includes("can't help") || msg.includes('can\u2019t help') || msg.includes("can't comply") || msg.includes('can\u2019t comply');
}
...
function randomImageProbeCode(len = 6) {
...
function buildLiveGatewayConfig(params) {
  ...
  return {
    ...
    models: Object.keys(providers).length > 0 ? { ...params.cfg.models, providers } : params.cfg.models
  };
}
...
async function runGatewayModelSuite(params) {
  ...
  const sanitizedStore = {
    ...
    order: hostStore.order ? { ...hostStore.order } : void 0,
    lastGood: hostStore.lastGood ? { ...hostStore.lastGood } : void 0,
    usageStats: hostStore.usageStats ? { ...hostStore.usageStats } : void 0
  };
  const tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'openclaw-live-state-'));
  process.env.OPENCLAW_STATE_DIR = tempStateDir;
  const tempAgentDir = path.join(tempStateDir, 'agents', DEFAULT_AGENT_ID, 'agent');
...
`);
  ...
  try {
    ...
    for (const [index, model] of params.candidates.entries()) {
      ...
      const attemptMax = model.provider === 'anthropic' && anthropicKeys.length > 0 ? anthropicKeys.length : 1;
      for (let attempt = 0; attempt < attemptMax; attempt += 1) {
        ...
        try {
          ...
          if (isEmptyStreamText(text) && (model.provider === 'minimax' || model.provider === 'openai-codex')) {
          ...
          const toolProbe = await client.request(
            'agent',
            {
              ...
              message: `OpenClaw live tool probe (local, safe): use the tool named \`read\` (or \`Read\`) with JSON arguments {"path":"${toolProbePath}"}. Then reply with the two nonce values you read (include both).`,
              thinking: params.thinkingLevel,
              deliver: false
            },
            { expectFinal: true }
          ...
          if (isEmptyStreamText(toolText) && (model.provider === 'minimax' || model.provider === 'openai-codex')) {
            logProgress(`${progressLabel}: skip (${model.provider} empty response)`);
            break;
          }
          assertNoReasoningTags({
          ...
          if (params.extraToolProbes) {
            ...
            const execReadProbe = await client.request(
              'agent',
              {
                ...
                message: `OpenClaw live tool probe (local, safe): use the tool named \`exec\` (or \`Exec\`) to run this command: mkdir -p "${tempDir}" && printf '%s' '${nonceC}' > "${toolWritePath}". Then use the tool named \`read\` (or \`Read\`) with JSON arguments {"path":"${toolWritePath}"}. Finally reply including the nonce text you read back.`,
                thinking: params.thinkingLevel,
                deliver: false
              },
              { expectFinal: true }
            ...
            if (isEmptyStreamText(execReadText) && (model.provider === 'minimax' || model.provider === 'openai-codex')) {
              logProgress(`${progressLabel}: skip (${model.provider} empty response)`);
              break;
            }
            assertNoReasoningTags({
          ...
          if (params.extraImageProbes && model.input?.includes('image')) {
            ...
            const imageProbe = await client.request(
              'agent',
              {
                ...
                message: 'Look at the attached image. Reply with exactly two tokens separated by a single space: (1) the animal shown or written in the image, lowercase; (2) the code printed in the image, uppercase. No extra text.',
                attachments: [
                  {
                    mimeType: 'image/png',
            ...
            if (imageProbe?.status !== 'ok') {
              ...
              if (isEmptyStreamText(imageText) && (model.provider === 'minimax' || model.provider === 'openai-codex')) {
          ...
          if (model.provider === 'openai' && model.api === 'openai-responses' || model.provider === 'openai-codex' && model.api === 'openai-codex-responses') {
          ...
          if (model.provider === 'anthropic' && isAnthropicRateLimitError(message) && attempt + 1 < attemptMax) {
          ...
          if (model.provider === 'anthropic' && isEmptyStreamText(message) && attempt + 1 < attemptMax) {
          ...
          if ((model.provider === 'openai' || model.provider === 'openai-codex') && isOpenAIReasoningSequenceError(message)) {
            skippedCount += 1;
            logProgress(`${progressLabel}: skip (openai reasoning sequence error)`);
            break;
          }
          if ((model.provider === 'openai' || model.provider === 'openai-codex') && isToolNonceRefusal(message)) {
            skippedCount += 1;
    ...
    if (failures.length > 0) {
      const preview = failures.slice(0, 20).map((f) => `- ${f.model}: ${f.error}`).join('\n');
...
describeLive('gateway live (dev agent, profile keys)', () => {
  it(
    ...
    async () => {
      ...
      const wanted = filter ? all.filter((m) => filter.has(`${m.provider}/${m.id}`)) : all.filter((m) => isModernModelRef({ provider: m.provider, id: m.id }));
      const candidates = [];
      for (const model of wanted) {
        ...
        try {
          ...
          // Model has no API key configured; skip
        }
      }
