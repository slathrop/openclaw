/** @module gateway/control-ui -- Control UI HTTP handler for serving the web-based gateway dashboard. */
...
function handleControlUiAvatarRequest(req, res, opts) {
  ...
  if (url.searchParams.get('meta') === '1') {
    const resolved2 = opts.resolveAvatar(agentId);
    const avatarUrl = resolved2.kind === 'local' ? buildControlUiAvatarUrl(basePath, agentId) : resolved2.kind === 'remote' || resolved2.kind === 'data' ? resolved2.url : null;
...
function injectControlUiConfig(html, opts) {
  ...
  const script = `<script>window.__OPENCLAW_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};window.__OPENCLAW_ASSISTANT_NAME__=${JSON.stringify(
...
function serveIndexHtml(res, indexPath, opts) {
  ...
  const identity = config ? resolveAssistantIdentity({ cfg: config, agentId }) : DEFAULT_ASSISTANT_IDENTITY;
