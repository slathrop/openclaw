...
const INHERIT_FLAGS = /* @__PURE__ */ new Set(['I', 'OI', 'CI', 'IO', 'NP']);
...
function parseIcaclsOutput(output, targetPath) {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (lower.startsWith('successfully processed') || lower.startsWith('processed') || lower.startsWith('failed processing') || lower.startsWith('no mapping between account names')) {
      continue;
    }
    let entry = trimmed;
