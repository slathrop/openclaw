...
function hasWebSearchKey(cfg, env) {
  ...
  return Boolean(
    search?.apiKey || search?.perplexity?.apiKey || env.BRAVE_API_KEY || env.PERPLEXITY_API_KEY || env.OPENROUTER_API_KEY
  );
}
...
async function collectPluginsTrustFindings(params) {
  ...
  if (!allowConfigured) {
    ...
    const hasAccountStringKey = (account, key) => Boolean(
      account && typeof account === 'object' && hasString(account[key])
    );
    const discordConfigured = hasString(params.cfg.channels?.discord?.token) || Boolean(
      params.cfg.channels?.discord?.accounts && Object.values(params.cfg.channels.discord.accounts).some(
        (a) => hasAccountStringKey(a, 'token')
      )
    ) || hasString(process.env.DISCORD_BOT_TOKEN);
    const telegramConfigured = hasString(params.cfg.channels?.telegram?.botToken) || hasString(params.cfg.channels?.telegram?.tokenFile) || Boolean(
      params.cfg.channels?.telegram?.accounts && Object.values(params.cfg.channels.telegram.accounts).some(
        (a) => hasAccountStringKey(a, 'botToken') || hasAccountStringKey(a, 'tokenFile')
      )
    ) || hasString(process.env.TELEGRAM_BOT_TOKEN);
    const slackConfigured = hasString(params.cfg.channels?.slack?.botToken) || hasString(params.cfg.channels?.slack?.appToken) || Boolean(
      params.cfg.channels?.slack?.accounts && Object.values(params.cfg.channels.slack.accounts).some(
        (a) => hasAccountStringKey(a, 'botToken') || hasAccountStringKey(a, 'appToken')
      )
    ) || hasString(process.env.SLACK_BOT_TOKEN) || hasString(process.env.SLACK_APP_TOKEN);
...
async function collectStateDeepFilesystemFindings(params) {
  ...
  const ids = Array.from(/* @__PURE__ */ new Set([defaultAgentId, ...agentIds])).map((id) => normalizeAgentId(id));
