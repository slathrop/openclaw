...
async function collectChannelSecurityFindings(params) {
  ...
  const warnDmPolicy = async (input) => {
    ...
    const allowCount = Array.from(/* @__PURE__ */ new Set([...normalizedCfg, ...normalizedStore])).length;
  ...
  for (const plugin of params.plugins) {
    ...
    const configured = plugin.config.isConfigured ? await plugin.config.isConfigured(account, params.cfg) : true;
    if (!configured) {
      continue;
    }
    if (plugin.id === 'discord') {
      ...
      const nativeSkillsEnabled = resolveNativeSkillsEnabled({
        providerId: 'discord',
        providerSetting: coerceNativeSetting(
          discordCfg.commands?.nativeSkills
        ),
      ...
      if (slashEnabled) {
        ...
        } else if (useAccessGroups && groupPolicy !== 'disabled' && guildsConfigured && !ownerAllowFromConfigured && !hasAnyUserAllowlist) {
          findings.push({
    ...
    if (plugin.id === 'slack') {
      ...
      const nativeSkillsEnabled = resolveNativeSkillsEnabled({
        providerId: 'slack',
        providerSetting: coerceNativeSetting(
          slackCfg.commands?.nativeSkills
        ),
        globalSetting: params.cfg.commands?.nativeSkills
      });
      const slashCommandEnabled = nativeEnabled || nativeSkillsEnabled || slackCfg.slashCommand?.enabled === true;
    ...
    if (plugin.id === 'telegram') {
      ...
      const anyGroupOverride = Boolean(
        groups && Object.values(groups).some((value) => {
          if (!value || typeof value !== 'object') {
            return false;
          }
          const group = value;
...
async function runSecurityAudit(opts) {
  ...
  const configSnapshot = opts.includeFilesystem !== false ? await readConfigSnapshotForAudit({ env, configPath }).catch(() => null) : null;
