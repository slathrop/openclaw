...
async function chmodCredentialsAndAgentState(params) {
  ...
  for (const agentId of ids) {
    ...
    params.actions.push(
      await params.applyPerms({ path: sessionsDir, mode: 448, require: 'dir' })
    );
...
async function fixSecurityFootguns(opts) {
  ...
  const applyPerms = (params) => isWindows ? safeAclReset({ path: params.path, require: params.require, env, exec }) : safeChmod({ path: params.path, mode: params.mode, require: params.require });
