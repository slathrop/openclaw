...
`,
  ...
  it('flags Discord native commands without a guild user allowlist', async () => {
    ...
    try {
      ...
      if (prevStateDir === null || prevStateDir === undefined) {
...
`, 'utf-8');
    ...
    try {
      ...
      const user = 'DESKTOP-TEST\\Tester';
      const execIcacls = isWindows ? async (_cmd, args) => {
        const target = args[0];
...
`,
      ...
      const res = await runSecurityAudit({
        ...
        stateDir,
        configPath,
        platform: isWindows ? 'win32' : void 0,
        env: isWindows ? { ...process.env, USERNAME: 'Tester', USERDOMAIN: 'DESKTOP-TEST' } : void 0,
        execIcacls
      });
  ...
  it('flags extensions without plugins.allow', async () => {
    ...
    try {
      ...
      if (prevDiscordToken === null || prevDiscordToken === undefined) {
        delete process.env.DISCORD_BOT_TOKEN;
      } else {
        process.env.DISCORD_BOT_TOKEN = prevDiscordToken;
      }
      if (prevTelegramToken === null || prevTelegramToken === undefined) {
        delete process.env.TELEGRAM_BOT_TOKEN;
      } else {
        process.env.TELEGRAM_BOT_TOKEN = prevTelegramToken;
      }
      if (prevSlackBotToken === null || prevSlackBotToken === undefined) {
        delete process.env.SLACK_BOT_TOKEN;
      } else {
        process.env.SLACK_BOT_TOKEN = prevSlackBotToken;
      }
      if (prevSlackAppToken === null || prevSlackAppToken === undefined) {
        delete process.env.SLACK_APP_TOKEN;
      } else {
  ...
  describe('maybeProbeGateway auth selection', () => {
    ...
    afterEach(() => {
      if (originalEnvToken === null || originalEnvToken === undefined) {
        delete process.env.OPENCLAW_GATEWAY_TOKEN;
      } else {
        process.env.OPENCLAW_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword === null || originalEnvPassword === undefined) {
        delete process.env.OPENCLAW_GATEWAY_PASSWORD;
      } else {
