...
describe('sendMessageTelegram', () => {
  ...
  it('retries on transient errors with retry_after', async () => {
    ...
    const sendMessage = vi.fn().mockRejectedValueOnce(err).mockResolvedValueOnce({
      message_id: 1,
      chat: { id: chatId }
    });
    const api = { sendMessage };
...
describe('sendStickerTelegram', () => {
  ...
  it('sends a sticker by file_id', async () => {
    ...
    expect(sendSticker).toHaveBeenCalledWith(chatId, fileId, void 0);
  ...
  it('normalizes chat ids with internal prefixes', async () => {
    ...
    expect(sendSticker).toHaveBeenCalledWith('123', 'fileId123', void 0);
  });
  it('parses message_thread_id from recipient string (telegram:group:...:topic:...)', async () => {
    const chatId = '-1001234567890';
    const sendSticker = vi.fn().mockResolvedValue({
  ...
  it('trims whitespace from fileId', async () => {
    ...
    expect(sendSticker).toHaveBeenCalledWith(chatId, 'fileId123', void 0);
  });
});
