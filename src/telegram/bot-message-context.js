...
const buildTelegramMessageContext = async ({
  ...
  if (!isGroup) {
    ...
    if (dmPolicy !== 'open') {
      ...
      const senderUsername2 = msg.from?.username ?? '';
  ...
  const implicitMention = botId !== null && botId !== undefined && replyFromId === botId;
...
` : '';
  ...
  if (shouldLogVerbose()) {
    ...
    const topicInfo = resolvedThreadId !== null && resolvedThreadId !== undefined ? ` topic=${resolvedThreadId}` : '';
    logVerbose(
