...
describe('resolveTelegramForumThreadId', () => {
  ...
  it('returns undefined for non-forum groups without messageThreadId', () => {
    expect(
      resolveTelegramForumThreadId({ isForum: false, messageThreadId: void 0 })
    ).toBeUndefined();
    expect(
      resolveTelegramForumThreadId({ isForum: void 0, messageThreadId: 99 })
    ).toBeUndefined();
  });
  it('returns General topic (1) for forum groups without messageThreadId', () => {
    expect(resolveTelegramForumThreadId({ isForum: true, messageThreadId: void 0 })).toBe(1);
...
describe('buildTypingThreadParams', () => {
  it('returns undefined when no thread id is provided', () => {
    expect(buildTypingThreadParams(void 0)).toBeUndefined();
  });
