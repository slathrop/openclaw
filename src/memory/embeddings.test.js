/** @module memory/embeddings.test - Tests for embedding providers. */
...
const createFetchMock = () => vi.fn(async () => ({
  ok: true,
  status: 200,
    ...
    vi.doMock('./node-llama.js', () => ({
      importNodeLlamaCpp: async () => ({
        getLlama: async () => ({
          loadModel: vi.fn().mockResolvedValue({
            createEmbeddingContext: vi.fn().mockResolvedValue({
              getEmbeddingFor: vi.fn().mockResolvedValueOnce({ vector: new Float32Array(unnormalizedVectors[0]) }).mockResolvedValueOnce({ vector: new Float32Array(unnormalizedVectors[1]) }).mockResolvedValueOnce({ vector: new Float32Array(unnormalizedVectors[2]) })
            })
          })
        }),
        resolveModelFile: async () => '/fake/model.gguf',
