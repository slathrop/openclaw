/** @module memory/manager.batch.test - Tests for batch embedding processing. */
...
describe('memory indexing with OpenAI batches', () => {
  ...
  beforeEach(async () => {
    ...
    embedBatch.mockImplementation(
      async (texts) => texts.map((_text, index) => [index + 1, 0, 0])
    );
    const realSetTimeout = setTimeout;
    setTimeoutSpy = vi.spyOn(global, 'setTimeout').mockImplementation(((handler, timeout, ...args) => {
      const delay = typeof timeout === 'number' ? timeout : 0;
      if (delay > 0 && delay <= 2e3) {
  ...
  it('uses OpenAI batch uploads when enabled', async () => {
    ...
    const fetchMock = vi.fn(async (input, init) => {
      ...
      if (url.endsWith('/files')) {
        ...
        for (const [key, value] of body.entries()) {
          ...
          if (typeof value === 'string') {
            uploadedRequests = value.split('\n').filter(Boolean).map((line) => JSON.parse(line));
          } else {
            const text = await value.text();
            uploadedRequests = text.split('\n').filter(Boolean).map((line) => JSON.parse(line));
          }
        }
      ...
      if (url.endsWith('/files/file_out/content')) {
        const lines = uploadedRequests.map(
          (request, index) => JSON.stringify({
