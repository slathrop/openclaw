/** @module memory/internal - Internal utilities for file hashing, chunking, and path resolution. */
...
function normalizeExtraMemoryPaths(workspaceDir, extraPaths) {
  ...
  const resolved = extraPaths.map((value) => value.trim()).filter(Boolean).map(
    (value) => path.isAbsolute(value) ? path.resolve(value) : path.resolve(workspaceDir, value)
  );
