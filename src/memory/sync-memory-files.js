/** @module memory/sync-memory-files - Memory file synchronization logic. */
...
async function syncMemoryFiles(params) {
  ...
  const tasks = fileEntries.map((entry) => async () => {
    const record = params.db.prepare('SELECT hash FROM files WHERE path = ? AND source = ?').get(entry.path, 'memory');
  ...
  const staleRows = params.db.prepare('SELECT path FROM files WHERE source = ?').all('memory');
  for (const stale of staleRows) {
    ...
    if (params.ftsEnabled && params.ftsAvailable) {
      try {
        params.db.prepare(`DELETE FROM ${params.ftsTable} WHERE path = ? AND source = ? AND model = ?`).run(stale.path, 'memory', params.model);
      } catch { /* intentionally ignored */ }
    }
  }
}
