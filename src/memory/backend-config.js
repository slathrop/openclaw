/** @module memory/backend-config - Memory backend configuration resolution. */
...
function resolveSessionConfig(cfg, workspaceDir) {
  ...
  const exportDir = exportDirRaw ? resolvePath(exportDirRaw, workspaceDir) : void 0;
  const retentionDays = cfg?.retentionDays && cfg.retentionDays > 0 ? Math.floor(cfg.retentionDays) : void 0;
  return {
    enabled,
    exportDir,
    retentionDays
