/** @module memory/sync-session-files - Session file synchronization logic. */
...
async function syncSessionFiles(params) {
  ...
  const tasks = files.map((absPath) => async () => {
    ...
    const record = params.db.prepare('SELECT hash FROM files WHERE path = ? AND source = ?').get(entry.path, 'sessions');
  ...
  const staleRows = params.db.prepare('SELECT path FROM files WHERE source = ?').all('sessions');
  for (const stale of staleRows) {
    ...
    if (params.ftsEnabled && params.ftsAvailable) {
      try {
        params.db.prepare(`DELETE FROM ${params.ftsTable} WHERE path = ? AND source = ? AND model = ?`).run(stale.path, 'sessions', params.model);
      } catch { /* intentionally ignored */ }
    }
  }
}
