/**
 * Subsystem-scoped logger with console and file output.
 *
 * Creates loggers namespaced by subsystem (e.g. "gateway/channels/discord")
 * that write to both the file logger and formatted console output. Supports
 * color-coded subsystem prefixes, redundant prefix stripping, and child
 * logger creation for hierarchical subsystem trees.
 */
...
/**
 * @typedef {object} SubsystemLogger
 * @property {string} subsystem
 * @property {(message: string, meta?: {[key: string]: unknown}) => void} trace
 * @property {(message: string, meta?: {[key: string]: unknown}) => void} debug
 * @property {(message: string, meta?: {[key: string]: unknown}) => void} info
 * @property {(message: string, meta?: {[key: string]: unknown}) => void} warn
 * @property {(message: string, meta?: {[key: string]: unknown}) => void} error
 * @property {(message: string, meta?: {[key: string]: unknown}) => void} fatal
 * @property {(message: string) => void} raw
 * @property {(name: string) => SubsystemLogger} child
 */

const shouldLogToConsole = (level, settings) => {
...
const pickSubsystemColor = (color, subsystem) => {
...
const formatSubsystemForConsole = (subsystem) => {
 ...
 * @param {string} displaySubsystem
 * @returns {string}
 */
export const stripRedundantSubsystemPrefixForConsole = (message, displaySubsystem) => {
  if (!displaySubsystem) {
    return message;
  }
...
const formatConsoleLine = (opts) => {
  ...
  const prefixStr = `[${displaySubsystem}]`;
...
const logToFile = (fileLogger, level, message, meta) => {
  if (level === 'silent') {
    return;
  }
 ...
 * @returns {SubsystemLogger}
 */
export const createSubsystemLogger = (subsystem) => {
  let fileLogger = null;
  const getFileLogger = () => {
    if (!fileLogger) {
...
/**
 * @param {SubsystemLogger} logger
 * @param {Function} [exit]
 * @returns {object}
 */
export const runtimeForLogger = (logger, exit = defaultRuntime.exit) => {
  return {
 ...
 * @returns {object}
 */
export const createSubsystemRuntime = (subsystem, exit = defaultRuntime.exit) => {
