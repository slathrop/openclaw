...
describe('tui-event-handlers: handleAgentEvent', () => {
  ...
  it('captures runId from chat events when activeChatRunId is unset', () => {
    ...
    expect(chatLog.startTool).toHaveBeenCalledWith('tc1', 'exec', void 0);
  });
  ...
  it('accepts tool events after chat final for the same run', () => {
    ...
    expect(chatLog.startTool).toHaveBeenCalledWith('tc-final', 'session_status', void 0);
  ...
  it('refreshes history after a non-local chat final', () => {
    const state = makeState({ activeChatRunId: null });
    const { chatLog, tui, setActivityStatus, loadHistory, isLocalRunId, forgetLocalRunId } = makeContext(state);
