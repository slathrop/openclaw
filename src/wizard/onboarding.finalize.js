...
async function finalizeOnboardingWizard(options) {
  ...
  const withWizardProgress = async (label, options2, work) => {
    const progress = prompter.progress(label);
    try {
      return await work(progress);
    } finally {
      progress.stop(options2.doneMessage);
    }
  };
  ...
  let installDaemon;
  if (explicitInstallDaemon !== void 0) {
  ...
  const shouldOpenControlUi = !opts.skipUi && settings.authMode === 'token' && Boolean(settings.gatewayToken) && hatchChoice === null;
  ...
  await prompter.outro(
    controlUiOpened ? 'Onboarding complete. Dashboard opened with your token; keep that tab to control OpenClaw.' : seededInBackground ? 'Onboarding complete. Web UI seeded in the background; open it anytime with the tokenized link above.' : 'Onboarding complete. Use the tokenized dashboard link above to control OpenClaw.'
  );
  return { launchedTui };
}
export {
