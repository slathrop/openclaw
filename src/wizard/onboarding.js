...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
  ...
  const explicitFlow = normalizedExplicitFlow === 'quickstart' || normalizedExplicitFlow === 'advanced' ? normalizedExplicitFlow : void 0;
  ...
  if (mode === 'remote') {
    let nextConfig2 = await promptRemoteGatewayConfig(baseConfig, prompter);
    nextConfig2 = applyWizardMetadata(nextConfig2, { command: 'onboard', mode });
  ...
  const authStore = ensureAuthProfileStore(void 0, {
