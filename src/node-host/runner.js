...
const blockedEnvKeys = /* @__PURE__ */ new Set([
  'NODE_OPTIONS',
  'PYTHONHOME',
  'PYTHONPATH',
  'PERL5LIB',
...
class SkillBinsCache {
  ...
  async current(force = false) {
    if (force || Date.now() - this._lastRefresh > this._ttlMs) {
      await this.refresh();
    }
    return this._bins;
  }
  async refresh() {
    try {
      const bins = await this._fetch();
      this._bins = new Set(bins);
      this._lastRefresh = Date.now();
    } catch {
      if (!this._lastRefresh) {
        this._bins = /* @__PURE__ */ new Set();
      }
    }
  }
}
...
async function handleSystemWhich(params, env) {
  const bins = params._bins.map((bin) => bin.trim()).filter(Boolean);
  const found = {};
  for (const bin of bins) {
    const path2 = resolveExecutable(bin, env);
    if (path2) {
      found[bin] = path2;
    }
  }
...
async function runNodeHost(opts) {
  ...
  const skillBins = new SkillBinsCache(async () => {
    const res = await client.request('skills._bins', {});
    const bins = Array.isArray(res?._bins) ? res._bins.map((bin) => String(bin)) : [];
    return bins;
  });
  client.start();
  await new Promise(() => {
...
async function handleInvoke(frame, client, skillBins) {
  ...
  if (command === 'system.execApprovals.set') {
    try {
      const params2 = decodeParams(frame.paramsJSON);
      if (!params2.file || typeof params2.file !== 'object') {
        throw new Error('INVALID_REQUEST: exec approvals file required');
      }
      ensureExecApprovals();
      const snapshot = readExecApprovalsSnapshot();
      requireExecApprovalsBaseHash(params2, snapshot);
      const normalized = normalizeExecApprovals(params2.file);
  ...
  if (command === 'system.which') {
    try {
      const params2 = decodeParams(frame.paramsJSON);
      if (!Array.isArray(params2._bins)) {
        throw new Error('INVALID_REQUEST: bins required');
      }
      const env2 = sanitizeEnv(void 0);
      const payload = await handleSystemWhich(params2, env2);
      await sendInvokeResult(client, frame, {
        ok: true,
        payloadJSON: JSON.stringify(payload)
      });
  ...
  if (command === 'browser.proxy') {
    try {
      const params2 = decodeParams(frame.paramsJSON);
      const pathValue = typeof params2.path === 'string' ? params2.path.trim() : '';
      if (!pathValue) {
      ...
      const resolved = resolveBrowserConfig(cfg2.browser, cfg2);
      const requestedProfile = typeof params2.profile === 'string' ? params2.profile.trim() : '';
      ...
      const method = typeof params2.method === 'string' ? params2.method.toUpperCase() : 'GET';
      const path2 = pathValue.startsWith('/') ? pathValue : `/${pathValue}`;
      const body = params2.body;
      const query = {};
      if (requestedProfile) {
        query.profile = requestedProfile;
      }
      const rawQuery = params2.query ?? {};
      ...
      if (allowedProfiles.length > 0 && path2 === '/profiles') {
        const obj = typeof result2 === 'object' && result2 !== null ? result2 : {};
      ...
      let files;
      const paths = collectBrowserProxyPaths(result2);
      if (paths.length > 0) {
      ...
      const payload = files ? { result: result2, files } : { result: result2 };
      await sendInvokeResult(client, frame, {
        ok: true,
        payloadJSON: JSON.stringify(payload)
      });
  ...
  if (rawCommand) {
    ...
    const analysis = analyzeArgvCommand({ argv, cwd: params.cwd ?? void 0, env });
  ...
  if (useMacAppExec) {
    const approvalDecision2 = params.approvalDecision === 'allow-once' || params.approvalDecision === 'allow-always' ? params.approvalDecision : null;
    const execRequest = {
      command: argv,
    ...
    if (!response) {
      ...
      const result2 = response.payload;
      const combined2 = [result2.stdout, result2.stderr, result2.error].filter(Boolean).join('\n');
      await sendNodeEvent(
        client,
        'exec.finished',
        buildExecEventPayload({
      ...
      await sendInvokeResult(client, frame, {
        ok: true,
        payloadJSON: JSON.stringify(result2)
      });
      return;
    }
  }
  ...
  let execArgv = argv;
  if (security === 'allowlist' && isWindows && !approvedByAsk && rawCommand && analysisOk && allowlistSatisfied && segments.length === 1 && segments[0]?.argv.length > 0) {
    execArgv = segments[0].argv;
  }
  const result = await runCommand(
    execArgv,
    params.cwd?.trim() || void 0,
    env,
    params.timeoutMs ?? void 0
  );
  if (result.truncated) {
