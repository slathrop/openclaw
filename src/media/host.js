...
const TTL_MS = 2 * 60 * 1e3;
let mediaServer = null;
async function ensureMediaHosted(source, opts = {}) {
  ...
  if (needsServerStart && opts.startServer) {
    if (!mediaServer) {
      ...
      logInfo(
        `\u{1F99E} Started temporary media host on http://localhost:${port}/media/:id (TTL ${TTL_MS / 1e3}s)`,
        runtime
      );
      mediaServer.unref?.();
    }
