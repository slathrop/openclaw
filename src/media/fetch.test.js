...
describe('fetchRemoteMedia', () => {
  it('rejects when content-length exceeds maxBytes', async () => {
    ...
    const fetchImpl = async () => new Response(makeStream([new Uint8Array([1, 2, 3, 4, 5])]), {
      status: 200,
  ...
  it('rejects when streamed payload exceeds maxBytes', async () => {
    const lookupFn = vi.fn(async () => [{ address: '93.184.216.34', family: 4 }]);
    const fetchImpl = async () => new Response(makeStream([new Uint8Array([1, 2, 3]), new Uint8Array([4, 5, 6])]), {
      status: 200
    });
    await expect(
      fetchRemoteMedia({
