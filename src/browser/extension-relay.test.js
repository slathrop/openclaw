...
function createMessageQueue(ws) {
  ...
  const next = (timeoutMs = 5e3) => new Promise((resolve, reject) => {
...
async function waitForListMatch(fetchList, predicate, timeoutMs = 2e3, intervalMs = 50) {
...
describe('chrome extension relay server', () => {
  ...
  it('tracks attached page targets and exposes them via CDP + /json/list', async () => {
    ...
    const list2 = await waitForListMatch(
      async () => await fetch(`${cdpUrl}/json/list`, {
        headers: relayAuthHeaders(cdpUrl)
      }).then((r) => r.json()),
      (list3) => list3.some(
