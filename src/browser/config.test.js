...
describe('browser config', () => {
  it('defaults to enabled with loopback defaults and lobster-orange color', () => {
    const resolved = resolveBrowserConfig(void 0);
    ...
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3e3);
  });
  ...
  it('derives default ports from gateway.port when env is unset', () => {
    ...
    delete process.env.OPENCLAW_GATEWAY_PORT;
    try {
      const resolved = resolveBrowserConfig(void 0, { gateway: { port: 19011 } });
  ...
  it('supports custom remote CDP timeouts', () => {
    const resolved = resolveBrowserConfig({
      ...
      remoteCdpHandshakeTimeoutMs: 5e3
    });
    expect(resolved.remoteCdpTimeoutMs).toBe(2200);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(5e3);
  });
