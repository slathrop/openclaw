...
import {
  ...
  resolveBrowserExecutableForPlatform as resolveBrowserExecutableForPlatform2
} from './chrome.executables.js';
import {
  decorateOpenClawProfile as decorateOpenClawProfile2,
  ensureProfileCleanExit as ensureProfileCleanExit2,
  isProfileDecorated as isProfileDecorated2
} from './chrome.profile-decoration.js';
...
async function launchOpenClawChrome(resolved, profile) {
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`\u{1F99E} openclaw browser profile decorated (${profile.color})`);
    } catch (err) {
  ...
  log.info(
    `\u{1F99E} openclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`
  );
  return {
    pid,
    exe,
...
export {
  decorateOpenClawProfile2 as decorateOpenClawProfile,
  ensureProfileCleanExit2 as ensureProfileCleanExit,
  findChromeExecutableLinux,
  findChromeExecutableMac,
  findChromeExecutableWindows,
  ...
  isChromeCdpReady,
  isChromeReachable,
  isProfileDecorated2 as isProfileDecorated,
  launchOpenClawChrome,
  resolveBrowserExecutableForPlatform2 as resolveBrowserExecutableForPlatform,
