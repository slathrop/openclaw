...
describe('browser screenshot normalization', () => {
  it('shrinks oversized images to <=2000x2000 and <=5MB', async () => {
    ...
    const bigPng = await sharp(raw, { raw: { width, height, channels: 3 } }).png({ compressionLevel: 0 }).toBuffer();
    ...
    expect(Number(meta.width)).toBeLessThanOrEqual(2e3);
    expect(Number(meta.height)).toBeLessThanOrEqual(2e3);
