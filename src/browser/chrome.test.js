...
describe('browser chrome helpers', () => {
  ...
  it('picks the first existing Chrome candidate on macOS', () => {
    const exists = vi.spyOn(fs, 'existsSync').mockImplementation(
      (p) => String(p).includes('Google Chrome.app/Contents/MacOS/Google Chrome')
    );
  ...
  it('picks the first existing Chrome candidate on Windows', () => {
    ...
    const exists = vi.spyOn(fs, 'existsSync').mockImplementation((p) => {
      ...
      return pathStr.includes('Google\\Chrome\\Application\\chrome.exe') || pathStr.includes('BraveSoftware\\Brave-Browser\\Application\\brave.exe') || pathStr.includes('Microsoft\\Edge\\Application\\msedge.exe');
    });
