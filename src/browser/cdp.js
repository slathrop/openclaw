...
import { appendCdpPath as appendCdpPath2, fetchJson as fetchJson2, fetchOk, getHeadersWithAuth } from './cdp.helpers.js';
...
async function snapshotDom(opts) {
  const limit = Math.max(1, Math.min(5e3, Math.floor(opts.limit ?? 800)));
  const maxTextChars = Math.max(0, Math.min(5e3, Math.floor(opts.maxTextChars ?? 220)));
  const expression = `(() => {
...
async function getDomText(opts) {
  const maxChars = Math.max(0, Math.min(5e6, Math.floor(opts.maxChars ?? 2e5)));
  ...
  const text = typeof textValue === 'string' ? textValue : typeof textValue === 'number' || typeof textValue === 'boolean' ? String(textValue) : '';
  return { text };
}
async function querySelector(opts) {
  const limit = Math.max(1, Math.min(200, Math.floor(opts.limit ?? 20)));
  const maxText = Math.max(0, Math.min(5e3, Math.floor(opts.maxTextChars ?? 500)));
  const maxHtml = Math.max(0, Math.min(2e4, Math.floor(opts.maxHtmlChars ?? 1500)));
  const expression = `(() => {
...
export {
  appendCdpPath2 as appendCdpPath,
  captureScreenshot,
  captureScreenshotPng,
  createTargetViaCdp,
  evaluateJavaScript,
  fetchJson2 as fetchJson,
  fetchOk,
  formatAriaSnapshot,
  getDomText,
  getHeadersWithAuth,
