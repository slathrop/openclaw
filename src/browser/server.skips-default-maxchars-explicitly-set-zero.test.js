...
describe('browser control server', () => {
  ...
  it('skips default maxChars when explicitly set to zero', async () => {
    ...
    const snapAi = await realFetch(`${base}/snapshot?format=ai&maxChars=0`).then(
      (r) => r.json()
    );
  ...
  it('validates agent inputs (agent routes)', async () => {
    ...
    const snapDefault = await realFetch(`${base}/snapshot?format=wat`).then((r) => r.json());
  ...
  it('allows attachOnly servers to ensure reachability via callback', async () => {
    ...
    const status = await realFetch(`${bridge.baseUrl}/`).then((r) => r.json());
