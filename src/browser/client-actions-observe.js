...
async function browserPageErrors(baseUrl, opts = {}) {
  ...
  const suffix = q.toString() ? `?${q.toString()}` : '';
  return await fetchBrowserJson(withBaseUrl(baseUrl, `/errors${suffix}`), { timeoutMs: 2e4 });
}
async function browserRequests(baseUrl, opts = {}) {
  ...
  const suffix = q.toString() ? `?${q.toString()}` : '';
  return await fetchBrowserJson(withBaseUrl(baseUrl, `/requests${suffix}`), { timeoutMs: 2e4 });
}
