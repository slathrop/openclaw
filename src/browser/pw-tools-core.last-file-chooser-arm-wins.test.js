...
describe('pw-tools-core', () => {
  ...
  it('last file-chooser arm wins', async () => {
    ...
    const waitForEvent = vi.fn().mockImplementationOnce(
      () => new Promise((r) => {
        resolve1 = r;
      })
  ...
  it('arms the next dialog and accepts/dismisses (default timeout)', async () => {
    ...
    await Promise.resolve();
    expect(waitForEvent).toHaveBeenCalledWith('dialog', { timeout: 12e4 });
  ...
  it('waits for selector, url, load state, and function', async () => {
    ...
    expect(currentPage.locator).toHaveBeenCalledWith('#main');
