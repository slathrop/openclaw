...
function createPage(opts) {
  const session = {
    ...
    detach: vi.fn().mockResolvedValue(void 0)
  };
  const context = {
    newCDPSession: vi.fn().mockResolvedValue(session)
  };
  const click = vi.fn().mockResolvedValue(void 0);
  const dblclick = vi.fn().mockResolvedValue(void 0);
  const fill = vi.fn().mockResolvedValue(void 0);
