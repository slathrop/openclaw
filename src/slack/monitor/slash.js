...
function registerSlackMonitorSlashCommands(params) {
    ...
    try {
      ...
      if (isDirectMessage) {
        ...
        if (ctx.dmPolicy !== 'open') {
          const sender2 = await ctx.resolveUserName(command.user_id);
          const senderName2 = sender2?.name ?? void 0;
          ...
          if (!allowMatch.allowed) {
            if (ctx.dmPolicy === 'pairing') {
              const { code, created } = await upsertChannelPairingRequest({
                channel: 'slack',
                id: command.user_id,
                meta: { name: senderName2 }
              });
              if (created) {
                logVerbose(
                  `slack pairing request sender=${command.user_id} name=${senderName2 ?? 'unknown'} (${allowMatchMeta})`
                );
                await respond({
  ...
  const nativeCommands = nativeEnabled ? listNativeCommandSpecsForConfig(cfg, { skillCommands, provider: 'slack' }) : [];
  if (nativeCommands.length > 0) {
    for (const command of nativeCommands) {
      ctx.app.command(
        `/${command.name}`,
        async ({ command: cmd, ack, respond }) => {
