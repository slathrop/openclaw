...
async function prepareSlackMessage(params) {
    ...
    if (ctx.dmPolicy !== 'open') {
      ...
      if (!allowMatch.allowed) {
        if (ctx.dmPolicy === 'pairing') {
          const sender2 = await ctx.resolveUserName(directUserId);
          ...
          if (created) {
            logVerbose(
              `slack pairing request sender=${directUserId} name=${senderName2 ?? 'unknown'} (${allowMatchMeta})`
            );
            try {
  ...
  const implicitMention = Boolean(
    !isDirectMessage && ctx.botUserId && message.thread_ts && message.parent_user_id === ctx.botUserId
  );
