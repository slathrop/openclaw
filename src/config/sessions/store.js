 ...
 * Session store persistence with caching, locking, and atomic writes.
 * SECURITY: File permissions set to 0o600 for session data.
 */
...
/**
 * Loads the session store from disk, using a TTL-based cache.
 * @param {string} storePath
 * @param {{ skipCache?: boolean }} [opts]
 * @returns {Record<string, import('./types.js').SessionEntry>}
 */
...
/**
 * @param {string} storePath
 * @param {Record<string, import('./types.js').SessionEntry>} store
...
/**
 * Atomically updates the session store with a mutator function.
 * @param {string} storePath
 * @param {(store: Record<string, import('./types.js').SessionEntry>) => Promise<T> | T} mutator
 * @returns {Promise<T>}
 * @template T
 */
 ...
 * @param {string} storePath
 * @param {() => Promise<T>} fn
 * @param {{ timeoutMs?: number, pollIntervalMs?: number, staleMs?: number }} [opts]
 * @returns {Promise<T>}
 * @template T
 */
...
/**
 * Updates a single session store entry atomically.
 * @param {{ storePath: string, sessionKey: string, update: (entry: object) => Promise<object | null> }} params
...
/**
 * Records session metadata from an inbound message.
 * @param {{ storePath: string, sessionKey: string, ctx: object, groupResolution?: object | null, createIfMissing?: boolean }} params
 * @returns {Promise<object | null>}
 */
...
/**
 * Updates the last delivery route for a session.
 * @param {{ storePath: string, sessionKey: string, channel?: string, to?: string, accountId?: string, threadId?: string | number, deliveryContext?: object, ctx?: object, groupResolution?: object | null }} params
