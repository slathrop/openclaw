/**
 * Legacy config migrations -- Part 2: agent and routing restructuring.
 *
 * Handles the complex agent model configuration migration and routing
 * structure moves:
 * - agent.model/allowedModels/modelAliases/modelFallbacks -> agents.defaults.model + models
 * - routing.agents/defaultAgentId -> agents.list
 * - routing.bindings/groupChat/queue/agentToAgent/transcribeAudio -> respective new locations
 *
 * Migration strategy: preserves existing values when conflicts are detected,
 * merges missing keys from legacy config into new structure.
 */
import {
  ensureAgentEntry,
  ensureRecord,
