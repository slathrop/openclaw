/**
 * Core/shared Zod validation schemas for config.
 *
 * Defines model, queue, DM policy, group policy, TTS, media understanding,
 * tool links, markdown, and other foundational schemas shared across the
 * config system.
 *
 * SECURITY: Contains normalizeAllowFrom/requireOpenAllowFrom helpers that
 * enforce DM open-policy wildcard requirements across all channel providers.
 */
import { z } from 'zod';
...
/** @param {Array<string | number>} [values] @returns {string[]} */
export const normalizeAllowFrom = (values) =>
  (values ?? []).map((v) => String(v).trim()).filter(Boolean);

/**
 * Validate that open DM policy has wildcard in allowFrom.
 * @param {object} params
 * @param {string} [params.policy]
 * @param {Array<string | number>} [params.allowFrom]
 * @param {import('zod').RefinementCtx} params.ctx
 * @param {Array<string | number>} params.path
