/**
 * Config schema: Zod-based schema composition for the OpenClaw config UI.
 *
 * Generates JSON Schema from the Zod schema, builds UI hints (labels, help text,
 * placeholders, sensitivity markers) for every config field, and merges
 * plugin/channel schemas and hints at runtime.
 */
...
function isSensitivePath(fieldPath) {
  return SENSITIVE_PATTERNS.some((pattern) => pattern.test(fieldPath));
}
...
function buildBaseHints() {
  ...
  for (const [hintPath, label] of Object.entries(FIELD_LABELS)) {
    const current = hints[hintPath];
    hints[hintPath] = current ? { ...current, label } : { label };
  }
  for (const [hintPath, help] of Object.entries(FIELD_HELP)) {
    const current = hints[hintPath];
    hints[hintPath] = current ? { ...current, help } : { help };
  }
  for (const [hintPath, placeholder] of Object.entries(FIELD_PLACEHOLDERS)) {
    const current = hints[hintPath];
    hints[hintPath] = current ? { ...current, placeholder } : { placeholder };
  }
  return hints;
}

/**
 * @param {Record<string, object>} hints
 * @returns {Record<string, object>}
 */
...
/**
 * @param {Record<string, object>} hints
 * @param {Array<{id: string, name?: string, description?: string, configUiHints?: Record<string, object>, configSchema?: object}>} plugins
 * @returns {Record<string, object>}
 */
...
/**
 * @param {Record<string, object>} hints
 * @param {Array<{id: string, label?: string, description?: string, configSchema?: object, configUiHints?: Record<string, object>}>} channels
 * @returns {Record<string, object>}
 */
...
/**
 * @param {object} schema
 * @param {Array<{id: string, configSchema?: object}>} plugins
 * @returns {object}
 */
...
/**
 * @param {object} schema
 * @param {Array<{id: string, configSchema?: object}>} channels
 * @returns {object}
 */
...
/**
 * @returns {{ schema: object, uiHints: Record<string, object>, version: string, generatedAt: string }}
 */
...
/**
 * Builds the full config schema with optional plugin and channel extensions.
 * @param {{ plugins?: Array<object>, channels?: Array<object> }} [params]
 * @returns {{ schema: object, uiHints: Record<string, object>, version: string, generatedAt: string }}
 */
