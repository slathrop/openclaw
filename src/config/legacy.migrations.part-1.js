/**
 * Legacy config migrations -- Part 1: structural moves.
 *
 * Handles the initial wave of config restructuring:
 * - bindings[].match.provider -> .channel renames
 * - bindings[].match.accountID -> .accountId casing fix
 * - session.sendPolicy.rules provider -> channel migration
 * - messages.queue.byProvider -> .byChannel rename
 * - Top-level provider sections (whatsapp/telegram/etc) -> channels.*
 * - routing.allowFrom -> channels.whatsapp.allowFrom
 * - routing.groupChat.requireMention -> per-channel groups defaults
 * - gateway.token -> gateway.auth.token
 * - telegram.requireMention -> channels.telegram.groups.*.requireMention
 *
 * Migration strategy: ordered rule application with backward compatibility.
 * Each migration checks for existing values before overwriting.
 */
import {
  ensureRecord,
  getRecord,
...
export const LEGACY_CONFIG_MIGRATIONS_PART_1 = [
  ...
  },
  {
    ...
    apply: (raw, changes) => {
      ...
      const groupChat =
        routing.groupChat && typeof routing.groupChat === 'object'
          ? routing.groupChat
          : null;
      if (!groupChat) {
        return;
