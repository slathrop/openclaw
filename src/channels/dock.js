...
const DOCKS = {
  ...
  whatsapp: {
    ...
    threading: {
      buildToolContext: ({ context, hasRepliedRef }) => {
        const channelId = context.From?.trim() || context.To?.trim() || void 0;
        return {
  ...
  googlechat: {
    ...
    config: {
      resolveAllowFrom: ({ cfg, accountId }) => {
        ...
        const account = channel?.accounts?.[normalized] ?? channel?.accounts?.[Object.keys(channel?.accounts ?? {}).find(
      ...
      formatAllowFrom: ({ allowFrom }) => allowFrom.map((entry) => String(entry).trim()).filter(Boolean).map(
        (entry) => entry.replace(/^(googlechat|google-chat|gchat):/i, '').replace(/^user:/i, '').replace(/^users\//i, '').toLowerCase()
      )
    },
    groups: {
  ...
  signal: {
    ...
    threading: {
      buildToolContext: ({ context, hasRepliedRef }) => {
        ...
        const channelId = (isDirect ? context.From ?? context.To : context.To)?.trim() || void 0;
        return {
          currentChannelId: channelId,
          currentThreadTs: context.ReplyToId,
          hasRepliedRef
  ...
  imessage: {
    ...
    outbound: { textChunkLimit: 4e3 },
    config: {
      resolveAllowFrom: ({ cfg, accountId }) => (resolveIMessageAccount({ cfg, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry)
      ),
