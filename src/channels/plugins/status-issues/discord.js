...
function readDiscordApplicationSummary(value) {
  ...
  return {
    intents: {
      messageContent: intentsRaw.messageContent === 'enabled' || intentsRaw.messageContent === 'limited' || intentsRaw.messageContent === 'disabled' ? intentsRaw.messageContent : void 0
    }
  };
}
function readDiscordPermissionsAuditSummary(value) {
  ...
  const channels = Array.isArray(channelsRaw) ? channelsRaw.map((entry) => {
    if (!isRecord(entry)) {
      return null;
    }
