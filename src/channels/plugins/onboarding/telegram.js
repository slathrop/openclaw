...
function setTelegramDmPolicy(cfg, dmPolicy2) {
  const allowFrom = dmPolicy2 === 'open' ? addWildcardAllowFrom(cfg.channels?.telegram?.allowFrom) : void 0;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
const telegramOnboardingAdapter = {
  channel,
  getStatus: async ({ cfg }) => {
    const configured = listTelegramAccountIds(cfg).some(
      (accountId) => Boolean(resolveTelegramAccount({ cfg, accountId }).token)
    );
    return {
      channel,
      configured,
