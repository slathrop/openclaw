...
async function promptWhatsAppAllowFrom(cfg, _runtime, prompter, options) {
  ...
  if (options?.forceAllowlist) {
    ...
    const merged = [
      ...existingAllowFrom.filter((item) => item !== '*').map((item) => normalizeE164(item)).filter(Boolean),
      normalized
    ];
    const unique = [...new Set(merged.filter(Boolean))];
    let next2 = setWhatsAppSelfChatMode(cfg, true);
    next2 = setWhatsAppDmPolicy(next2, 'allowlist');
    next2 = setWhatsAppAllowFrom(next2, unique);
    await prompter.note(
  ...
  if (mode === 'keep') {
    // Intentionally keep current allowFrom unchanged
  } else if (mode === 'unset') {
    ...
    const allowRaw = await prompter.text({
      ...
      validate: (value) => {
        ...
        for (const part of parts2) {
          ...
          const normalized2 = normalizeE164(part);
          if (!normalized2) {
    ...
    const parts = String(allowRaw).split(/[\n,;]+/g).map((p) => p.trim()).filter(Boolean);
