...
function setSignalDmPolicy(cfg, dmPolicy2) {
  const allowFrom = dmPolicy2 === 'open' ? addWildcardAllowFrom(cfg.channels?.signal?.allowFrom) : void 0;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
async function promptSignalAllowFrom(params) {
  ...
  const entry = await params.prompter.text({
    ...
    validate: (value) => {
      ...
      const parts2 = parseSignalAllowFromInput(raw);
