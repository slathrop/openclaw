...
const channel = 'slack';
function setSlackDmPolicy(cfg, dmPolicy2) {
  const allowFrom = dmPolicy2 === 'open' ? addWildcardAllowFrom(cfg.channels?.slack?.dm?.allowFrom) : void 0;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
async function promptSlackAllowFrom(params) {
  ...
  while (true) {
    ...
    if (!token) {
      ...
      return setSlackAllowFrom(params.cfg, unique2);
    }
...
const slackOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    ...
    const canUseEnv = allowEnv && Boolean(process.env.SLACK_BOT_TOKEN?.trim()) && Boolean(process.env.SLACK_APP_TOKEN?.trim());
        ...
        if (accountWithTokens.botToken && accessConfig.entries.length > 0) {
          try {
            ...
            if (resolvedKeys.length > 0 || unresolved.length > 0) {
              await prompter.note(
                [
                  resolvedKeys.length > 0 ? `Resolved: ${resolvedKeys.join(', ')}` : void 0,
