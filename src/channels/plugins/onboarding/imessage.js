...
function setIMessageDmPolicy(cfg, dmPolicy2) {
  const allowFrom = dmPolicy2 === 'open' ? addWildcardAllowFrom(cfg.channels?.imessage?.allowFrom) : void 0;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
async function promptIMessageAllowFrom(params) {
  ...
  const entry = await params.prompter.text({
    ...
    validate: (value) => {
      ...
      const parts2 = parseIMessageAllowFromInput(raw);
...
const imessageOnboardingAdapter = {
  ...
  getStatus: async ({ cfg }) => {
    const configured = listIMessageAccountIds(cfg).some((accountId) => {
      ...
      return Boolean(
        account.config.cliPath || account.config.dbPath || account.config.allowFrom || account.config.service || account.config.region
      );
    });
