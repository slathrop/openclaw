...
describe('tts', () => {
  ...
  describe('parseTtsDirectives', () => {
    it('extracts overrides and strips directives when enabled', () => {
      ...
      const input = 'Hello [[tts:provider=elevenlabs voiceId=pMsXgVXv3BLzUgSXRplE stability=0.4 speed=1.1]] world\n\n[[tts:text]](laughs) Read the song once more.[[/tts:text]]';
  ...
  describe('summarizeText', () => {
    ...
    it('summarizes text and returns result with metrics', async () => {
      ...
      expect(result.inputLength).toBe(2e3);
    ...
    it('uses summaryModel override when configured', async () => {
      ...
      expect(resolveModel).toHaveBeenCalledWith('openai', 'gpt-4.1-mini', void 0, cfg);
    });
  ...
  describe('getTtsProvider', () => {
    ...
    const restoreEnv = (snapshot) => {
      const keys = ['OPENAI_API_KEY', 'ELEVENLABS_API_KEY', 'XI_API_KEY'];
