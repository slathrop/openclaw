...
const TEMP_FILE_CLEANUP_DELAY_MS = 5 * 60 * 1e3;
...
const TELEPHONY_OUTPUT = {
  openai: { format: 'pcm', sampleRate: 24e3 },
...
function buildTtsSystemPromptHint(cfg) {
  ...
  const autoHint = autoMode === 'inbound' ? "Only use TTS when the user's last message includes audio/voice." : autoMode === 'tagged' ? 'Only use TTS when you include [[tts]] or [[tts:text]] tags.' : void 0;
  return [
...
function parseTtsDirectives(text, policy) {
  ...
  cleanedText = cleanedText.replace(blockRegex, (_match, inner) => {
    hasDirective = true;
    if (policy.allowText && overrides.ttsText === null || overrides.ttsText === undefined) {
...
${text}
      ...
      const summary = res.content.filter(isTextContentBlock).map((block) => block.text.trim()).filter(Boolean).join(' ').trim();
      if (!summary) {
  ...
  for (const provider2 of providers) {
    ...
    try {
      if (provider2 === 'edge') {
        ...
        const tempDir2 = mkdtempSync(path.join(tmpdir(), 'tts-'));
        ...
        const attemptEdgeTts = async (outputFormat) => {
          ...
          const audioPath2 = path.join(tempDir2, `voice-${Date.now()}${extension}`);
          await edgeTTS({
            text: params.text,
            outputPath: audioPath2,
            config: {
              ...config.edge,
              outputFormat
            },
          ...
          return { audioPath: audioPath2, outputFormat };
        };
        let edgeResult;
        try {
      ...
      const apiKey = resolveTtsApiKey(config, provider2);
      if (!apiKey) {
        lastError = `No API key for ${provider2}`;
        continue;
      }
      let audioBuffer;
      if (provider2 === 'elevenlabs') {
      ...
      return {
        ...
        latencyMs,
        provider: provider2,
        outputFormat: provider2 === 'openai' ? output.openai : output.elevenlabs,
        voiceCompatible: output.voiceCompatible
      };
    } catch (err) {
      const error = err;
      if (error.name === 'AbortError') {
        lastError = `${provider2}: request timed out`;
      } else {
        lastError = `${provider2}: ${error.message}`;
      }
    }
  }
  return {
...
async function textToSpeechTelephony(params) {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (provider === 'elevenlabs') {
        const output2 = TELEPHONY_OUTPUT.elevenlabs;
