...
describe('CronService', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(/* @__PURE__ */ new Date('2025-12-13T00:00:00.000Z'));
  ...
  it('runs a one-shot main job and disables it after success when requested', async () => {
    ...
    vi.setSystemTime(/* @__PURE__ */ new Date('2025-12-13T00:00:02.000Z'));
  ...
  it('wakeMode now waits for heartbeat completion when available', async () => {
    ...
    const job = await cron.add({
      ...
      enabled: true,
      schedule: { kind: 'at', at: (/* @__PURE__ */ new Date(1)).toISOString() },
      sessionTarget: 'main',
      wakeMode: 'now',
      payload: { kind: 'systemEvent', text: 'hello' }
    });
  ...
  it('runs an isolated job and posts summary to main', async () => {
    ...
    vi.setSystemTime(/* @__PURE__ */ new Date('2025-12-13T00:00:01.000Z'));
    await vi.runOnlyPendingTimersAsync();
    await cron.list({ includeDisabled: true });
  ...
  it('rejects unsupported session/payload combinations', async () => {
    ...
    await expect(
      cron.add({
        ...
        enabled: true,
        schedule: { kind: 'every', everyMs: 1e3 },
        sessionTarget: 'main',
