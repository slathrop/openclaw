/**
 * ANSI-safe table renderer with flexible column widths.
 *
 * Renders data as Unicode or ASCII box-drawing tables with support for
 * column alignment, flex sizing, min/max width constraints, and ANSI-aware
 * word wrapping that preserves SGR and OSC-8 escape sequences.
...
/**
 * @typedef {object} RenderTableOptions
 * @property {TableColumn[]} columns
 * @property {Array<{[key: string]: string}>} rows
 * @property {number} [width]
 * @property {number} [padding]
 * @property {"unicode" | "ascii" | "none"} [border]
 */
...
const wrapLine = (text, width) => {
  ...
  const flushAt = (breakAt) => {
    ...
    if (breakAt === null || breakAt === undefined || breakAt <= 0) {
...
/**
 * @param {RenderTableOptions} opts
 * @returns {string}
 */
export const renderTable = (opts) => {
  ...
  if (maxWidth && total > maxWidth) {
    ...
    const shrink = (order, minWidthsArr) => {
      while (over > 0) {
        let progressed = false;
        for (const i of order) {
          if ((widths[i] ?? 0) <= (minWidthsArr[i] ?? 0)) {
            continue;
          }
  ...
  if (maxWidth) {
    const sepCountInner = columns.length + 1;
    const currentTotal = widths.reduce((a, b) => a + b, 0) + sepCountInner;
  ...
  const renderRow = (record, isHeader = false) => {
    ...
    for (let li = 0; li < height; li += 1) {
      const parts = wrapped.map((lineArr, i) => {
        const raw = lineArr[li] ?? '';
