...
describe('buildStatusMessage', () => {
  ...
  it('inserts usage summary beneath context line', () => {
    const text = buildStatusMessage({
      agent: { model: 'anthropic/claude-opus-4-5', contextTokens: 32e3 },
      sessionEntry: { sessionId: 'u1', updatedAt: 0, totalTokens: 1e3 },
      sessionKey: 'agent:main:main',
      sessionScope: 'per-sender',
      queue: { mode: 'collect', depth: 0 },
      usageLine: '\u{1F4CA} Usage: Claude 80% left (5h)',
      modelAuth: 'api-key'
    });
  ...
  it('hides cost when not using an API key', () => {
    ...
    expect(text).not.toContain('\u{1F4B5} Cost:');
  });
      ...
      async (dir) => {
        ...
        const text = buildStatusMessageDynamic({
          ...
          sessionEntry: {
            ...
            updatedAt: 0,
            totalTokens: 3,
            // would be wrong if cached prompt tokens exist
            contextTokens: 32e3
          },
          sessionKey: 'agent:main:main',
          sessionScope: 'per-sender',
