...
describe('commands registry args', () => {
  ...
  it('resolves auto arg menus when missing a choice arg', () => {
    ...
    const menu = resolveCommandArgMenu({ command, args: void 0, cfg: {} });
      ...
      args: [
        {
          ...
          description: 'level',
          type: 'string',
          choices: ({ provider, model, command: command2, arg }) => {
            seen = { provider, model, commandKey: command2.key, argName: arg.name };
            return ['low', 'high'];
          }
        }
      ]
