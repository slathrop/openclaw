...
describe('stripHeartbeatToken', () => {
  it('skips empty or token-only replies', () => {
    expect(stripHeartbeatToken(void 0, { mode: 'heartbeat' })).toEqual({
      shouldSkip: true,
      text: '',
      didStrip: false
    });
  ...
  it('drops heartbeats with small junk in heartbeat mode', () => {
    expect(stripHeartbeatToken('HEARTBEAT_OK \u{1F99E}', { mode: 'heartbeat' })).toEqual({
      shouldSkip: true,
      text: '',
      didStrip: true
    });
    expect(stripHeartbeatToken(`\u{1F99E} ${HEARTBEAT_TOKEN}`, { mode: 'heartbeat' })).toEqual({
      shouldSkip: true,
      text: '',
      didStrip: true
    });
...
describe('isHeartbeatContentEffectivelyEmpty', () => {
  it('returns false for undefined/null (missing file should not skip)', () => {
    expect(isHeartbeatContentEffectivelyEmpty(void 0)).toBe(false);
