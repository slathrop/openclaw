...
async function applyInlineDirectiveOverrides(params) {
  ...
  if (isDirectiveOnly({
    ...
    if (!command.isAuthorizedSender) {
      typing.cleanup();
      return { kind: 'reply', reply: void 0 };
    }
...
${statusReply.text}` }
  ...
  const hasAnyDirective = directives.hasThinkDirective || directives.hasVerboseDirective || directives.hasReasoningDirective || directives.hasElevatedDirective || directives.hasExecDirective || directives.hasModelDirective || directives.hasQueueDirective || directives.hasStatusDirective;
