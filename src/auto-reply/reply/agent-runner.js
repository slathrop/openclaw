...
async function runReplyAgent(params) {
  ...
  const blockReplyCoalescing = blockStreamingEnabled && opts?.onBlockReply ? resolveBlockStreamingCoalescing(
    cfg,
    sessionCtx.Provider,
    sessionCtx.AccountId,
    blockReplyChunking
  ...
  try {
    ...
    const responseUsageRaw = activeSessionEntry?.responseUsage ?? (sessionKey ? activeSessionStore?.[sessionKey]?.responseUsage : void 0);
    ...
    if (autoCompactionCompleted) {
      ...
      if (verboseEnabled) {
        ...
        finalPayloads = [{ text: `\u{1F9F9} Auto-compaction complete${suffix}.` }, ...finalPayloads];
      }
    }
    if (verboseEnabled && activeIsNewSession) {
      finalPayloads = [{ text: `\u{1F9ED} New session: ${followupRun.run.sessionId}` }, ...finalPayloads];
    }
