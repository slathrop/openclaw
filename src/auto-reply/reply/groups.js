...
function extractGroupId(raw) {
  ...
  if (parts.length >= 2 && parts[0]?.toLowerCase() === 'whatsapp' && trimmed.toLowerCase().includes('@g.us')) {
    return parts.slice(1).join(':') || void 0;
  }
...
function buildGroupIntro(params) {
  ...
  const providerIdsLine = providerId ? getChannelDock(providerId)?.groups?.resolveGroupIntroHint?.({
    cfg: params.cfg,
    groupId,
    groupChannel,
    groupSpace,
  ...
  const silenceLine = activation === 'always' ? `If no response is needed, reply with exactly "${params.silentToken}" (and nothing else) so OpenClaw stays silent. Do not add any other words, punctuation, tags, markdown/code blocks, or explanations.` : void 0;
  const cautionLine = activation === 'always' ? 'Be extremely selective: reply only when directly addressed or clearly helpful. Otherwise stay silent.' : void 0;
