...
${threadStarterBody}` : void 0;
  ...
  if (resolvedThinkLevel === 'xhigh' && !supportsXHighThinking(provider, model)) {
    const explicitThink = directives.hasThinkDirective && directives.thinkLevel !== void 0;
    if (explicitThink) {
      typing.cleanup();
      return {
  ...
  if (resetTriggered && command.isAuthorizedSender) {
    ...
    if (channel && to) {
      ...
      const text = modelLabel === defaultLabel ? `\u2705 New session started \xB7 model: ${modelLabel}` : `\u2705 New session started \xB7 model: ${modelLabel} (default: ${defaultLabel})`;
      await routeReply({
        payload: { text },
        channel,
        to,
