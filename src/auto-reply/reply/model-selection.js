...
function boundedLevenshteinDistance(a, b, maxDistance) {
  ...
  if (dist === null || dist === undefined || dist > maxDistance) {
    return null;
  }
  return dist;
}
function resolveModelOverrideFromEntry(entry) {
  ...
  const provider = entry?.providerOverride?.trim() || void 0;
...
function scoreFuzzyMatch(params) {
  ...
  const scoreFragment = (value, weights) => {
    ...
    let score2 = 0;
    if (value === fragment) {
      score2 = Math.max(score2, weights.exact);
    }
    if (value.startsWith(fragment)) {
      score2 = Math.max(score2, weights.starts);
    }
    if (value.includes(fragment)) {
      score2 = Math.max(score2, weights.includes);
    }
    return score2;
  };
  let score = 0;
  ...
  if (distModel !== null && distModel !== undefined) {
...
function resolveModelDirectiveSelection(params) {
  ...
  const resolveFuzzy = (params2) => {
    const fragment = params2.fragment.trim().toLowerCase();
    if (!fragment) {
      return {};
    }
    const providerFilter = params2.provider ? normalizeProviderId(params2.provider) : void 0;
    const candidates = [];
  ...
  if (!resolved) {
    const fuzzy2 = resolveFuzzy({ fragment: rawTrimmed });
    if (fuzzy2.selection || fuzzy2.error) {
      return fuzzy2;
    }
    return {
  ...
  if (rawLower.includes('/')) {
    ...
    const fuzzy2 = resolveFuzzy({ provider, fragment });
    if (fuzzy2.selection || fuzzy2.error) {
      return fuzzy2;
    }
  }
