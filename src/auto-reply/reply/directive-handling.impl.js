...
async function handleDirectiveOnly(params) {
  ...
  if (directives.hasThinkDirective && directives.thinkLevel) {
    parts.push(
      directives.thinkLevel === 'off' ? 'Thinking disabled.' : `Thinking level set to ${directives.thinkLevel}.`
    );
  }
  if (directives.hasVerboseDirective && directives.verboseLevel) {
    parts.push(
      directives.verboseLevel === 'off' ? formatDirectiveAck('Verbose logging disabled.') : directives.verboseLevel === 'full' ? formatDirectiveAck('Verbose logging set to full.') : formatDirectiveAck('Verbose logging enabled.')
    );
  }
  if (directives.hasReasoningDirective && directives.reasoningLevel) {
    parts.push(
      directives.reasoningLevel === 'off' ? formatDirectiveAck('Reasoning visibility disabled.') : directives.reasoningLevel === 'stream' ? formatDirectiveAck('Reasoning stream enabled (Telegram only).') : formatDirectiveAck('Reasoning visibility enabled.')
    );
  }
  if (directives.hasElevatedDirective && directives.elevatedLevel) {
    parts.push(
      directives.elevatedLevel === 'off' ? formatDirectiveAck('Elevated mode disabled.') : directives.elevatedLevel === 'full' ? formatDirectiveAck('Elevated mode set to full (auto-approve).') : formatDirectiveAck('Elevated mode set to ask (approvals may still apply).')
    );
