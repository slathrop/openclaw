...
function resolveModelAuthLabel(provider, cfg, sessionEntry, agentDir) {
  ...
  for (const profileId of candidates) {
    ...
    if (profile.type === 'token') {
      const snippet2 = formatApiKeySnippet(profile.token);
      return `token ${snippet2}${label ? ` (${label})` : ''}`;
    }
...
async function buildStatusReply(params) {
  ...
  const statusAgentId = sessionKey ? resolveSessionAgentId({ sessionKey, config: cfg }) : resolveDefaultAgentId(cfg);
  ...
  if (currentUsageProvider) {
    try {
      ...
      if (usageEntry && !usageEntry.error && usageEntry.windows.length > 0) {
        ...
        if (summaryLine) {
          usageLine = `\u{1F4CA} Usage: ${summaryLine}`;
        }
      }
    } catch {
      usageLine = null;
  ...
  if (sessionKey) {
    ...
    if (runs.length > 0) {
      ...
      if (verboseEnabled) {
        ...
        subagentsLine = `\u{1F916} Subagents: ${active.length} active${labelText} \xB7 ${done} done`;
      } else if (active.length > 0) {
        subagentsLine = `\u{1F916} Subagents: ${active.length} active`;
      }
    }
  }
