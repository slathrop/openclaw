...
async function resolveReplyDirectives(params) {
  ...
  const skillCommands = shouldResolveSkillCommands ? listSkillCommandsForWorkspace({
    workspaceDir,
    cfg,
    skillFilter
  }) : [];
  const reservedCommands = new Set(
    listChatCommands().flatMap(
      (cmd) => cmd.textAliases.map((a) => a.replace(/^\//, '').toLowerCase())
    )
  );
  for (const command2 of skillCommands) {
    reservedCommands.add(command2.name.toLowerCase());
  }
  ...
  const hasInlineDirective = parsedDirectives.hasThinkDirective || parsedDirectives.hasVerboseDirective || parsedDirectives.hasReasoningDirective || parsedDirectives.hasElevatedDirective || parsedDirectives.hasExecDirective || parsedDirectives.hasModelDirective || parsedDirectives.hasQueueDirective;
  ...
  const isModelListAlias = directives.hasModelDirective && ['status', 'list'].includes(directives.rawModelDirective?.trim().toLowerCase() ?? '');
  const effectiveModelDirective = isModelListAlias ? void 0 : directives.rawModelDirective;
