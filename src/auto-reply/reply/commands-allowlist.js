...
const ACTIONS = /* @__PURE__ */ new Set(['list', 'add', 'remove']);
const SCOPES = /* @__PURE__ */ new Set(['dm', 'group', 'all']);
...
const handleAllowlistCommand = async (params, allowTextCommands) => {
  ...
  if (parsed.action === 'list') {
    ...
    const resolvedDm = parsed.resolve && dmDisplay.length > 0 && channelId === 'slack' ? await resolveSlackNames({ cfg: params.cfg, accountId, entries: dmDisplay }) : parsed.resolve && dmDisplay.length > 0 && channelId === 'discord' ? await resolveDiscordNames({ cfg: params.cfg, accountId, entries: dmDisplay }) : void 0;
  ...
  if (shouldUpdateConfig) {
    ...
    const actionLabel2 = parsed.action === 'add' ? 'added' : 'removed';
    const scopeLabel2 = scope === 'dm' ? 'DM' : 'group';
    const locations = [];
    if (configChanged) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: `\u2705 ${scopeLabel2} allowlist ${actionLabel2}: ${targetLabel}.`
      }
    };
  }
  if (!shouldTouchStore) {
