...
const resolveAuthLabel = async (provider, cfg, modelsPath, agentDir, mode = 'compact') => {
  ...
  if (order.length > 0) {
    if (mode === 'compact') {
      ...
      if (profile.type === 'token') {
        const exp2 = typeof profile.expires === 'number' && Number.isFinite(profile.expires) && profile.expires > 0 ? profile.expires <= now ? ' expired' : ` exp ${formatUntil(profile.expires)}` : '';
        return {
          label: `${profileId} token ${maskApiKey(profile.token)}${exp2}${more}`,
          source: ''
        };
      }
    ...
    const labels = order.map((profileId) => {
      ...
      if (!profile || configProfile?.provider && configProfile.provider !== profile.provider || configProfile?.mode && configProfile.mode !== profile.type && !(configProfile.mode === 'oauth' && profile.type === 'token')) {
        const suffix2 = flags.length > 0 ? ` (${flags.join(', ')})` : '';
        return `${profileId}=missing${suffix2}`;
      }
      if (profile.type === 'api_key') {
        const suffix2 = flags.length > 0 ? ` (${flags.join(', ')})` : '';
        return `${profileId}=${maskApiKey(profile.key ?? '')}${suffix2}`;
      }
      if (profile.type === 'token') {
        ...
        const suffix2 = flags.length > 0 ? ` (${flags.join(', ')})` : '';
        return `${profileId}=token:${maskApiKey(profile.token)}${suffix2}`;
      }
  ...
  if (envKey) {
    const isOAuthEnv = envKey.source.includes('ANTHROPIC_OAUTH_TOKEN') || envKey.source.toLowerCase().includes('oauth');
