...
const ACTIONS = /* @__PURE__ */ new Set(['list', 'stop', 'log', 'send', 'info', 'help']);
...
function resolveSubagentTarget(runs, token) {
  ...
  if (trimmed === 'last') {
    const sorted2 = sortSubagentRuns(runs);
    return { entry: sorted2[0] };
  }
...
const handleSubagentsCommand = async (params, allowTextCommands) => {
  ...
  if (action === 'list') {
    if (runs.length === 0) {
      return { shouldContinue: false, reply: { text: '\u{1F9ED} Subagents: none for this session.' } };
    }
    const sorted = sortSubagentRuns(runs);
    const active = sorted.filter((entry) => !entry.endedAt);
    const done = sorted.length - active.length;
    const lines = ['\u{1F9ED} Subagents (current session)', `Active: ${active.length} \xB7 Done: ${done}`];
    sorted.forEach((entry, index) => {
      ...
      lines.push(
        `${index + 1}) ${status} \xB7 ${label} \xB7 ${runtime} \xB7 run ${runId} \xB7 ${entry.childSessionKey}`
      );
    });
  ...
  if (action === 'log') {
    const target = restTokens[0];
    if (!target) {
      return { shouldContinue: false, reply: { text: '\u{1F4DC} Usage: /subagents log <id|#> [limit]' } };
    }
    ...
    const header = `\u{1F4DC} Subagent log: ${formatRunLabel(resolved.entry)}`;
    if (lines.length === 0) {
...
(no messages)` } };
  ...
  if (action === 'send') {
    ...
    const replyText = last ? extractAssistantText(last) : void 0;
    return {
      shouldContinue: false,
      reply: {
