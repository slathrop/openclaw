...
describe('resolveTextChunkLimit', () => {
  it('uses per-provider defaults', () => {
    expect(resolveTextChunkLimit(void 0, 'whatsapp')).toBe(4e3);
    expect(resolveTextChunkLimit(void 0, 'telegram')).toBe(4e3);
    expect(resolveTextChunkLimit(void 0, 'slack')).toBe(4e3);
    expect(resolveTextChunkLimit(void 0, 'signal')).toBe(4e3);
    expect(resolveTextChunkLimit(void 0, 'imessage')).toBe(4e3);
    expect(resolveTextChunkLimit(void 0, 'discord')).toBe(4e3);
    expect(
      resolveTextChunkLimit(void 0, 'discord', void 0, {
        fallbackLimit: 2e3
      })
    ).toBe(2e3);
  });
  it('supports provider overrides', () => {
    ...
    expect(resolveTextChunkLimit(cfg, 'whatsapp')).toBe(4e3);
  ...
  it('uses the matching provider override', () => {
    ...
    expect(resolveTextChunkLimit(cfg, 'telegram')).toBe(4e3);
  });
});
...
describe('chunkByNewline', () => {
  it('splits text on newlines', () => {
    const text = 'Line one\nLine two\nLine three';
    const chunks = chunkByNewline(text, 1e3);
  ...
  it('keeps whitespace when trimLines is false', () => {
    ...
    const chunks = chunkByNewline(text, 1e3, { trimLines: false });
...
describe('chunkTextWithMode', () => {
  it('uses length-based chunking for length mode', () => {
    const text = 'Line one\nLine two';
    const chunks = chunkTextWithMode(text, 1e3, 'length');
    expect(chunks).toEqual(['Line one\nLine two']);
  });
  it('uses paragraph-based chunking for newline mode', () => {
    const text = 'Line one\nLine two';
    const chunks = chunkTextWithMode(text, 1e3, 'newline');
    expect(chunks).toEqual(['Line one\nLine two']);
  });
...
describe('chunkMarkdownTextWithMode', () => {
  it('uses markdown-aware chunking for length mode', () => {
    const text = 'Line one\nLine two';
    expect(chunkMarkdownTextWithMode(text, 1e3, 'length')).toEqual(chunkMarkdownText(text, 1e3));
  });
  it('uses paragraph-based chunking for newline mode', () => {
    const text = 'Line one\nLine two';
    expect(chunkMarkdownTextWithMode(text, 1e3, 'newline')).toEqual(['Line one\nLine two']);
  });
  it('splits on blank lines for newline mode', () => {
    const text = 'Para one\n\nPara two';
    expect(chunkMarkdownTextWithMode(text, 1e3, 'newline')).toEqual(['Para one', 'Para two']);
  });
  it('does not split single-newline code fences in newline mode', () => {
    const text = '```js\nconst a = 1;\nconst b = 2;\n```\nAfter';
    expect(chunkMarkdownTextWithMode(text, 1e3, 'newline')).toEqual([text]);
  });
...
After`;
    expect(chunkMarkdownTextWithMode(text, 1e3, 'newline')).toEqual([fence, 'After']);
  });
});
describe('resolveChunkMode', () => {
  it('returns length as default', () => {
    expect(resolveChunkMode(void 0, 'telegram')).toBe('length');
    expect(resolveChunkMode({}, 'discord')).toBe('length');
    expect(resolveChunkMode(void 0, 'bluebubbles')).toBe('length');
  });
