...
function createSignalEventHandler(deps) {
  ...
  return async (event) => {
    ...
    if (reaction && !hasBodyContent) {
      ...
      const senderDisplay2 = formatSignalSenderDisplay(sender);
      const senderName2 = envelope.sourceName ?? senderDisplay2;
      logVerbose(`signal reaction: ${emojiLabel} from ${senderName2}`);
      ...
      const groupId2 = reaction.groupInfo?.groupId ?? void 0;
      const groupName2 = reaction.groupInfo?.groupName ?? void 0;
      const isGroup2 = Boolean(groupId2);
      const senderPeerId2 = resolveSignalPeerId(sender);
      const route = resolveAgentRoute({
        cfg: deps.cfg,
        channel: 'signal',
        accountId: deps.accountId,
        peer: {
          kind: isGroup2 ? 'group' : 'dm',
          id: isGroup2 ? groupId2 ?? 'unknown' : senderPeerId2
        }
      });
      const groupLabel = isGroup2 ? `${groupName2 ?? 'Signal Group'} id:${groupId2}` : void 0;
      const messageId2 = reaction.targetSentTimestamp ? String(reaction.targetSentTimestamp) : 'unknown';
      const text = deps.buildSignalReactionSystemEventText({
        emojiLabel,
        actorLabel: senderName2,
