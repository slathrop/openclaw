...
vi.mock('./send.js', () => ({
  sendMessageIMessage: (...args) => sendMock(...args)
}));
...
vi.mock('./client.js', () => ({
  createIMessageRpcClient: vi.fn(async (opts) => {
...
beforeEach(() => {
  ...
  requestMock.mockReset().mockImplementation((method) => {
    if (method === 'watch.subscribe') {
      return Promise.resolve({ subscription: 1 });
    }
    return Promise.resolve({});
  });
  stopMock.mockReset().mockResolvedValue(void 0);
