/**
 * iMessage monitor provider - handles inbound message processing
 */
...
class SentMessageCache {
  _cache = /** @type {Map<string, number>} */ (new Map());
  _ttlMs = 5e3;
  // 5 seconds
  remember(scope, text) {
    if (!text?.trim()) {
  ...
  _cleanup() {
    const now = Date.now();
    for (const [text, timestamp] of this._cache.entries()) {
...
async function monitorIMessageProvider(opts = {}) {
  ...
  const inboundDebouncer = createInboundDebouncer({
    ...
    buildKey: (entry) => {
      ...
      const conversationId = entry.message.chat_id !== null && entry.message.chat_id !== undefined ? `chat:${entry.message.chat_id}` : entry.message.chat_guid ?? entry.message.chat_identifier ?? 'unknown';
  ...
  async function handleMessageNow(message) {
    ...
    const effectiveGroupAllowFrom = Array.from(/* @__PURE__ */ new Set([...groupAllowFrom, ...storeAllowFrom])).map((v) => String(v).trim()).filter(Boolean);
    if (isGroup) {
