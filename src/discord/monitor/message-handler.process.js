...
async function processDiscordMessage(ctx) {
  ...
  const ackReactionPromise = shouldAckReaction() ? reactMessageDiscord(message.channelId, message.id, ackReaction, {
    rest: client.rest
  }).then(
    () => true,
    (err) => {
...
${forumContextLine}`;
  ...
  const typingChannelId = deliverTarget.startsWith('channel:') ? deliverTarget.slice('channel:'.length) : message.channelId;
