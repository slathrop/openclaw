...
async function preflightDiscordMessage(params) {
  ...
  const route = resolveAgentRoute({
    ...
    parentPeer: earlyThreadParentId ? { kind: 'channel', id: earlyThreadParentId } : void 0
  });
  ...
  const hasAnyMention = Boolean(
    !isDirectMessage && (message.mentionedEveryone || (message.mentionedUsers?.length ?? 0) > 0 || (message.mentionedRoles?.length ?? 0) > 0)
  );
  ...
  if (!isDirectMessage) {
    ...
    const ownerOk = ownerAllowList ? allowListMatches(ownerAllowList, {
      id: sender.id,
      name: sender.name,
      tag: sender.tag
    }) : false;
