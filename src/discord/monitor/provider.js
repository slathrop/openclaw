...
async function monitorDiscordProvider(opts = {}) {
  ...
  if (discordCfg.groupPolicy === void 0 && discordCfg.guilds === void 0 && defaultGroupPolicy === void 0 && groupPolicy === 'open') {
    runtime.log?.(
      warn(
      ...
      try {
        ...
        if (entries.length > 0) {
          ...
          for (const entry of resolved) {
            ...
            mapping.push(
              entry.channelId ? `${entry.input}\u2192${entry.guildId}/${entry.channelId}` : `${entry.input}\u2192${entry.guildId}`
            );
    ...
    if (guildEntries && Object.keys(guildEntries).length > 0) {
      ...
      if (userEntries.size > 0) {
        try {
          ...
          const mapping = resolvedUsers.filter((entry) => entry.resolved && entry.id).map((entry) => `${entry.input}\u2192${entry.id}`);
  ...
  let commandSpecs = nativeEnabled ? listNativeCommandSpecsForConfig(cfg, { skillCommands, provider: 'discord' }) : [];
  ...
  const commands = commandSpecs.map(
    (spec) => createDiscordNativeCommand({
      command: spec,
      cfg,
