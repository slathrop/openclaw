...
function buildDiscordCommandOptions(params) {
  ...
  return args.map((arg) => {
    ...
    const shouldAutocomplete = resolvedChoices.length > 0 && (typeof arg.choices === 'function' || resolvedChoices.length > 25);
...
async function handleDiscordCommandArgInteraction(interaction, data, ctx) {
  ...
  const updated = await safeDiscordInteractionCall(
    'command arg update',
    () => interaction.update({
...
async function dispatchDiscordCommandInteraction(params) {
  ...
  const ownerOk = ownerAllowList && user ? allowListMatches(ownerAllowList, {
    id: sender.id,
    name: sender.name,
    tag: sender.tag
  }) : false;
  ...
  if (isDirectMessage) {
    ...
    if (dmPolicy !== 'open') {
      ...
      const permitted = allowList ? allowListMatches(allowList, {
        id: sender.id,
        name: sender.name,
        tag: sender.tag
      }) : false;
  ...
  if (!isDirectMessage) {
    ...
    const authorizers = useAccessGroups ? [
      { configured: ownerAllowList !== null && ownerAllowList !== undefined, allowed: ownerOk },
  ...
  const route = resolveAgentRoute({
    ...
    parentPeer: threadParentId ? { kind: 'channel', id: threadParentId } : void 0
  });
  ...
  const ctxPayload = finalizeInboundContext({
    ...
    UntrustedContext: isGuild ? (() => {
      const channelTopic = channel && 'topic' in channel ? channel.topic ?? void 0 : void 0;
