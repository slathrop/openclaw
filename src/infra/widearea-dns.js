/**
 * Wide-area DNS-SD zone file rendering.
 *
 * Generates DNS zone files for wide-area service discovery of OpenClaw
 * gateway instances, including PTR/SRV/TXT records for mDNS-compatible
 * DNS-SD browsing over Tailnet DNS.
 */
...
/**
 * @param {{ env?: NodeJS.ProcessEnv, configDomain?: string | null }} [params]
 * @returns {string | null}
 */
...
/**
 * @param {number | null} existingSerial
 * @param {Date} now
 * @returns {number}
 */
...
function computeContentHash(body) {
  // Cheap stable hash; avoids importing crypto (and keeps deterministic
  // across runtimes).
  let h = 2166136261;
...
/**
 * @param {WideAreaGatewayZoneOpts & { serial: number }} opts
 * @returns {string}
 */
...
/**
 * Render a wide-area gateway DNS-SD zone file as text.
 * @param {WideAreaGatewayZoneOpts & { serial: number }} opts
 * @returns {string}
 */
...
/**
 * Write a wide-area gateway DNS-SD zone file, skipping writes when unchanged.
 * @param {WideAreaGatewayZoneOpts} opts
 * @returns {Promise<{ zonePath: string, changed: boolean }>}
 */
