...
describe('tailscale helpers', () => {
  ...
  it('ensureGoInstalled installs when missing and user agrees', async () => {
    ...
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: /** @type {(code: number) => never} */ ((code) => {
        throw new Error(`exit ${code}`);
      })
    };
    await ensureGoInstalled(/** @type {never} */ (exec), prompt, runtime);
  ...
  it('ensureTailscaledInstalled installs when missing and user agrees', async () => {
    ...
    await ensureTailscaledInstalled(/** @type {never} */ (exec), prompt, runtime);
  ...
  it('enableTailscaleServe attempts normal first, then sudo', async () => {
    ...
    await enableTailscaleServe(3000, /** @type {never} */ (exec));
  ...
  it('disableTailscaleServe uses fallback', async () => {
    ...
    await disableTailscaleServe(/** @type {never} */ (exec));
  ...
  it('ensureFunnel uses fallback for enabling', async () => {
    ...
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: /** @type {unknown} */ (vi.fn())
    };
    const prompt = vi.fn();

    await ensureFunnel(8080, /** @type {never} */ (exec), /** @type {never} */ (runtime), prompt);
  ...
  it('enableTailscaleServe skips sudo on non-permission errors', async () => {
    ...
    await expect(enableTailscaleServe(3000, /** @type {never} */ (exec))).rejects.toThrow('boom');
  ...
  it('enableTailscaleServe rethrows original error if sudo fails', async () => {
    ...
    await expect(enableTailscaleServe(3000, /** @type {never} */ (exec))).rejects.toBe(originalError);
