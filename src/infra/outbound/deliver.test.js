...
describe('deliverOutboundPayloads', () => {
    ...
    try {
      ...
      for (const call of sendTelegram.mock.calls) {
        expect(call[2]).toEqual(
          expect.objectContaining({ accountId: void 0, verbose: false, textMode: 'html' })
        );
      }
  ...
  it('chunks WhatsApp text and returns all results', async () => {
    const sendWhatsApp = vi.fn().mockResolvedValueOnce({ messageId: 'w1', toJid: 'jid' }).mockResolvedValueOnce({ messageId: 'w2', toJid: 'jid' });
    const cfg = {
  ...
  it('respects newline chunk mode for WhatsApp', async () => {
    ...
    const cfg = {
      channels: { whatsapp: { textChunkLimit: 4e3, chunkMode: 'newline' } }
    };
    await deliverOutboundPayloads({
      cfg,
      channel: 'whatsapp',
  ...
  it('preserves fenced blocks for markdown chunkers in newline mode', async () => {
    const chunker = vi.fn((text2) => text2 ? [text2] : []);
    const sendText = vi.fn().mockImplementation(async ({ text: text2 }) => ({
      channel: 'matrix',
      messageId: text2,
      roomId: 'r1'
    }));
    const sendMedia = vi.fn().mockImplementation(async ({ text: text2 }) => ({
      channel: 'matrix',
      messageId: text2,
      roomId: 'r1'
    }));
    ...
    const cfg = {
      channels: { matrix: { textChunkLimit: 4e3, chunkMode: 'newline' } }
    };
    const text = '```js\nconst a = 1;\nconst b = 2;\n```\nAfter';
    await deliverOutboundPayloads({
      cfg,
    ...
    expect(chunker).toHaveBeenNthCalledWith(1, text, 4e3);
  });
  ...
  it('continues on errors when bestEffort is enabled', async () => {
    const sendWhatsApp = vi.fn().mockRejectedValueOnce(new Error('fail')).mockResolvedValueOnce({ messageId: 'w2', toJid: 'jid' });
    const onError = vi.fn();
    const cfg = {};
    const results = await deliverOutboundPayloads({
      cfg,
