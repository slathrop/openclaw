/**
 * SECURITY: Message action execution engine.
 * Validates action parameters, enforces cross-context policy, resolves
 * targets through the directory, and hydrates attachment payloads.
 * Abort signal support prevents runaway action execution.
 * @module
 */
...
function resolveAttachmentMaxBytes(params) {
  ...
  if (params.channel !== 'bluebubbles') {
    return typeof fallback === 'number' ? fallback * 1024 * 1024 : void 0;
  }
...
async function handleSendAction(ctx) {
  ...
  const hasCard = params.card !== null && params.card !== undefined && typeof params.card === 'object';
  ...
  if (!params.media) {
    params.media = mergedMediaUrls[0] || void 0;
  }
  ...
  const slackAutoThreadId = channel === 'slack' && !replyToId && !threadId ? resolveSlackAutoThreadId({ to, toolContext: input.toolContext }) : void 0;
  // Telegram forum topic auto-threading: inject threadId so media/buttons land in the correct topic.
  const telegramAutoThreadId = channel === 'telegram' && !threadId ? resolveTelegramAutoThreadId({ to, toolContext: input.toolContext }) : void 0;
...
async function runMessageAction(input) {
  ...
  const resolvedAgentId = input.agentId ?? (input.sessionKey ? resolveSessionAgentId({ sessionKey: input.sessionKey, config: cfg }) : void 0);
