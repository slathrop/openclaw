/**
 * SECURITY: Outbound session routing and key management.
 * Maps outbound send targets to session keys for transcript mirroring.
 * Handles per-channel target parsing and normalization to prevent
 * session key confusion attacks across channels.
 * @module
 */
...
function resolveSignalSession(params) {
  ...
  if (lowered.startsWith('group:')) {
    ...
    const peer2 = { kind: 'group', id: groupId };
    const baseSessionKey2 = buildBaseSessionKey({
      cfg: params.cfg,
      agentId: params.agentId,
      channel: 'signal',
      accountId: params.accountId,
    ...
    return {
      sessionKey: baseSessionKey2,
      baseSessionKey: baseSessionKey2,
      peer: peer2,
      chatType: 'group',
      from: `group:${groupId}`,
      to: `group:${groupId}`
