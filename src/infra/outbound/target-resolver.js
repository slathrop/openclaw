/**
 * SECURITY: Channel target resolution with directory lookups.
 * Resolves human-readable target names to channel-specific IDs via
 * cached directory entries. Prevents ambiguous target injection.
 * @module
 */
...
function resolveMatch(params) {
  const matches = params.entries.filter(
    (entry) => matchesDirectoryEntry({ channel: params.channel, entry, query: params.query })
  );
  if (matches.length === 0) {
    return { kind: 'none' };
  }
...
async function listDirectoryEntries(params) {
  ...
  if (params.kind === 'user') {
    const fn2 = useLive ? directory.listPeersLive ?? directory.listPeers : directory.listPeers;
    if (!fn2) {
      return [];
    }
    return await fn2({
...
async function lookupDirectoryDisplay(params) {
  ...
  const findMatch = (candidates) => candidates.find(
