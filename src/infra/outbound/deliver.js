/**
 * SECURITY: Outbound message delivery engine.
 * Handles chunking, routing, and sending messages across all channel providers.
 * Validates delivery targets and enforces abort signal cancellation.
 * @module
 */
...
async function deliverOutboundPayloads(params) {
  ...
  const textLimit = handler.chunker ? resolveTextChunkLimit(cfg, channel, accountId, {
  ...
  const sendTextChunks = async (text) => {
    ...
    if (chunkMode === 'newline') {
      ...
      const blockChunks = mode === 'markdown' ? chunkMarkdownTextWithMode(text, textLimit, 'newline') : chunkByParagraph(text, textLimit);
      if (!blockChunks.length && text) {
        blockChunks.push(text);
      }
      for (const blockChunk of blockChunks) {
        const chunks2 = handler.chunker(blockChunk, textLimit);
        if (!chunks2.length && blockChunk) {
          chunks2.push(blockChunk);
        }
  ...
  const sendSignalTextChunks = async (text) => {
    throwIfAborted(abortSignal);
    let signalChunks = textLimit === void 0 ? markdownToSignalTextChunks(text, Number.POSITIVE_INFINITY, {
