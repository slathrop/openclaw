/**
 * Exec approval system: allowlist, security policy, and command analysis.
 *
 * Manages the command execution approval flow: reading/writing the approvals
 * config file, resolving per-agent security policies, analyzing shell commands,
 * evaluating allowlists with glob matching, and requesting approval via socket.
 * SECURITY: Approvals file written with 0o600 permissions.
 * SECURITY: Socket-based approval uses token authentication.
 * SECURITY: Shell command analysis rejects dangerous tokens ($(), backticks, etc.).
 */
...
/**
 * @typedef {'sandbox' | 'gateway' | 'node'} ExecHost
 */
...
/**
 * @typedef {'allow-once' | 'allow-always' | 'deny'} ExecApprovalDecision
 */
...
/**
 * Normalizes an exec approvals file, migrating legacy agent entries.
 * @param {ExecApprovalsFile} file
 * @returns {ExecApprovalsFile}
 */
...
/**
 * Reads the exec approvals file and returns a snapshot with hash.
 * @returns {ExecApprovalsSnapshot}
 */
...
/**
 * SECURITY: File written with 0o600 permissions.
 * @param {ExecApprovalsFile} file
 */
...
/**
 * Ensures the approvals file exists with a socket path and token.
 * SECURITY: Generates a cryptographic token if none exists.
 * @returns {ExecApprovalsFile}
 */
...
/**
 * @param {ExecSecurity | undefined} value
 * @param {ExecSecurity} fallback
 * @returns {ExecSecurity}
 */
...
/**
 * @param {ExecAsk | undefined} value
 * @param {ExecAsk} fallback
 * @returns {ExecAsk}
 */
...
/**
 * Resolves exec approvals for a specific agent, including defaults and wildcard.
 * @param {string} [agentId]
 * @param {ExecApprovalsDefaultOverrides} [overrides]
 * @returns {ExecApprovalsResolved}
 */
...
/**
 * Resolves exec approvals from a provided file object (no disk I/O).
 * @param {{ file: ExecApprovalsFile, agentId?: string, overrides?: ExecApprovalsDefaultOverrides, path?: string, socketPath?: string, token?: string }} params
 * @returns {ExecApprovalsResolved}
 */
...
/**
 * Resolves a command string to its executable path and name.
 * @param {string} command
 * @param {string} [cwd]
 * @param {NodeJS.ProcessEnv} [env]
 * @returns {{ rawExecutable: string, resolvedPath?: string, executableName: string } | null}
 */
...
/**
 * Matches a command resolution against an allowlist.
 * SECURITY: Only matches entries containing path separators (prevents bare-name bypass).
 * @param {ExecAllowlistEntry[]} entries
 * @param {{ rawExecutable: string, resolvedPath?: string, executableName: string } | null} resolution
 * @returns {ExecAllowlistEntry | null}
 */
...
/**
 * SECURITY: Rejects dangerous shell tokens (||, |&, &, ;, redirects, subshells).
 * @param command
 */
...
/**
 * Analyzes a shell command string, splitting by chain and pipe operators.
 * SECURITY: Rejects command substitution, backticks, redirects, and other unsafe tokens.
 * @param {{ command: string, cwd?: string, env?: NodeJS.ProcessEnv, platform?: string | null }} params
 * @returns {ExecCommandAnalysis}
 */
...
/**
 * @param {{ argv: string[], cwd?: string, env?: NodeJS.ProcessEnv }} params
 * @returns {ExecCommandAnalysis}
 */
...
/**
 * Checks if a command uses a safe bin with non-file arguments.
 * SECURITY: Rejects safe bin usage if any argument looks like a file path.
 * @param {{argv: string[], resolution: object | null, safeBins: Set<string>, cwd?: string, fileExists?: Function}} params
 * @returns {boolean}
 */
...
/**
 * Evaluates an analyzed command against an allowlist and safe bins.
 * @param {{ analysis: ExecCommandAnalysis, allowlist: ExecAllowlistEntry[], safeBins: Set<string>, cwd?: string, skillBins?: Set<string>, autoAllowSkills?: boolean }} params
 * @returns {ExecAllowlistEvaluation}
 */
 ...
 * @param {{ command: string, allowlist: ExecAllowlistEntry[], safeBins: Set<string>, cwd?: string, env?: NodeJS.ProcessEnv, skillBins?: Set<string>, autoAllowSkills?: boolean, platform?: string | null }} params
...
/**
 * Determines whether a command requires explicit approval.
 * @param {{ ask: ExecAsk, security: ExecSecurity, analysisOk: boolean, allowlistSatisfied: boolean }} params
 * @returns {boolean}
 */
...
/**
 * Records usage metadata on an allowlist entry after a command match.
 * @param {ExecApprovalsFile} approvals
 * @param {string | undefined} agentId
 * @param {ExecAllowlistEntry} entry
 * @param {string} command
...
/**
 * Adds a new allowlist entry for an agent.
 * SECURITY: Deduplicates entries by pattern to prevent allowlist bloat.
 * @param {ExecApprovalsFile} approvals
 * @param {string | undefined} agentId
...
/**
 * Returns the more restrictive of two security levels.
 * @param {ExecSecurity} a
 * @param {ExecSecurity} b
 * @returns {ExecSecurity}
 */
...
/**
 * Returns the more aggressive of two ask modes.
 * @param {ExecAsk} a
 * @param {ExecAsk} b
 * @returns {ExecAsk}
 */
...
/**
 * Requests an exec approval decision via Unix socket.
 * SECURITY: Token-authenticated socket communication with timeout.
 * @param {{ socketPath: string, token: string, request: Record<string, unknown>, timeoutMs?: number }} params
 * @returns {Promise<ExecApprovalDecision | null>}
 */
