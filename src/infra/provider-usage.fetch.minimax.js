/**
 * MiniMax provider usage fetcher.
 *
 * Queries the MiniMax API for coding plan usage. Supports multiple
 * response shapes via heuristic field scanning to handle API changes.
 */
...
const RESET_KEYS = [
  ...
  'next_reset_at', 'nextResetAt', 'next_reset_time', 'nextResetTime',
  'expires_at', 'expiresAt', 'expire_at', 'expireAt',
  'end_time', 'endTime', 'window_end', 'windowEnd'
];
...
const USED_KEYS = [
  'used', 'usage', 'used_amount', 'usedAmount',
  'used_tokens', 'usedTokens', 'used_quota', 'usedQuota',
  'used_times', 'usedTimes', 'prompt_used', 'promptUsed',
  'used_prompt', 'usedPrompt', 'prompts_used', 'promptsUsed',
...
const TOTAL_KEYS = [
  ...
  'total_tokens', 'totalTokens', 'total_quota', 'totalQuota',
  'total_times', 'totalTimes', 'prompt_total', 'promptTotal',
  'total_prompt', 'totalPrompt', 'prompt_limit', 'promptLimit',
  'limit_prompt', 'limitPrompt', 'prompts_total', 'promptsTotal',
  'total_prompts', 'totalPrompts',
  'current_interval_total_count', 'currentIntervalTotalCount',
  'limit', 'quota', 'quota_limit', 'quotaLimit', 'max'
];

const REMAINING_KEYS = [
  'remain', 'remaining', 'remain_amount', 'remainingAmount',
  'remaining_amount', 'remain_tokens', 'remainingTokens',
  'remaining_tokens', 'remain_quota', 'remainingQuota',
  'remaining_quota', 'remain_times', 'remainingTimes',
  'remaining_times', 'prompt_remain', 'promptRemain',
  'remain_prompt', 'remainPrompt', 'prompt_remaining', 'promptRemaining',
  'remaining_prompt', 'remainingPrompt', 'prompts_remaining', 'promptsRemaining',
  'prompt_left', 'promptLeft', 'prompts_left', 'promptsLeft', 'left'
];

const PLAN_KEYS = ['plan', 'plan_name', 'planName', 'product', 'tier'];

const WINDOW_HOUR_KEYS = [
  'window_hours', 'windowHours', 'duration_hours', 'durationHours', 'hours'
];

const WINDOW_MINUTE_KEYS = [
  'window_minutes', 'windowMinutes', 'duration_minutes', 'durationMinutes', 'minutes'
];
...
/**
 * Picks the first finite number from a record matching the given keys.
...
/**
 * Picks the first non-empty string from a record matching the given keys.
 * @param {Record<string, unknown>} record
 * @param {readonly string[]} keys
...
/**
 * Parses various epoch/ISO date formats into epoch milliseconds.
 * @param {unknown} value
 * @returns {number | undefined}
 */
...
/**
 * Scores a record for how likely it contains usage data.
 * @param {Record<string, unknown>} record
 * @returns {number}
 */
...
/**
 * BFS-scans a response object for candidate usage records.
...
/**
 * Derives a human-readable window label from a payload.
...
/**
 * Derives the used-percent from a usage record using multiple heuristics.
 * @param {Record<string, unknown>} payload
 * @returns {number | null}
 */
...
/**
 * Fetches MiniMax usage from the MiniMax API.
 * @param {string} apiKey
