/**
 * Tailscale integration helpers.
 *
 * Provides binary detection, Funnel/Serve management, whois identity lookups,
 * and sudo-fallback command execution for Tailscale CLI operations.
 */
...
/** @type {Map<string, TailscaleWhoisCacheEntry>} */
 ...
 * @returns {Promise<string | null>} Path to Tailscale binary or null if not found
 */
export async function findTailscaleBinary() {
  // Helper to check if a binary exists and is executable
  const checkBinary = async (binaryPath) => {
    if (!binaryPath || !existsSync(binaryPath)) {
      return false;
    }
    try {
      await Promise.race([
        runExec(binaryPath, ['--version'], {timeoutMs: 3000}),
    ...
    try {
      ...
      const self =
        typeof parsed.Self === 'object' && parsed.Self !== null
          ? /** @type {Record<string, unknown>} */ (parsed.Self)
          : undefined;
      const dns = typeof self?.DNSName === 'string' ? self.DNSName : undefined;
      const ips = Array.isArray(self?.TailscaleIPs)
        ? /** @type {string[]} */ (/** @type {Record<string, unknown>} */ (parsed.Self).TailscaleIPs ?? [])
        : [];
      if (dns && dns.length > 0) {
        return dns.replace(/\.$/, '');
      }
...
/**
 * @param {typeof runExec} [exec]
 * @param {typeof promptYesNo} [prompt]
...
/**
 * @param {unknown} err
 * @returns {{ stdout: string, stderr: string, message: string, code: string }}
 */
function extractExecErrorText(err) {
  const errOutput = /** @type {ExecErrorDetails} */ (err);
 ...
 * @param {string} bin
 * @param {string[]} args
 * @param {{ maxBuffer?: number, timeoutMs?: number }} opts
...
export async function ensureFunnel(port, exec = runExec, runtime = defaultRuntime, prompt = promptYesNo) {
  ...
  try {
    ...
    const errOutput = /** @type {{ stdout?: unknown, stderr?: unknown }} */ (err);
    const stdout = typeof errOutput.stdout === 'string' ? errOutput.stdout : '';
    const stderr = typeof errOutput.stderr === 'string' ? errOutput.stderr : '';
 ...
 * @returns {TailscaleWhoisIdentity | null}
 */
 ...
 * @param {string} ip
 * @param {number} now
 * @returns {TailscaleWhoisIdentity | null | undefined}
 */
...
/**
 * @param {string} ip
 * @param {TailscaleWhoisIdentity | null} value
 * @param {number} ttlMs
 */
...
/**
 * Read Tailscale whois identity for an IP address.
 * @param {string} ip
 * @param {typeof runExec} [exec]
 * @param {{ timeoutMs?: number, cacheTtlMs?: number, errorTtlMs?: number }} [opts]
 * @returns {Promise<TailscaleWhoisIdentity | null>}
 */
