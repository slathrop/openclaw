/**
 * Global package manager install detection and update helpers.
 *
 * Detects whether OpenClaw was installed globally via npm, pnpm, or bun,
 * resolves global root paths, and provides install/cleanup utilities.
 */
import fs from 'node:fs/promises';
import os from 'node:os';
import path from 'node:path';

/** @typedef {'npm' | 'pnpm' | 'bun'} GlobalInstallManager */

/**
 * @typedef {(
 *   argv: string[],
 *   options: { timeoutMs: number, cwd?: string, env?: NodeJS.ProcessEnv }
 * ) => Promise<{ stdout: string, stderr: string, code: number | null }>} CommandRunner
 */

const PRIMARY_PACKAGE_NAME = 'openclaw';
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME];
const GLOBAL_RENAME_PREFIX = '.';

/**
 * @param {string} targetPath
 * @returns {Promise<boolean>}
 */
async function pathExists(targetPath) {
  try {
    await fs.access(targetPath);
    return true;
  } catch {
    return false;
  }
}

/**
 * @param {string} targetPath
 * @returns {Promise<string>}
 */
async function tryRealpath(targetPath) {
  try {
    return await fs.realpath(targetPath);
  } catch {
    return path.resolve(targetPath);
  }
}

/**
 * @returns {string}
 */
function resolveBunGlobalRoot() {
  const bunInstall = process.env.BUN_INSTALL?.trim() || path.join(os.homedir(), '.bun');
  return path.join(bunInstall, 'install', 'global', 'node_modules');
}

/**
 * @param {GlobalInstallManager} manager
 * @param {CommandRunner} runCommand
 * @param {number} timeoutMs
 * @returns {Promise<string | null>}
 */
export async function resolveGlobalRoot(manager, runCommand, timeoutMs) {
  if (manager === 'bun') {
    return resolveBunGlobalRoot();
  }
  const argv = manager === 'pnpm' ? ['pnpm', 'root', '-g'] : ['npm', 'root', '-g'];
  const res = await runCommand(argv, {timeoutMs}).catch(() => null);
  if (!res || res.code !== 0) {
    return null;
  }
  const root = res.stdout.trim();
  return root || null;
}

/**
 * @param {GlobalInstallManager} manager
 * @param {CommandRunner} runCommand
 * @param {number} timeoutMs
 * @returns {Promise<string | null>}
 */
export async function resolveGlobalPackageRoot(manager, runCommand, timeoutMs) {
  const root = await resolveGlobalRoot(manager, runCommand, timeoutMs);
  if (!root) {
    return null;
  }
  return path.join(root, PRIMARY_PACKAGE_NAME);
}

/**
 * @param {CommandRunner} runCommand
 * @param {string} pkgRoot
 * @param {number} timeoutMs
 * @returns {Promise<GlobalInstallManager | null>}
 */
export async function detectGlobalInstallManagerForRoot(runCommand, pkgRoot, timeoutMs) {
  const pkgReal = await tryRealpath(pkgRoot);

  const candidates = [
    {manager: /** @type {'npm'} */ ('npm'), argv: ['npm', 'root', '-g']},
    {manager: /** @type {'pnpm'} */ ('pnpm'), argv: ['pnpm', 'root', '-g']}
  ];

  for (const {manager, argv} of candidates) {
    const res = await runCommand(argv, {timeoutMs}).catch(() => null);
    if (!res || res.code !== 0) {
      continue;
    }
    const globalRoot = res.stdout.trim();
    if (!globalRoot) {
      continue;
    }
    const globalReal = await tryRealpath(globalRoot);
    for (const name of ALL_PACKAGE_NAMES) {
      const expected = path.join(globalReal, name);
      if (path.resolve(expected) === path.resolve(pkgReal)) {
        return manager;
      }
    }
  }

  const bunGlobalRoot = resolveBunGlobalRoot();
  const bunGlobalReal = await tryRealpath(bunGlobalRoot);
  for (const name of ALL_PACKAGE_NAMES) {
    const bunExpected = path.join(bunGlobalReal, name);
    if (path.resolve(bunExpected) === path.resolve(pkgReal)) {
      return 'bun';
    }
  }

  return null;
}

/**
 * @param {CommandRunner} runCommand
 * @param {number} timeoutMs
 * @returns {Promise<GlobalInstallManager | null>}
 */
export async function detectGlobalInstallManagerByPresence(runCommand, timeoutMs) {
  for (const manager of /** @type {const} */ (['npm', 'pnpm'])) {
    const root = await resolveGlobalRoot(manager, runCommand, timeoutMs);
    if (!root) {
      continue;
    }
    for (const name of ALL_PACKAGE_NAMES) {
      if (await pathExists(path.join(root, name))) {
        return manager;
      }
    }
  }

  const bunRoot = resolveBunGlobalRoot();
  for (const name of ALL_PACKAGE_NAMES) {
    if (await pathExists(path.join(bunRoot, name))) {
      return 'bun';
    }
  }
  return null;
}

/**
 * @param {GlobalInstallManager} manager
 * @param {string} spec
 * @returns {string[]}
 */
export function globalInstallArgs(manager, spec) {
  if (manager === 'pnpm') {
    return ['pnpm', 'add', '-g', spec];
  }
  if (manager === 'bun') {
    return ['bun', 'add', '-g', spec];
  }
  return ['npm', 'i', '-g', spec];
}

/**
 * Clean up stale global rename directories left by failed updates.
 * @param {{ globalRoot: string, packageName: string }} params
 * @returns {Promise<{ removed: string[] }>}
 */
export async function cleanupGlobalRenameDirs(params) {
  const removed = [];
  const root = params.globalRoot.trim();
  const name = params.packageName.trim();
  if (!root || !name) {
    return {removed};
  }
  const prefix = `${GLOBAL_RENAME_PREFIX}${name}-`;
  let entries = [];
  try {
    entries = await fs.readdir(root);
  } catch {
    return {removed};
  }
  for (const entry of entries) {
    if (!entry.startsWith(prefix)) {
      continue;
    }
    const target = path.join(root, entry);
    try {
      const stat = await fs.lstat(target);
      if (!stat.isDirectory()) {
        continue;
      }
      await fs.rm(target, {recursive: true, force: true});
      removed.push(entry);
    } catch {
      // ignore cleanup failures
    }
  }
  return {removed};
}
