/**
 * Anthropic Claude usage fetcher.
 *
 * Queries the Anthropic OAuth usage API for rate-limit windows.
 * Falls back to the claude.ai web API when the OAuth token lacks
 * the required user:profile scope.
 *
 * SECURITY: Handles OAuth tokens and session keys for claude.ai.
 * Session keys are read from env vars. Never log resolved credentials.
 */
...
/**
 * Resolves the claude.ai web session key from env vars.
 * SECURITY: Reads sensitive session key from environment.
 ...
 * @returns {Promise<import('./provider-usage.types.js').ProviderUsageSnapshot | null>}
 */
...
/**
 * Fetches Claude usage from the Anthropic OAuth usage API.
 * @param {string} token
 * @param {number} timeoutMs
 * @param {typeof fetch} fetchFn
