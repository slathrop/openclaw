/**
 * Update status checking for git and npm installs.
 *
 * Detects install kind (git vs package), checks dependency freshness,
 * queries the npm registry for latest versions, and compares semver strings.
 */
...
/**
 * @typedef {'pnpm' | 'bun' | 'npm' | 'unknown'} PackageManager
 */
...
/**
 * Check git update status for a repository root.
 * @param {{
 *   root: string,
 *   timeoutMs?: number,
 *   fetch?: boolean
 * }} params
 * @returns {Promise<GitUpdateStatus>}
 */
...
/**
 * @param {{ root: string, manager: PackageManager }} params
 * @returns {{ lockfilePath: string | null, markerPath: string | null }}
 */
...
/**
 * Check dependency install freshness.
 * @param {{ root: string, manager: PackageManager }} params
 * @returns {Promise<DepsStatus>}
 */
...
/**
 * @param {{ tag: string, timeoutMs?: number }} params
 * @returns {Promise<NpmTagStatus>}
 */
...
/**
 * Resolve the best npm tag for the given update channel.
 * For beta, falls back to latest if the beta tag is older.
 * @param {{ channel: import('./update-channels.js').UpdateChannel, timeoutMs?: number }} params
...
/**
 * Comprehensive update status check combining git, deps, and registry info.
 * @param {{
 *   root: string | null,
 *   timeoutMs?: number,
 *   fetchGit?: boolean,
