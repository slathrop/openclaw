 ...
 *
 * Spawns and manages SSH port-forwarding tunnels with automatic ephemeral
 * port fallback, connection health monitoring, and graceful shutdown.
 *
 * SECURITY: Rejects hostnames starting with '-' to prevent argument injection.
 * Uses '--' separator before user@host to prevent option injection.
 */
...
/**
 * Parse an SSH target string into user, host, and port components.
 * SECURITY: Rejects hostnames starting with '-' to prevent argument injection.
 * @param {string} raw
 * @returns {SshParsedTarget | null}
 */
