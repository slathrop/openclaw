/**
 * Unhandled promise rejection handler with error classification.
 *
 * Installs a global handler that classifies unhandled rejections into
 * categories (fatal, config, transient network, abort) and takes
 * appropriate action: exit for fatal/config errors, warn for transient.
 * SECURITY: Fatal and config errors trigger immediate process exit.
 * SECURITY: Transient network errors are logged but do not crash the gateway.
 */
...
/**
 * Registers a custom handler for unhandled rejections.
 * @param {Function} handler - Returns true if the rejection was handled
...
/**
 * Installs the global unhandledRejection handler on the process.
 * SECURITY: Fatal errors (OOM, worker failures) trigger immediate exit(1).
 * SECURITY: Config errors (missing keys) also trigger immediate exit(1).
 */
