...
const getCronChannelOptions = /* @__PURE__ */ __name(() => ['last', ...listChannelPlugins().map((plugin) => plugin.id)].join('|'), 'getCronChannelOptions');
async function warnIfCronSchedulerDisabled(opts) {
  try {
    ...
    // Best-effort warning; ignore config read failures
  }
}
__name(warnIfCronSchedulerDisabled, 'warnIfCronSchedulerDisabled');
...
const pad = /* @__PURE__ */ __name((value, width) => value.padEnd(width), 'pad');
const truncate = /* @__PURE__ */ __name((value, width) => {
...
const formatIsoMinute = /* @__PURE__ */ __name((iso) => {
...
const formatSpan = /* @__PURE__ */ __name((ms) => {
  if (ms < 6e4) {
    return '<1m';
  }
  if (ms < 36e5) {
...
const formatRelative = /* @__PURE__ */ __name((ms, nowMs) => {
  if (!ms) {
    return '-';
  }
...
const formatSchedule = /* @__PURE__ */ __name((schedule) => {
  if (schedule.kind === 'at') {
...
const formatStatus = /* @__PURE__ */ __name((job) => {
  if (!job.enabled) {
    return 'disabled';
  }
...
__name(printCronList, 'printCronList');
export {
