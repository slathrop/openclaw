...
const normalizeWakeMode = /* @__PURE__ */ __name((raw) => {
...
function registerSystemCli(program) {
  const system = program.command('system').description('System tools (events, heartbeat, presence)').addHelpText(
    'after',
    () => `
...
`
  ...
  addGatewayClientOptions(
    system.command('event').description('Enqueue a system event and optionally trigger a heartbeat').requiredOption('--text <text>', 'System event text').option('--mode <mode>', 'Wake mode (now|next-heartbeat)', 'next-heartbeat').option('--json', 'Output JSON', false)
  ).action(async (opts) => {
    try {
  ...
  addGatewayClientOptions(
    ...
    try {
      const result = await callGatewayFromCli('last-heartbeat', opts, void 0, {
        expectFinal: false
      });
      defaultRuntime.log(JSON.stringify(result, null, 2));
    } catch (err) {
  ...
  addGatewayClientOptions(
    ...
    try {
      const result = await callGatewayFromCli('system-presence', opts, void 0, {
        expectFinal: false
      });
      defaultRuntime.log(JSON.stringify(result, null, 2));
    } catch (err) {
...
__name(registerSystemCli, 'registerSystemCli');
export {
  registerSystemCli
};
