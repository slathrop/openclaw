...
function registerBrowserElementCommands(browser, parentOpts) {
  ...
  browser.command('press').description('Press a key').argument('<key>', 'Key to press (e.g. Enter)').option('--target-id <id>', 'CDP target id (or unique prefix)').action(async (key, opts, cmd) => {
    ...
    try {
      const result = await callBrowserAct({
        parent,
        profile,
        body: { kind: 'press', key, targetId: opts.targetId?.trim() || void 0 }
      });
      if (parent?.json) {
        defaultRuntime.log(JSON.stringify(result, null, 2));
        return;
  ...
  browser.command('hover').description('Hover an element by ai ref').argument('<ref>', 'Ref id from snapshot').option('--target-id <id>', 'CDP target id (or unique prefix)').action(async (ref, opts, cmd) => {
    ...
    try {
      const result = await callBrowserAct({
        parent,
        profile,
        body: { kind: 'hover', ref, targetId: opts.targetId?.trim() || void 0 }
      });
      if (parent?.json) {
        defaultRuntime.log(JSON.stringify(result, null, 2));
        return;
  ...
  browser.command('scrollintoview').description('Scroll an element into view by ref from snapshot').argument('<ref>', 'Ref id from snapshot').option('--target-id <id>', 'CDP target id (or unique prefix)').option(
    '--timeout-ms <ms>',
...
__name(registerBrowserElementCommands, 'registerBrowserElementCommands');
export {
