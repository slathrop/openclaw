...
__name(resolveTargetNodeId, 'resolveTargetNodeId');
...
__name(loadSnapshotLocal, 'loadSnapshotLocal');
function saveSnapshotLocal(file) {
  saveExecApprovals(file);
  return loadSnapshotLocal();
}
__name(saveSnapshotLocal, 'saveSnapshotLocal');
...
__name(loadSnapshotTarget, 'loadSnapshotTarget');
function formatCliError(err) {
  const msg = describeUnknownError(err);
  return msg.includes('\n') ? msg.split('\n')[0] : msg;
}
__name(formatCliError, 'formatCliError');
...
__name(renderApprovalsSnapshot, 'renderApprovalsSnapshot');
...
__name(resolveAgentKey, 'resolveAgentKey');
...
__name(ensureAgent, 'ensureAgent');
...
__name(isEmptyAgent, 'isEmptyAgent');
function registerExecApprovalsCli(program) {
  const formatExample = /* @__PURE__ */ __name((cmd, desc) => `  ${theme.command(cmd)}
    ${theme.muted(desc)}`, 'formatExample');
  const approvals = program.command('approvals').alias('exec-approvals').description('Manage exec approvals (gateway or node host)').addHelpText(
    'after',
    () => `
${theme.muted('Docs:')} ${formatDocsLink('/cli/approvals', 'docs.openclaw.ai/cli/approvals')}
`
  );
  const getCmd = approvals.command('get').description('Fetch exec approvals snapshot').option('--node <node>', 'Target node id/name/IP').option('--gateway', 'Force gateway approvals', false).action(async (opts) => {
    try {
      ...
      const muted = /* @__PURE__ */ __name((text) => isRich() ? theme.muted(text) : text, 'muted');
      if (source === 'local') {
  ...
  const setCmd = approvals.command('set').description('Replace exec approvals with a JSON file').option('--node <node>', 'Target node id/name/IP').option('--gateway', 'Force gateway approvals', false).option('--file <path>', 'Path to JSON file to upload').option('--stdin', 'Read JSON from stdin', false).action(async (opts) => {
    try {
  ...
  const allowlist = approvals.command('allowlist').description('Edit the per-agent allowlist').addHelpText(
    'after',
    () => `
...
`
  ...
  const allowlistAdd = allowlist.command('add <pattern>').description('Add a glob pattern to an allowlist').option('--node <node>', 'Target node id/name/IP').option('--gateway', 'Force gateway approvals', false).option('--agent <id>', 'Agent id (defaults to "*")').action(async (pattern, opts) => {
    try {
  ...
  const allowlistRemove = allowlist.command('remove <pattern>').description('Remove a glob pattern from an allowlist').option('--node <node>', 'Target node id/name/IP').option('--gateway', 'Force gateway approvals', false).option('--agent <id>', 'Agent id (defaults to "*")').action(async (pattern, opts) => {
    try {
      const trimmed = pattern.trim();
      if (!trimmed) {
        defaultRuntime.error('Pattern required.');
...
__name(registerExecApprovalsCli, 'registerExecApprovalsCli');
export {
