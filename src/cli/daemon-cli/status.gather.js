...
__name(shouldReportPortUsage, 'shouldReportPortUsage');
...
__name(gatherDaemonStatus, 'gatherDaemonStatus');
...
__name(renderPortDiagnosticsForCli, 'renderPortDiagnosticsForCli');
function resolvePortListeningAddresses(status) {
  const addrs = Array.from(
    new Set(
      status.port?.listeners?.map((l) => l.address ? normalizeListenerAddress(l.address) : '').filter((v) => Boolean(v)) ?? []
    )
  );
  return addrs;
}
__name(resolvePortListeningAddresses, 'resolvePortListeningAddresses');
export {
  gatherDaemonStatus,
