...
const TAGLINES = [
  ...
  'The only crab in your contacts you actually want to hear from. \u{1F99E}',
  ...
  'Ah, the fruit tree company! \u{1F34E}',
...
__name(utcParts, 'utcParts');
const onMonthDay = /* @__PURE__ */ __name((month, day) => (date) => {
  const parts = utcParts(date);
  return parts.month === month && parts.day === day;
}, 'onMonthDay');
const onSpecificDates = /* @__PURE__ */ __name((dates, durationDays = 1) => (date) => {
  const parts = utcParts(date);
...
const inYearWindow = /* @__PURE__ */ __name((windows) => (date) => {
  const parts = utcParts(date);
...
const isFourthThursdayOfNovember = /* @__PURE__ */ __name((date) => {
  const parts = utcParts(date);
  if (parts.month !== 10) {
    return false;
  }
...
__name(isTaglineActive, 'isTaglineActive');
...
__name(activeTaglines, 'activeTaglines');
...
__name(pickTagline, 'pickTagline');
export {
  DEFAULT_TAGLINE,
  HOLIDAY_RULES,
  TAGLINES,
