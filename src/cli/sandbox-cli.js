...
__name(createRunner, 'createRunner');
function registerSandboxCli(program) {
  const sandbox = program.command('sandbox').description('Manage sandbox containers (Docker-based agent isolation)').addHelpText(
    'after',
    () => `
...
`
  ...
  sandbox.command('list').description('List sandbox containers and their status').option('--json', 'Output result as JSON', false).option('--browser', 'List browser containers only', false).addHelpText(
    'after',
    () => `
${theme.heading('Examples:')}
...
${theme.muted('  --force        Skip confirmation prompt')}`
  ...
  sandbox.command('explain').description('Explain effective sandbox/tool policy for a session/agent').option('--session <key>', 'Session key to inspect (defaults to agent main)').option('--agent <id>', 'Agent id to inspect (defaults to derived agent)').option('--json', 'Output result as JSON', false).addHelpText(
    'after',
    () => `
${theme.heading('Examples:')}
...
__name(registerSandboxCli, 'registerSandboxCli');
export {
  registerSandboxCli
};
