...
__name(appendClawHubHint, 'appendClawHubHint');
function formatSkillStatus(skill) {
  ...
  if (skill.blockedByAllowlist) {
    return theme.warn('\u{1F6AB} blocked');
  }
  return theme.error('\u2717 missing');
}
__name(formatSkillStatus, 'formatSkillStatus');
function formatSkillName(skill) {
  const emoji = skill.emoji ?? '\u{1F4E6}';
  return `${emoji} ${theme.command(skill.name)}`;
}
__name(formatSkillName, 'formatSkillName');
...
__name(formatSkillMissingSummary, 'formatSkillMissingSummary');
function formatSkillsList(report, opts) {
  ...
  if (skills.length === 0) {
    const message = opts.eligible ? `No eligible skills found. Run \`${formatCliCommand('openclaw skills list')}\` to see all skills.` : 'No skills found.';
...
__name(formatSkillsList, 'formatSkillsList');
function formatSkillInfo(report, skillName, opts) {
  ...
  const lines = [];
  const emoji = skill.emoji ?? '\u{1F4E6}';
  const status = skill.eligible ? theme.success('\u2713 Ready') : skill.disabled ? theme.warn('\u23F8 Disabled') : skill.blockedByAllowlist ? theme.warn('\u{1F6AB} Blocked by allowlist') : theme.error('\u2717 Missing requirements');
...
__name(formatSkillInfo, 'formatSkillInfo');
function formatSkillsCheck(report, opts) {
  ...
  lines.push(`${theme.warn('\u{1F6AB}')} ${theme.muted('Blocked by allowlist:')} ${blocked.length}`);
...
__name(formatSkillsCheck, 'formatSkillsCheck');
function registerSkillsCli(program) {
  const skills = program.command('skills').description('List and inspect available skills').addHelpText(
    'after',
    () => `
...
__name(registerSkillsCli, 'registerSkillsCli');
export {
  formatSkillInfo,
  formatSkillsCheck,
  formatSkillsList,
