...
describe('cli program (smoke)', () => {
  ...
  it('runs tui without overriding timeout', async () => {
    ...
    expect(runTui).toHaveBeenCalledWith(expect.objectContaining({ timeoutMs: void 0 }));
  });
  it('runs tui with explicit timeout override', async () => {
    ...
    expect(runTui).toHaveBeenCalledWith(expect.objectContaining({ timeoutMs: 45e3 }));
  });
  ...
  it('runs channels login', async () => {
    ...
    expect(runChannelLogin).toHaveBeenCalledWith(
      { channel: void 0, account: 'work', verbose: false },
      runtime
    );
  });
  it('runs channels logout', async () => {
    ...
    expect(runChannelLogout).toHaveBeenCalledWith({ channel: void 0, account: 'work' }, runtime);
  });
});
