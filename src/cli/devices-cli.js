...
const devicesCallOpts = /* @__PURE__ */ __name((cmd, defaults) => cmd.option('--url <url>', 'Gateway WebSocket URL (defaults to gateway.remote.url when configured)').option('--token <token>', 'Gateway token (if required)').option('--password <password>', 'Gateway password (password auth)').option('--timeout <ms>', 'Timeout in ms', String(defaults?.timeoutMs ?? 1e4)).option('--json', 'Output JSON', false), 'devicesCallOpts');
...
__name(parseDevicePairingList, 'parseDevicePairingList');
function formatTokenSummary(tokens) {
  if (!tokens || tokens.length === 0) {
    return 'none';
  }
  const parts = tokens.map((t) => `${t.role}${t.revokedAtMs ? ' (revoked)' : ''}`).toSorted((a, b) => a.localeCompare(b));
  return parts.join(', ');
}
__name(formatTokenSummary, 'formatTokenSummary');
function registerDevicesCli(program) {
  const devices = program.command('devices').description('Device pairing and auth tokens');
  devicesCallOpts(
    devices.command('list').description('List pending and paired devices').action(async (opts) => {
  ...
  devicesCallOpts(
    devices.command('approve').description('Approve a pending device pairing request').argument('<requestId>', 'Pending request id').action(async (requestId, opts) => {
  ...
  devicesCallOpts(
    devices.command('reject').description('Reject a pending device pairing request').argument('<requestId>', 'Pending request id').action(async (requestId, opts) => {
  ...
  devicesCallOpts(
    devices.command('rotate').description('Rotate a device token for a role').requiredOption('--device <id>', 'Device id').requiredOption('--role <role>', 'Role name').option('--scope <scope...>', 'Scopes to attach to the token (repeatable)').action(async (opts) => {
  ...
  devicesCallOpts(
    devices.command('revoke').description('Revoke a device token for a role').requiredOption('--device <id>', 'Device id').requiredOption('--role <role>', 'Role name').action(async (opts) => {
      const deviceId = String(opts.device ?? '').trim();
      const role = String(opts.role ?? '').trim();
      if (!deviceId || !role) {
        defaultRuntime.error('--device and --role required');
...
__name(registerDevicesCli, 'registerDevicesCli');
export {
  registerDevicesCli
};
