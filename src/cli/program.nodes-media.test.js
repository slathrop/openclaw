...
describe('cli program (nodes media)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    runTui.mockResolvedValue(void 0);
  });
  it('runs nodes camera snap and prints two MEDIA paths', async () => {
    ...
    const invokeCalls = callGateway.mock.calls.map((call) => call[0]).filter((call) => call.method === 'node.invoke');
    const facings = invokeCalls.map((call) => call.params?.params?.facing).filter(Boolean).toSorted((a, b) => a.localeCompare(b));
    expect(facings).toEqual(['back', 'front']);
    const out = String(runtime.log.mock.calls[0]?.[0] ?? '');
    const mediaPaths = out.split('\n').filter((l) => l.startsWith('MEDIA:')).map((l) => l.replace(/^MEDIA:/, '')).filter(Boolean);
