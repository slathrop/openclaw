...
__name(runGatewayCommand, 'runGatewayCommand');
...
__name(parseDaysOption, 'parseDaysOption');
...
__name(renderCostUsageSummary, 'renderCostUsageSummary');
function registerGatewayCli(program) {
  const gateway = addGatewayRunCommand(
    program.command('gateway').description('Run the WebSocket Gateway').addHelpText(
      'after',
      () => `
...
`
  ...
  gatewayCallOpts(
    gateway.command('call').description('Call a Gateway method').argument('<method>', 'Method name (health/status/system-presence/cron.*)').option('--params <json>', 'JSON object string for params', '{}').action(async (method, opts) => {
  ...
  gatewayCallOpts(
    gateway.command('usage-cost').description('Fetch usage cost summary from session logs').option('--days <days>', 'Number of days to include', '30').action(async (opts) => {
      await runGatewayCommand(async () => {
  ...
  gatewayCallOpts(
    gateway.command('health').description('Fetch Gateway health').action(async (opts) => {
      await runGatewayCommand(async () => {
        ...
        defaultRuntime.log(
          `${colorize(rich, theme.success, 'OK')}${durationMs !== null && durationMs !== undefined ? ` (${durationMs}ms)` : ''}`
        );
  ...
  gateway.command('discover').description('Discover gateways via Bonjour (local + wide-area if configured)').option('--timeout <ms>', 'Per-command timeout in ms', '2000').option('--json', 'Output JSON', false).action(async (opts) => {
    await runGatewayCommand(async () => {
      ...
      const deduped = dedupeBeacons(beacons).toSorted(
        (a, b) => String(a.displayName || a.instanceName).localeCompare(
...
__name(registerGatewayCli, 'registerGatewayCli');
export {
  registerGatewayCli
};
