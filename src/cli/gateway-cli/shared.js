...
const toOptionString = /* @__PURE__ */ __name((value) => {
...
__name(describeUnknownError, 'describeUnknownError');
...
__name(extractGatewayMiskeys, 'extractGatewayMiskeys');
...
__name(renderGatewayServiceStopHints, 'renderGatewayServiceStopHints');
async function maybeExplainGatewayServiceStop() {
  ...
  defaultRuntime.error(
    loaded ? `Gateway service appears ${service.loadedText}. Stop it first.` : 'Gateway service status unknown; if supervised, stop it first.'
  );
  for (const hint of renderGatewayServiceStopHints()) {
    defaultRuntime.error(hint);
  }
}
__name(maybeExplainGatewayServiceStop, 'maybeExplainGatewayServiceStop');
export {
