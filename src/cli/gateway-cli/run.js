...
async function runGatewayCommand(opts) {
  ...
  try {
    await runGatewayLoop({
      ...
      start: /* @__PURE__ */ __name(async () => await startGatewayServer(port, {
        bind,
...
If the gateway is supervised, stop it with: ${formatCliCommand('openclaw gateway stop')}`
      ...
      try {
        ...
        // Best-effort port diagnostics; ignore failures
      }
...
function addGatewayRunCommand(cmd) {
  return cmd.option('--port <port>', 'Port for the gateway WebSocket').option(
  ...
  ).option('--auth <mode>', 'Gateway auth mode ("token"|"password")').option('--password <password>', 'Password for auth mode=password').option('--tailscale <mode>', 'Tailscale exposure mode ("off"|"serve"|"funnel")').option(
    ...
    false
  ).option('--dev', 'Create a dev config + workspace if missing (no BOOTSTRAP.md)', false).option(
    '--reset',
    'Reset dev config + credentials + sessions + workspace (requires --dev)',
    false
  ).option('--force', 'Kill any existing listener on the target port before starting', false).option('--verbose', 'Verbose logging to stdout/stderr', false).option(
    '--claude-cli-logs',
    'Only show claude-cli logs in the console (includes stdout/stderr)',
    false
  ).option('--ws-log <style>', 'WebSocket log style ("auto"|"full"|"compact")', 'auto').option('--compact', 'Alias for "--ws-log compact"', false).option('--raw-stream', 'Log raw model stream events to jsonl', false).option('--raw-stream-path <path>', 'Raw stream jsonl path').action(async (opts) => {
    await runGatewayCommand(opts);
  });
}
__name(addGatewayRunCommand, 'addGatewayRunCommand');
export {
