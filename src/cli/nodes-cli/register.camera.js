...
const parseFacing = /* @__PURE__ */ __name((value) => {
...
function registerNodesCameraCommands(nodes) {
  ...
  nodesCallOpts(
    camera.command('list').description('List available cameras on a node').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').action(async (opts) => {
      await runNodesCommand('camera list', async () => {
        ...
        if (devices.length === 0) {
          const { muted: muted2 } = getNodesTheme();
          defaultRuntime.log(muted2('No cameras reported.'));
          return;
        }
  ...
  nodesCallOpts(
    camera.command('clip').description('Capture a short video clip from a node camera (prints MEDIA:<path>)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--facing <front|back>', 'Camera facing', 'front').option('--device-id <id>', 'Camera device id (from nodes camera list)').option(
      '--duration <ms|10s|1m>',
      'Duration (default 3000ms; supports ms/s/m, e.g. 10s)',
      '3000'
    ).option('--no-audio', 'Disable audio capture').option('--invoke-timeout <ms>', 'Node invoke timeout in ms (default 90000)', '90000').action(async (opts) => {
...
__name(registerNodesCameraCommands, 'registerNodesCameraCommands');
export {
  registerNodesCameraCommands
};
