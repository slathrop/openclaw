...
function registerNodesScreenCommands(nodes) {
  ...
  nodesCallOpts(
    screen.command('record').description('Capture a short screen recording from a node (prints MEDIA:<path>)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--screen <index>', 'Screen index (0 = primary)', '0').option('--duration <ms|10s>', 'Clip duration (ms or 10s)', '10000').option('--fps <fps>', 'Frames per second', '10').option('--no-audio', 'Disable microphone audio capture').option('--out <path>', 'Output path').option('--invoke-timeout <ms>', 'Node invoke timeout in ms (default 120000)', '120000').action(async (opts) => {
...
__name(registerNodesScreenCommands, 'registerNodesScreenCommands');
export {
