...
function registerNodesPairingCommands(nodes) {
  nodesCallOpts(
    nodes.command('pending').description('List pending pairing requests').action(async (opts) => {
      await runNodesCommand('pending', async () => {
        ...
        if (pending.length === 0) {
          ...
          defaultRuntime.log(muted2('No pending pairing requests.'));
          return;
        }
  ...
  nodesCallOpts(
    nodes.command('approve').description('Approve a pending pairing request').argument('<requestId>', 'Pending request id').action(async (requestId, opts) => {
  ...
  nodesCallOpts(
    nodes.command('reject').description('Reject a pending pairing request').argument('<requestId>', 'Pending request id').action(async (requestId, opts) => {
  ...
  nodesCallOpts(
    nodes.command('rename').description('Rename a paired node (display name override)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').requiredOption('--name <displayName>', 'New display name').action(async (opts) => {
...
__name(registerNodesPairingCommands, 'registerNodesPairingCommands');
export {
  registerNodesPairingCommands
};
