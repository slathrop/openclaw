...
function registerNodesNotifyCommand(nodes) {
  nodesCallOpts(
    nodes.command('notify').description('Send a local notification on a node (mac only)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--title <text>', 'Notification title').option('--body <text>', 'Notification body').option('--sound <name>', 'Notification sound').option('--priority <passive|active|timeSensitive>', 'Notification priority').option('--delivery <system|overlay|auto>', 'Delivery mode', 'system').option('--invoke-timeout <ms>', 'Node invoke timeout in ms (default 15000)', '15000').action(async (opts) => {
...
__name(registerNodesNotifyCommand, 'registerNodesNotifyCommand');
export {
  registerNodesNotifyCommand
};
