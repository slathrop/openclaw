...
__name(invokeCanvas, 'invokeCanvas');
function registerNodesCanvasCommands(nodes) {
  ...
  nodesCallOpts(
    canvas.command('snapshot').description('Capture a canvas snapshot (prints MEDIA:<path>)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--format <png|jpg|jpeg>', 'Image format', 'jpg').option('--max-width <px>', 'Max width in px (optional)').option('--quality <0-1>', 'JPEG quality (optional)').option('--invoke-timeout <ms>', 'Node invoke timeout in ms (default 20000)', '20000').action(async (opts) => {
  ...
  nodesCallOpts(
    canvas.command('present').description('Show the canvas (optionally with a target URL/path)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--target <urlOrPath>', 'Target URL/path (optional)').option('--x <px>', 'Placement x coordinate').option('--y <px>', 'Placement y coordinate').option('--width <px>', 'Placement width').option('--height <px>', 'Placement height').option('--invoke-timeout <ms>', 'Node invoke timeout in ms').action(async (opts) => {
      await runNodesCommand('canvas present', async () => {
        ...
        if (Number.isFinite(placement.x) || Number.isFinite(placement.y) || Number.isFinite(placement.width) || Number.isFinite(placement.height)) {
  ...
  nodesCallOpts(
    canvas.command('hide').description('Hide the canvas').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--invoke-timeout <ms>', 'Node invoke timeout in ms').action(async (opts) => {
  ...
  nodesCallOpts(
    canvas.command('navigate').description('Navigate the canvas to a URL').argument('<url>', 'Target URL/path').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--invoke-timeout <ms>', 'Node invoke timeout in ms').action(async (url, opts) => {
  ...
  nodesCallOpts(
    canvas.command('eval').description('Evaluate JavaScript in the canvas').argument('[js]', 'JavaScript to evaluate').option('--js <code>', 'JavaScript to evaluate').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--invoke-timeout <ms>', 'Node invoke timeout in ms').action(async (jsArg, opts) => {
  ...
  nodesCallOpts(
    a2ui.command('push').description('Push A2UI JSONL to the canvas').option('--jsonl <path>', 'Path to JSONL payload').option('--text <text>', 'Render a quick A2UI text payload').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--invoke-timeout <ms>', 'Node invoke timeout in ms').action(async (opts) => {
  ...
  nodesCallOpts(
    a2ui.command('reset').description('Reset A2UI renderer state').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--invoke-timeout <ms>', 'Node invoke timeout in ms').action(async (opts) => {
...
__name(registerNodesCanvasCommands, 'registerNodesCanvasCommands');
export {
