...
__name(normalizeExecSecurity, 'normalizeExecSecurity');
...
__name(normalizeExecAsk, 'normalizeExecAsk');
...
__name(mergePathPrepend, 'mergePathPrepend');
...
__name(applyPathPrepend, 'applyPathPrepend');
...
__name(resolveExecDefaults, 'resolveExecDefaults');
...
function registerNodesInvokeCommands(nodes) {
  ...
  nodesCallOpts(
    nodes.command('run').description('Run a shell command on a node (mac only)').option('--node <idOrNameOrIp>', 'Node id, name, or IP').option('--cwd <path>', 'Working directory').option(
      '--env <key=val>',
      'Environment override (repeatable)',
      (value, prev = []) => [...prev, value]
    ).option('--raw <command>', 'Run a raw shell command string (sh -lc / cmd.exe /c)').option('--agent <id>', 'Agent id (default: configured default agent)').option('--ask <mode>', 'Exec ask mode (off|on-miss|always)').option('--security <mode>', 'Exec security mode (deny|allowlist|full)').option('--command-timeout <ms>', 'Command timeout (ms)').option('--needs-screen-recording', 'Require screen recording permission').option('--invoke-timeout <ms>', 'Node invoke timeout in ms (default 30000)', '30000').argument('[command...]', 'Command and args').action(async (command, opts) => {
        ...
        if (requiresAsk) {
          ...
          if (!decision) {
            if (askFallback === 'full') {
              ...
              // Allowlist fallback: no approval needed, fall through
            } else {
...
__name(registerNodesInvokeCommands, 'registerNodesInvokeCommands');
export {
  registerNodesInvokeCommands
};
