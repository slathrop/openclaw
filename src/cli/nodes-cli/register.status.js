...
function resolveNodeVersions(node) {
  ...
  return headless ? { core: legacy, ui: void 0 } : { core: void 0, ui: legacy };
}
__name(resolveNodeVersions, 'resolveNodeVersions');
...
__name(formatNodeVersions, 'formatNodeVersions');
...
__name(formatPathEnv, 'formatPathEnv');
...
__name(parseSinceMs, 'parseSinceMs');
function registerNodesStatusCommands(nodes) {
  nodesCallOpts(
    nodes.command('status').description('List known nodes with connection status and capabilities').option('--connected', 'Only show connected nodes').option('--last-connected <duration>', 'Only show nodes connected within duration (e.g. 24h)').action(async (opts) => {
      await runNodesCommand('status', async () => {
        ...
        const now = Date.now();
        const nodes2 = parseNodeList(result);
        const lastConnectedById = sinceMs !== void 0 ? new Map(
          parsePairingList(await callGatewayCli('node.pair.list', opts, {})).paired.map(
            (entry) => [entry.nodeId, entry]
          )
        ) : null;
        const filtered = nodes2.filter((n) => {
        ...
        const filteredLabel = filtered.length !== nodes2.length ? ` (of ${nodes2.length})` : '';
        defaultRuntime.log(
  ...
  nodesCallOpts(
    nodes.command('describe').description('Describe a node (capabilities + supported invoke commands)').requiredOption('--node <idOrNameOrIp>', 'Node id, name, or IP').action(async (opts) => {
  ...
  nodesCallOpts(
    nodes.command('list').description('List pending and paired nodes').option('--connected', 'Only show connected nodes').option('--last-connected <duration>', 'Only show nodes connected within duration (e.g. 24h)').action(async (opts) => {
      await runNodesCommand('list', async () => {
        ...
        const connectedById = hasFilters ? new Map(
...
__name(registerNodesStatusCommands, 'registerNodesStatusCommands');
export {
  registerNodesStatusCommands
};
