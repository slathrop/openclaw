...
const nodesCallOpts = /* @__PURE__ */ __name((cmd, defaults) => cmd.option('--url <url>', 'Gateway WebSocket URL (defaults to gateway.remote.url when configured)').option('--token <token>', 'Gateway token (if required)').option('--timeout <ms>', 'Timeout in ms', String(defaults?.timeoutMs ?? 1e4)).option('--json', 'Output JSON', false), 'nodesCallOpts');
...
function unauthorizedHintForMessage(message) {
  ...
  if (haystack.includes('unauthorizedclient') || haystack.includes('bridge client is not authorized') || haystack.includes('unsigned bridge clients are not allowed')) {
    return [
...
__name(unauthorizedHintForMessage, 'unauthorizedHintForMessage');
function normalizeNodeKey(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+/, '').replace(/-+$/, '');
}
__name(normalizeNodeKey, 'normalizeNodeKey');
