...
function registerDirectoryCli(program) {
  const directory = program.command('directory').description('Directory lookups (self, peers, groups) for channels that support it').addHelpText(
    'after',
    () => `
...
`
  ...
  const withChannel = /* @__PURE__ */ __name((cmd) => cmd.option('--channel <name>', 'Channel (auto when only one is configured)').option('--account <id>', 'Account id (accountId)').option('--json', 'Output JSON', false), 'withChannel');
  const resolve = /* @__PURE__ */ __name(async (opts) => {
  ...
  withChannel(peers.command('list').description('List peers')).option('--query <text>', 'Optional search query').option('--limit <n>', 'Limit results').action(async (opts) => {
    try {
      const { cfg, channelId, accountId, plugin } = await resolve({
        channel: opts.channel,
        account: opts.account
  ...
  withChannel(
    groups.command('members').description('List group members').requiredOption('--group-id <id>', 'Group id')
  ).option('--limit <n>', 'Limit results').action(async (opts) => {
    try {
      const { cfg, channelId, accountId, plugin } = await resolve({
        channel: opts.channel,
        account: opts.account
...
__name(registerDirectoryCli, 'registerDirectoryCli');
export {
