...
function takeValue(raw, next) {
  if (raw.includes('=')) {
    ...
    return { value: trimmed2 || null, consumedNext: false };
  }
...
__name(parseCliProfileArgs, 'parseCliProfileArgs');
function resolveProfileStateDir(profile, homedir) {
  const suffix = profile.toLowerCase() === 'default' ? '' : `-${profile}`;
  return path.join(homedir(), `.openclaw${suffix}`);
}
__name(resolveProfileStateDir, 'resolveProfileStateDir');
...
__name(applyCliProfileEnv, 'applyCliProfileEnv');
export {
  applyCliProfileEnv,
  parseCliProfileArgs
};
