...
function registerAcpCli(program) {
  ...
  acp.option('--url <url>', 'Gateway WebSocket URL (defaults to gateway.remote.url when configured)').option('--token <token>', 'Gateway token (if required)').option('--password <password>', 'Gateway password (if required)').option('--session <key>', 'Default session key (e.g. agent:main:main)').option('--session-label <label>', 'Default session label to resolve').option('--require-existing', 'Fail if the session key/label does not exist', false).option('--reset-session', 'Reset the session key before first use', false).option('--no-prefix-cwd', 'Do not prefix prompts with the working directory', false).option('--verbose, -v', 'Verbose logging to stderr', false).addHelpText(
    'after',
    () => `
...
__name(registerAcpCli, 'registerAcpCli');
export {
  registerAcpCli
};
