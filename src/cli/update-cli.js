...
__name(pickUpdateQuip, 'pickUpdateQuip');
...
__name(normalizeVersionTag, 'normalizeVersionTag');
...
__name(resolveTargetVersion, 'resolveTargetVersion');
...
__name(isGitCheckout, 'isGitCheckout');
...
__name(readPackageName, 'readPackageName');
async function isCorePackage(root) {
  const name = await readPackageName(root);
  return Boolean(name && CORE_PACKAGE_NAMES.has(name));
}
__name(isCorePackage, 'isCorePackage');
...
__name(tryWriteCompletionCache, 'tryWriteCompletionCache');
...
__name(tryInstallShellCompletion, 'tryInstallShellCompletion');
...
__name(resolveGitInstallDir, 'resolveGitInstallDir');
function resolveDefaultGitDir() {
  return DEFAULT_GIT_DIR;
}
__name(resolveDefaultGitDir, 'resolveDefaultGitDir');
...
__name(resolveNodeRunner, 'resolveNodeRunner');
...
__name(runUpdateStep, 'runUpdateStep');
...
__name(ensureGitCheckout, 'ensureGitCheckout');
async function resolveGlobalManager(params) {
  const runCommand = /* @__PURE__ */ __name(async (argv, options) => {
...
__name(resolveGlobalManager, 'resolveGlobalManager');
...
__name(formatGitStatusLine, 'formatGitStatusLine');
async function updateStatusCommand(opts) {
  ...
  const installLabel = update.installKind === 'git' ? `git (${update.root ?? 'unknown'})` : update.installKind === 'package' ? update.packageManager : 'unknown';
  const rows = [
...
__name(updateStatusCommand, 'updateStatusCommand');
function getStepLabel(step) {
  return STEP_LABELS[step.name] ?? step.name;
}
__name(getStepLabel, 'getStepLabel');
function createUpdateProgress(enabled) {
  ...
  const progress = {
    onStepStart: /* @__PURE__ */ __name((step) => {
      currentSpinner = spinner();
      currentSpinner.start(theme.accent(getStepLabel(step)));
    }, 'onStepStart'),
    onStepComplete: /* @__PURE__ */ __name((step) => {
      if (!currentSpinner) {
        return;
      }
...
__name(createUpdateProgress, 'createUpdateProgress');
...
__name(formatStepStatus, 'formatStepStatus');
const selectStyled = /* @__PURE__ */ __name((params) => select({
  ...params,
...
__name(printResult, 'printResult');
async function updateCommand(opts) {
  ...
  if (updateInstallKind !== 'git') {
    ...
    const targetVersion = explicitTag ? await resolveTargetVersion(tag, timeoutMs) : await resolveNpmChannelTag({ channel, timeoutMs }).then((resolved) => {
      tag = resolved.tag;
      fallbackToLatest = channel === 'beta' && resolved.tag === 'latest';
      return resolved.version;
    });
    const cmp = currentVersion && targetVersion ? compareSemverStrings(currentVersion, targetVersion) : null;
    const needsConfirm = !fallbackToLatest && currentVersion !== null && currentVersion !== undefined && (targetVersion === null || targetVersion === undefined || cmp !== null && cmp !== undefined && cmp > 0);
  ...
  if (activeConfig) {
    const pluginLogger = opts.json ? {} : {
      ...
      error: /* @__PURE__ */ __name((msg) => defaultRuntime.log(theme.error(msg)), 'error')
    };
    if (!opts.json) {
      defaultRuntime.log('');
    ...
    if (!opts.json) {
      const summarizeList = /* @__PURE__ */ __name((list) => {
        if (list.length <= 6) {
          return list.join(', ');
        }
...
async function updateWizardCommand(opts = {}) {
  ...
  const channelInfo = resolveEffectiveUpdateChannel({
    ...
    git: updateStatus.git ? { tag: updateStatus.git.tag, branch: updateStatus.git.branch } : void 0
  });
  const channelLabel = formatUpdateChannelLabel({
    channel: channelInfo.channel,
    source: channelInfo.source,
...
__name(updateWizardCommand, 'updateWizardCommand');
...
${theme.muted('Docs:')} ${formatDocsLink('/cli/update', 'docs.openclaw.ai/cli/update')}`;
  ...
  update.command('wizard').description('Interactive update wizard').option('--timeout <seconds>', 'Timeout for each update step in seconds (default: 1200)').addHelpText(
    'after',
    `
${theme.muted('Docs:')} ${formatDocsLink('/cli/update', 'docs.openclaw.ai/cli/update')}
`
  ...
  update.command('status').description('Show update channel and version status').option('--json', 'Output result as JSON', false).option('--timeout <seconds>', 'Timeout for update checks in seconds (default: 3)').addHelpText(
    'after',
    () => `
${theme.heading('Examples:')}
${formatHelpExamples([
...
__name(registerUpdateCli, 'registerUpdateCli');
export {
  registerUpdateCli,
  updateCommand,
  updateStatusCommand,
