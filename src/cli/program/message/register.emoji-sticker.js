...
function registerMessageEmojiCommands(message, helpers) {
  ...
  ).requiredOption('--emoji-name <name>', 'Emoji name').requiredOption('--media <path-or-url>', 'Emoji media (path or URL)').option('--role-ids <id>', 'Role id (repeat)', collectOption, []).action(async (opts) => {
    await helpers.runMessageAction('emoji-upload', opts);
  });
}
__name(registerMessageEmojiCommands, 'registerMessageEmojiCommands');
function registerMessageStickerCommands(message, helpers) {
  ...
  ).requiredOption('--sticker-id <id>', 'Sticker id (repeat)', collectOption).option('-m, --message <text>', 'Optional message body').action(async (opts) => {
  ...
  ).requiredOption('--sticker-name <name>', 'Sticker name').requiredOption('--sticker-desc <text>', 'Sticker description').requiredOption('--sticker-tags <tags>', 'Sticker tags').requiredOption('--media <path-or-url>', 'Sticker media (path or URL)').action(async (opts) => {
    await helpers.runMessageAction('sticker-upload', opts);
  });
}
__name(registerMessageStickerCommands, 'registerMessageStickerCommands');
export {
