...
function registerMessageThreadCommands(message, helpers) {
  ...
  ).option('--message-id <id>', 'Message id (optional)').option('--auto-archive-min <n>', 'Thread auto-archive minutes').action(async (opts) => {
    await helpers.runMessageAction('thread-create', opts);
  });
  helpers.withMessageBase(
    thread.command('list').description('List threads').requiredOption('--guild-id <id>', 'Guild id')
  ).option('--channel-id <id>', 'Channel id').option('--include-archived', 'Include archived threads', false).option('--before <id>', 'Read/search before id').option('--limit <n>', 'Result limit').action(async (opts) => {
  ...
  ).option('--reply-to <id>', 'Reply-to message id').action(async (opts) => {
    await helpers.runMessageAction('thread-reply', opts);
  });
}
__name(registerMessageThreadCommands, 'registerMessageThreadCommands');
export {
