...
function registerMessageReactionsCommands(message, helpers) {
  ...
  ).requiredOption('--message-id <id>', 'Message id').option('--limit <n>', 'Result limit').action(async (opts) => {
    await helpers.runMessageAction('reactions', opts);
  });
}
__name(registerMessageReactionsCommands, 'registerMessageReactionsCommands');
export {
