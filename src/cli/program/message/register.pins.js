...
function registerMessagePinCommands(message, helpers) {
  const pins = [
    ...
    ).requiredOption('--message-id <id>', 'Message id').action(async (opts) => {
    ...
    ).option('--limit <n>', 'Result limit').action(async (opts) => {
      await helpers.runMessageAction('list-pins', opts);
    })
  ];
  void pins;
}
__name(registerMessagePinCommands, 'registerMessagePinCommands');
export {
