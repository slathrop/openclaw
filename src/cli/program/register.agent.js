...
function registerAgentCommands(program, args) {
  ...
  ).option('--reply-to <target>', 'Delivery target override (separate from session routing)').option('--reply-channel <channel>', 'Delivery channel override (separate from routing)').option('--reply-account <id>', 'Delivery account id override').option(
    '--local',
    'Run the embedded agent locally (requires model provider API keys in your shell)',
    false
  ).option('--deliver', "Send the agent's reply back to the selected channel", false).option('--json', 'Output result as JSON', false).option(
    '--timeout <seconds>',
...
${theme.muted('Docs:')} ${formatDocsLink('/cli/agent', 'docs.openclaw.ai/cli/agent')}`
  ...
  const agents = program.command('agents').description('Manage isolated agents (workspaces + auth + routing)').addHelpText(
    'after',
    () => `
...
__name(registerAgentCommands, 'registerAgentCommands');
export {
