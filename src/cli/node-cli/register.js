...
__name(parsePortWithFallback, 'parsePortWithFallback');
function registerNodeCli(program) {
  const node = program.command('node').description('Run a headless node host (system.run/system.which)').addHelpText(
    'after',
    () => `
...
__name(registerNodeCli, 'registerNodeCli');
export {
  registerNodeCli
};
