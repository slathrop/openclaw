...
__name(runModelsCommand, 'runModelsCommand');
function registerModelsCli(program) {
  const models = program.command('models').description('Model discovery, scanning, and configuration').option('--status-json', 'Output JSON (alias for `models status --json`)', false).option('--status-plain', 'Plain output (alias for `models status --plain`)', false).option(
    '--agent <id>',
...
`
  ...
  models.command('status').description('Show configured model state').option('--json', 'Output JSON', false).option('--plain', 'Plain output', false).option(
    '--check',
    'Exit non-zero if auth is expiring/expired (1=expired/missing, 2=expiring)',
    false
  ).option('--probe', 'Probe configured provider auth (live)', false).option('--probe-provider <name>', 'Only probe a single provider').option(
    '--probe-profile <id>',
  ...
  ).option('--probe-timeout <ms>', 'Per-probe timeout in ms').option('--probe-concurrency <n>', 'Concurrent probes').option('--probe-max-tokens <n>', 'Probe max tokens (best-effort)').option(
    '--agent <id>',
    'Agent id to inspect (overrides OPENCLAW_AGENT_DIR/PI_CODING_AGENT_DIR)'
  ...
  auth.command('paste-token').description('Paste a token into auth-profiles.json and update config').requiredOption('--provider <name>', 'Provider id (e.g. anthropic)').option('--profile-id <id>', 'Auth profile id (default: <provider>:manual)').option(
    '--expires-in <duration>',
...
__name(registerModelsCli, 'registerModelsCli');
export {
  registerModelsCli
};
