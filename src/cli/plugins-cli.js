...
function formatPluginLine(plugin, verbose = false) {
  ...
  const desc = plugin.description ? theme.muted(
    plugin.description.length > 60 ? `${plugin.description.slice(0, 57)}...` : plugin.description
...
__name(formatPluginLine, 'formatPluginLine');
...
__name(applySlotSelectionForPlugin, 'applySlotSelectionForPlugin');
...
__name(logSlotWarnings, 'logSlotWarnings');
function registerPluginsCli(program) {
  const plugins = program.command('plugins').description('Manage OpenClaw plugins/extensions').addHelpText(
    'after',
    () => `
  ...
  plugins.command('install').description('Install a plugin (path, archive, or npm spec)').argument('<path-or-spec>', 'Path (.ts/.js/.zip/.tgz/.tar.gz) or an npm package spec').option('-l, --link', 'Link a local path instead of copying', false).action(async (raw, opts) => {
    ...
    if (fs.existsSync(resolved)) {
      if (opts.link) {
        ...
        const slotResult3 = applySlotSelectionForPlugin(next3, probe.pluginId);
        next3 = slotResult3.config;
        await writeConfigFile(next3);
        logSlotWarnings(slotResult3.warnings);
      ...
      let next2 = {
        ...
        plugins: {
          ...
          entries: {
            ...cfg.plugins?.entries,
            [result2.pluginId]: {
              ...cfg.plugins?.entries?.[result2.pluginId],
              enabled: true
            }
          }
        }
      ...
      const slotResult2 = applySlotSelectionForPlugin(next2, result2.pluginId);
      next2 = slotResult2.config;
      await writeConfigFile(next2);
      logSlotWarnings(slotResult2.warnings);
      defaultRuntime.log(`Installed plugin: ${result2.pluginId}`);
      defaultRuntime.log('Restart the gateway to load plugins.');
      return;
    }
    if (opts.link) {
...
__name(registerPluginsCli, 'registerPluginsCli');
export {
  registerPluginsCli
};
