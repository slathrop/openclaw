...
function normalizeAllowFromE164(values) {
  ...
  return list.map((entry) => String(entry).trim()).filter((entry) => entry && entry !== '*').map((entry) => normalizeE164(entry)).filter((entry) => Boolean(entry));
}
    ...
    dispatcherOptions: {
      ...
      deliver: async (payload, info) => {
        ...
        if (info.kind === 'final') {
          ...
          if (shouldLogVerbose()) {
            const preview = payload.text !== null && payload.text !== undefined ? elide(payload.text, 400) : '<media>';
