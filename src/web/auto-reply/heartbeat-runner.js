...
async function runWebHeartbeatOnce(opts) {
  ...
  try {
    if (overrideBody) {
      ...
      whatsappHeartbeatLog.info(`manual heartbeat sent to ${to} (id ${sendResult2.messageId})`);
      return;
    }
    ...
    if (!replyPayload || !replyPayload.text && !replyPayload.mediaUrl && !replyPayload.mediaUrls?.length) {
      ...
      if (visibility.showOk) {
        if (dryRun) {
          ...
          const sendResult2 = await sender(to, heartbeatOkText, { verbose });
          okSent = true;
          heartbeatLogger.info(
            {
              to,
          ...
          whatsappHeartbeatLog.info(`heartbeat ok sent to ${to} (id ${sendResult2.messageId})`);
        }
      }
      emitHeartbeatEvent({
        status: 'ok-empty',
