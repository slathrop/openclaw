...
function extractContactPlaceholder(rawMessage) {
  ...
  const labels = contactsArray.map((entry) => describeContact({ displayName: entry.displayName, vcard: entry.vcard })).map((entry) => formatContactLabel(entry.name, entry.phones)).filter((value) => Boolean(value));
...
function formatContactsPlaceholder(labels, total) {
  ...
  if (cleaned.length === 0) {
    const suffix2 = total === 1 ? 'contact' : 'contacts';
    return `<contacts: ${total} ${suffix2}>`;
  }
...
function extractLocationData(rawMessage) {
  ...
  if (live) {
    ...
    if (latitudeRaw !== null && latitudeRaw !== undefined && longitudeRaw !== null && longitudeRaw !== undefined) {
