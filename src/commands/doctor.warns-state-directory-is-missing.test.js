...
describe('doctor command', () => {
  ...
  it('warns about opencode provider overrides', async () => {
    ...
    const warned = note.mock.calls.some(
      ([message, title]) => title === 'OpenCode Zen' && String(message).includes('models.providers.opencode')
    );
  ...
  it('skips gateway auth warning when OPENCLAW_GATEWAY_TOKEN is set', async () => {
    ...
    const warned = note.mock.calls.some(
      ([message]) => String(message).includes('Gateway auth is off or missing a token')
    );
