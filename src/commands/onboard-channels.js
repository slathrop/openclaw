...
__name(promptConfiguredAction, 'promptConfiguredAction');
...
__name(promptRemovalAccountId, 'promptRemovalAccountId');
async function collectChannelStatus(params) {
  ...
  const statusEntries = await Promise.all(
    listChannelOnboardingAdapters().map(
...
__name(collectChannelStatus, 'collectChannelStatus');
...
__name(noteChannelPrimer, 'noteChannelPrimer');
function resolveQuickstartDefault(statusByChannel) {
  let best = null;
  for (const [channel, status] of statusByChannel) {
    if (status.quickstartScore === null || status.quickstartScore === undefined) {
      continue;
    }
...
__name(resolveQuickstartDefault, 'resolveQuickstartDefault');
async function maybeConfigureDmPolicies(params) {
  ...
  let cfg = params.cfg;
  const selectPolicy = /* @__PURE__ */ __name(async (policy) => {
    await prompter.note(
      [
...
__name(maybeConfigureDmPolicies, 'maybeConfigureDmPolicies');
async function setupChannels(cfg, runtime, prompter, options) {
  ...
  const shouldConfigure = options?.skipConfirm ? true : await prompter.confirm({
  ...
  const recordAccount = /* @__PURE__ */ __name((channel, accountId) => {
  ...
  const selection = [];
  const addSelection = /* @__PURE__ */ __name((channel) => {
    if (!selection.includes(channel)) {
      selection.push(channel);
    }
  }, 'addSelection');
  const resolveDisabledHint = /* @__PURE__ */ __name((channel) => {
    const plugin = getChannelPlugin(channel);
    if (!plugin) {
  ...
  const buildSelectionOptions = /* @__PURE__ */ __name((entries) => entries.map((entry) => {
    const status = statusByChannel.get(entry.id);
    const disabledHint = resolveDisabledHint(entry.id);
    const hint = [status?.selectionHint, disabledHint].filter(Boolean).join(' \xB7 ') || void 0;
    return {
      value: entry.meta.id,
  ...
  const getChannelEntries = /* @__PURE__ */ __name(() => {
  ...
  const refreshStatus = /* @__PURE__ */ __name(async (channel) => {
    const adapter = getChannelOnboardingAdapter(channel);
    if (!adapter) {
      return;
    }
  ...
  const ensureBundledPluginEnabled = /* @__PURE__ */ __name(async (channel) => {
  ...
  const configureChannel = /* @__PURE__ */ __name(async (channel) => {
    const adapter = getChannelOnboardingAdapter(channel);
    if (!adapter) {
  ...
  const handleConfiguredChannel = /* @__PURE__ */ __name(async (channel, label) => {
    const plugin = getChannelPlugin(channel);
    const adapter = getChannelOnboardingAdapter(channel);
    ...
    const shouldPromptAccount = action === 'delete' ? Boolean(plugin?.config.deleteAccount) : Boolean(plugin?.config.setAccountEnabled);
    const accountId = shouldPromptAccount ? await promptRemovalAccountId({
      cfg: next,
      prompter,
      label,
      channel
    }) : DEFAULT_ACCOUNT_ID;
    const resolvedAccountId = normalizeAccountId(accountId) ?? (plugin ? resolveChannelDefaultAccountId({ plugin, cfg: next }) : DEFAULT_ACCOUNT_ID);
    ...
    await refreshStatus(channel);
  }, 'handleConfiguredChannel');
  const handleChannelChoice = /* @__PURE__ */ __name(async (channel) => {
...
__name(setupChannels, 'setupChannels');
export {
  noteChannelStatus,
  setupChannels
};
