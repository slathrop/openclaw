...
__name(resolveHomeDir, 'resolveHomeDir');
...
__name(noteMacLaunchAgentOverrides, 'noteMacLaunchAgentOverrides');
...
__name(launchctlGetenv, 'launchctlGetenv');
...
__name(hasConfigGatewayCreds, 'hasConfigGatewayCreds');
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines2 = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) => `- \`${key}\` is set; use \`OPENCLAW_${key.slice(key.indexOf('_') + 1)}\` instead.`
      )
    ];
    (deps?.noteFn ?? note)(lines2.join('\n'), 'Gateway (macOS)');
  }
  const tokenEntries = [
...
__name(noteMacLaunchctlGatewayEnvOverrides, 'noteMacLaunchctlGatewayEnvOverrides');
...
__name(noteDeprecatedLegacyEnvVars, 'noteDeprecatedLegacyEnvVars');
export {
