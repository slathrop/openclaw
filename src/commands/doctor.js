...
async function doctorCommand(runtime = defaultRuntime, options = {}) {
  ...
  const updateResult = await maybeOfferUpdateBeforeDoctor({
    ...
    options,
    root,
    confirm: /* @__PURE__ */ __name((p) => prompter.confirm(p), 'confirm'),
    outro
  });
  ...
  if (resolveMode(cfg) === 'local') {
    ...
    if (needsToken) {
      ...
      const shouldSetToken = options.generateGatewayToken === true ? true : options.nonInteractive === true ? false : await prompter.confirmRepair({
  ...
  if (legacyState.preview.length > 0) {
    ...
    const migrate = options.nonInteractive === true ? true : await prompter.confirm({
  ...
  if (options.nonInteractive !== true && process.platform === 'linux' && resolveMode(cfg) === 'local') {
    ...
    if (loaded) {
      await ensureSystemdUserLingerInteractive({
        runtime,
        prompter: {
          confirm: /* @__PURE__ */ __name(async (p) => prompter.confirm(p), 'confirm'),
          note
        },
...
__name(doctorCommand, 'doctorCommand');
export {
  doctorCommand
};
