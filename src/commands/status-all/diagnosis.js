...
async function appendStatusAllDiagnosis(params) {
  ...
  const emitCheck = /* @__PURE__ */ __name((label, status) => {
  ...
  if (params.sentinel?.payload) {
    ...
    lines.push(
      `  ${muted(`${summarizeRestartSentinel(params.sentinel.payload)} \xB7 ${formatAge(Date.now() - params.sentinel.payload.ts)}`)}`
    );
  } else {
  ...
  }
  {
    ...
    const label = params.tailscaleMode === 'off' ? `Tailscale: off \xB7 ${backend}${params.tailscale.dnsName ? ` \xB7 ${params.tailscale.dnsName}` : ''}` : `Tailscale: ${params.tailscaleMode} \xB7 ${backend}${params.tailscale.dnsName ? ` \xB7 ${params.tailscale.dnsName}` : ''}`;
...
__name(appendStatusAllDiagnosis, 'appendStatusAllDiagnosis');
export {
  appendStatusAllDiagnosis
};
