...
async function buildStatusAllReportLines(params) {
  ...
  const warn = /* @__PURE__ */ __name((text) => rich ? theme.warn(text) : text, 'warn');
  const fail = /* @__PURE__ */ __name((text) => rich ? theme.error(text) : text, 'fail');
  ...
  const agentRows = params.agentStatus.agents.map((a) => ({
    ...
    Active: a.lastActiveAgeMs !== null && a.lastActiveAgeMs !== undefined ? formatAge(a.lastActiveAgeMs) : 'unknown',
...
__name(buildStatusAllReportLines, 'buildStatusAllReportLines');
export {
