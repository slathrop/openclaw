...
const asRecord = /* @__PURE__ */ __name((value) => value && typeof value === 'object' ? value : {}, 'asRecord');
function summarizeSources(sources) {
  ...
  const parts = [...counts.entries()].toSorted((a, b) => b[1] - a[1]).map(([key, n]) => `${key}${n > 1 ? `\xD7${n}` : ''}`);
  const label = parts.length > 0 ? parts.join('+') : 'unknown';
  return { label, parts };
}
__name(summarizeSources, 'summarizeSources');
...
function formatTokenHint(token, opts) {
  ...
  if (!opts.showSecrets) {
    return `sha256:${sha256HexPrefix(t)} \xB7 len ${t.length}`;
  }
...
__name(formatTokenHint, 'formatTokenHint');
const formatAccountLabel = /* @__PURE__ */ __name((params) => {
...
const resolveAccountEnabled = /* @__PURE__ */ __name((plugin, account, cfg) => {
...
const resolveAccountConfigured = /* @__PURE__ */ __name(async (plugin, account, cfg) => {
  if (plugin.config.isConfigured) {
    return await plugin.config.isConfigured(account, cfg);
  }
  const configured = asRecord(account).configured;
  return configured !== false;
}, 'resolveAccountConfigured');
const buildAccountSnapshot = /* @__PURE__ */ __name((params) => {
...
const formatAllowFrom = /* @__PURE__ */ __name((params) => {
...
const buildAccountNotes = /* @__PURE__ */ __name((params) => {
...
__name(resolveLinkFields, 'resolveLinkFields');
...
__name(collectMissingPaths, 'collectMissingPaths');
function summarizeTokenConfig(params) {
  ...
  if (hasBotOrAppTokenFields) {
    ...
    const botSources = summarizeSources(ready2.map((a) => a.snapshot.botTokenSource ?? 'none'));
    const appSources = summarizeSources(ready2.map((a) => a.snapshot.appTokenSource ?? 'none'));
    const sample2 = ready2[0]?.account ? asRecord(ready2[0].account) : {};
    const botToken = typeof sample2.botToken === 'string' ? sample2.botToken : '';
    const appToken = typeof sample2.appToken === 'string' ? sample2.appToken : '';
    ...
    const hint2 = botHint || appHint ? ` (bot ${botHint || '?'}, app ${appHint || '?'})` : '';
    return {
      state: 'ok',
      detail: `tokens ok (bot ${botSources.label}, app ${appSources.label})${hint2} \xB7 accounts ${ready2.length}/${enabled.length || 1}`
    };
  }
...
__name(summarizeTokenConfig, 'summarizeTokenConfig');
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    const detail = (() => {
      ...
      if (link.linked !== null) {
        ...
        if (link.linked && link.authAgeMs !== null && link.authAgeMs !== undefined && link.authAgeMs >= 0) {
      ...
      const reason = defaultEntry && plugin.config.unconfiguredReason ? plugin.config.unconfiguredReason(defaultEntry.account, cfg) : null;
...
__name(buildChannelsTable, 'buildChannelsTable');
export {
  buildChannelsTable
};
