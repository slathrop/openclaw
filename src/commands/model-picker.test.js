...
const resolveEnvApiKey = vi.hoisted(() => vi.fn(() => void 0));
const getCustomProviderApiKey = vi.hoisted(() => vi.fn(() => void 0));
...
describe('promptModelAllowlist', () => {
  it('filters internal router models from the selection list', async () => {
    ...
    const multiselect = vi.fn(
      async (params) => params.options.map((option) => option.value)
    );
