...
async function agentCommand(opts, runtime = defaultRuntime, deps = createDefaultDeps()) {
  ...
  if (agentIdOverride && opts.sessionKey) {
    const sessionAgentId2 = resolveAgentIdFromSessionKey(opts.sessionKey);
    if (sessionAgentId2 !== agentIdOverride) {
      throw new Error(
        `Agent id "${agentIdOverrideRaw}" does not match session key agent "${sessionAgentId2}".`
      );
    }
  }
  ...
  try {
    ...
    try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: /* @__PURE__ */ __name((providerOverride, modelOverride) => {
          ...
          return runEmbeddedPiAgent({
            ...
            streamParams: opts.streamParams,
            agentDir,
            onAgentEvent: /* @__PURE__ */ __name((evt) => {
              if (evt.stream === 'lifecycle' && typeof evt.data?.phase === 'string' && (evt.data.phase === 'end' || evt.data.phase === 'error')) {
                lifecycleEnded = true;
              }
            }, 'onAgentEvent')
          });
...
__name(agentCommand, 'agentCommand');
export {
  agentCommand
};
