...
const mocks = vi.hoisted(() => ({
  ...
  pickPrimaryTailnetIPv4: vi.fn(() => void 0)
}));
...
describe('openUrl', () => {
  it("quotes URLs on win32 so '&' is not treated as cmd separator", async () => {
    ...
    const url = 'https://accounts.google.com/o/oauth2/v2/auth?client_id=abc&response_type=code&redirect_uri=http%3A%2F%2Flocalhost';
...
describe('normalizeGatewayTokenInput', () => {
  it('returns empty string for undefined or null', () => {
    expect(normalizeGatewayTokenInput(void 0)).toBe('');
