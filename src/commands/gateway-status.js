...
async function gatewayStatusCommand(opts, runtime) {
  ...
  const { discovery, probed } = await withProgress(
    ...
    async () => {
      const tryStartTunnel = /* @__PURE__ */ __name(async () => {
        if (!sshTarget) {
          return null;
        }
        try {
          const tunnel2 = await startSshPortForward({
            target: sshTarget,
      ...
      const [discovery2, tunnelFirst] = await Promise.all([discoveryTask, tunnelTask]);
      if (!sshTarget && opts.sshAuto) {
        const user = process.env.USER?.trim() || '';
        const candidates = discovery2.map((b) => {
          ...
          (candidate) => Boolean(candidate && parseSshTarget(candidate))
        );
      ...
      try {
        const probed2 = await Promise.all(
        ...
        return { discovery: discovery2, probed: probed2 };
      } finally {
        if (tunnel) {
          try {
            await tunnel.stop();
  ...
  runtime.log(
    discovery.length > 0 ? `Found ${discovery.length} gateway(s) via Bonjour (${discoveryDomains})` : `Found 0 gateways via Bonjour (${discoveryDomains})`
  );
...
__name(gatewayStatusCommand, 'gatewayStatusCommand');
...
__name(inferSshTargetFromRemoteUrl, 'inferSshTargetFromRemoteUrl');
...
__name(buildSshTarget, 'buildSshTarget');
async function resolveSshTarget(rawTarget, identity, overallTimeoutMs) {
  ...
  if (!config) {
    return { target: rawTarget, identity: identity ?? void 0 };
  }
  ...
  const identityFile = identity ?? config.identityFiles.find((entry) => entry.trim().length > 0)?.trim() ?? void 0;
  return { target, identity: identityFile };
}
__name(resolveSshTarget, 'resolveSshTarget');
export {
