...
__name(promptConfigureSection, 'promptConfigureSection');
...
__name(promptChannelMode, 'promptChannelMode');
...
__name(promptWebToolsConfig, 'promptWebToolsConfig');
async function runConfigureWizard(opts, runtime = defaultRuntime) {
  try {
    ...
    const persistConfig = /* @__PURE__ */ __name(async () => {
    ...
    if (opts.sections) {
      ...
      if (selected.includes('health')) {
        ...
        const remoteUrl2 = nextConfig.gateway?.remote?.url?.trim();
        const wsUrl = nextConfig.gateway?.mode === 'remote' && remoteUrl2 ? remoteUrl2 : localLinks.wsUrl;
        const token2 = nextConfig.gateway?.auth?.token ?? process.env.OPENCLAW_GATEWAY_TOKEN;
...
__name(runConfigureWizard, 'runConfigureWizard');
export {
  runConfigureWizard
};
