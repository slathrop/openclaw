...
__name(canWriteDir, 'canWriteDir');
...
__name(dirPermissionHint, 'dirPermissionHint');
function addUserRwx(mode) {
  const perms = mode & 511;
  return perms | 448;
}
__name(addUserRwx, 'addUserRwx');
...
__name(countJsonlLines, 'countJsonlLines');
...
__name(findOtherStateDirs, 'findOtherStateDirs');
async function noteStateIntegrity(cfg, prompter, configPath) {
  ...
  if (stateDirExists) {
    ...
    const displayDirFor = /* @__PURE__ */ __name((dir) => {
      if (dir === sessionsDir) {
...
__name(noteStateIntegrity, 'noteStateIntegrity');
...
__name(noteWorkspaceBackupTip, 'noteWorkspaceBackupTip');
export {
  noteStateIntegrity,
