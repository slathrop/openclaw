...
__name(summarizeInstallFailure, 'summarizeInstallFailure');
...
__name(formatSkillHint, 'formatSkillHint');
...
__name(upsertSkillEntry, 'upsertSkillEntry');
  ...
  if (installable.length > 0) {
    const toInstall = await prompter.multiselect({
      ...
      options: [
        ...
        ...installable.map((skill) => ({
          value: skill.name,
          label: `${skill.emoji ?? '\u{1F9E9}'} ${skill.name}`,
    ...
    for (const name of selected) {
      ...
      if (result.ok) {
        ...
        const code = result.code === null || result.code === undefined ? '' : ` (exit ${result.code})`;
