...
const debugHealth = /* @__PURE__ */ __name((...args) => {
  if (isTruthyEnvValue(process.env.OPENCLAW_DEBUG_HEALTH)) {
    console.warn('[health:debug]', ...args);
  }
}, 'debugHealth');
const formatDurationParts = /* @__PURE__ */ __name((ms) => {
  ...
  const units = [
    { label: 'w', size: 7 * 24 * 60 * 60 * 1e3 },
    { label: 'd', size: 24 * 60 * 60 * 1e3 },
    { label: 'h', size: 60 * 60 * 1e3 },
    { label: 'm', size: 60 * 1e3 },
    { label: 's', size: 1e3 }
  ];
...
const resolveHeartbeatSummary = /* @__PURE__ */ __name((cfg, agentId) => resolveHeartbeatSummaryForAgent(cfg, agentId), 'resolveHeartbeatSummary');
const resolveAgentOrder = /* @__PURE__ */ __name((cfg) => {
  ...
  for (const entry of entries) {
    ...
    seen.add(id);
    ordered.push({ id, name: typeof entry.name === 'string' ? entry.name : void 0 });
  }
...
const buildSessionSummary = /* @__PURE__ */ __name((storePath) => {
  const store = loadSessionStore(storePath);
  const sessions = Object.entries(store).filter(([key]) => key !== 'global' && key !== 'unknown').map(([key, entry]) => ({ key, updatedAt: entry?.updatedAt ?? 0 })).toSorted((a, b) => b.updatedAt - a.updatedAt);
...
const isAccountEnabled = /* @__PURE__ */ __name((account) => {
...
const asRecord = /* @__PURE__ */ __name((value) => value && typeof value === 'object' ? value : null, 'asRecord');
const formatProbeLine = /* @__PURE__ */ __name((probe, opts = {}) => {
  ...
  if (ok) {
    let label2 = 'ok';
    if (usernames.size > 0) {
      label2 += ` (@${Array.from(usernames).join(', @')})`;
    }
    if (elapsedMs !== null && elapsedMs !== undefined) {
      label2 += ` (${elapsedMs}ms)`;
    }
    if (webhookUrl) {
      label2 += ` - webhook ${webhookUrl}`;
    }
    return label2;
  }
...
const formatAccountProbeTiming = /* @__PURE__ */ __name((summary) => {
  ...
  if (elapsedMs === null || elapsedMs === undefined && ok !== true) {
    return null;
  }
  ...
  const timing = elapsedMs !== null && elapsedMs !== undefined ? `${elapsedMs}ms` : 'ok';
  return `${handle}:${accountId}:${timing}`;
}, 'formatAccountProbeTiming');
const isProbeFailure = /* @__PURE__ */ __name((summary) => {
  const probe = asRecord(summary.probe);
  if (!probe) {
    return false;
  }
...
function styleHealthChannelLine(line) {
  ...
  const applyPrefix = /* @__PURE__ */ __name((prefix, color) => `${label} ${color(detail.slice(0, prefix.length))}${detail.slice(prefix.length)}`, 'applyPrefix');
...
__name(styleHealthChannelLine, 'styleHealthChannelLine');
const formatHealthChannelLines = /* @__PURE__ */ __name((summary, opts = {}) => {
  ...
  for (const channelId of channelOrder) {
    ...
    if (linked !== null) {
      if (linked) {
        ...
        const authLabel = authAgeMs !== null && authAgeMs !== undefined ? ` (auth age ${Math.round(authAgeMs / 6e4)}m)` : '';
    ...
    const failedSummary = listSummaries.find((summary2) => isProbeFailure(summary2));
    if (failedSummary) {
...
async function getHealthSnapshot(params) {
  ...
  const agents = ordered.map((entry) => {
    ...
    const sessions2 = sessionCache.get(storePath) ?? buildSessionSummary(storePath);
    sessionCache.set(storePath, sessions2);
    return {
      agentId: entry.id,
      name: entry.name,
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    for (const accountId of accountIdsToProbe) {
      ...
      const summary2 = plugin.status?.buildChannelSummary ? await plugin.status.buildChannelSummary({
        account,
        cfg,
        defaultAccountId: accountId,
        snapshot
      }) : void 0;
      const record = summary2 && typeof summary2 === 'object' ? summary2 : {
        accountId,
        configured,
        probe,
        lastProbeAt
      };
      if (record.configured === void 0) {
        record.configured = configured;
      }
      if (record.lastProbeAt === void 0 && lastProbeAt) {
        record.lastProbeAt = lastProbeAt;
      }
      record.accountId = accountId;
      accountSummaries[accountId] = record;
    }
    const defaultSummary = accountSummaries[preferredAccountId] ?? accountSummaries[defaultAccountId] ?? accountSummaries[accountIdsToProbe[0] ?? preferredAccountId];
...
__name(getHealthSnapshot, 'getHealthSnapshot');
async function healthCommand(opts, runtime) {
  ...
  if (opts.json) {
    ...
    if (resolvedAgents.length > 0) {
      const agentLabels = resolvedAgents.map(
        (agent) => agent.isDefault ? `${agent.agentId} (default)` : agent.agentId
      );
...
__name(healthCommand, 'healthCommand');
export {
  formatHealthChannelLines,
  getHealthSnapshot,
  healthCommand
