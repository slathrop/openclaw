...
__name(modelsImageFallbacksListCommand, 'modelsImageFallbacksListCommand');
async function modelsImageFallbacksAddCommand(modelRaw, runtime) {
  const updated = await updateConfig((cfg) => {
    ...
    const existingKeys = existing.map(
      (entry) => resolveModelRefFromString({
        raw: String(entry ?? ''),
        defaultProvider: DEFAULT_PROVIDER,
        aliasIndex
      })
...
__name(modelsImageFallbacksAddCommand, 'modelsImageFallbacksAddCommand');
...
__name(modelsImageFallbacksRemoveCommand, 'modelsImageFallbacksRemoveCommand');
...
__name(modelsImageFallbacksClearCommand, 'modelsImageFallbacksClearCommand');
export {
