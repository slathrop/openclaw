...
__name(resolveTokenProvider, 'resolveTokenProvider');
function resolveDefaultTokenProfileId(provider) {
  return `${normalizeProviderId(provider)}:manual`;
}
__name(resolveDefaultTokenProfileId, 'resolveDefaultTokenProfileId');
async function modelsAuthSetupTokenCommand(opts, runtime) {
  ...
  const tokenInput = await text({
    ...
    validate: /* @__PURE__ */ __name((value) => validateAnthropicSetupToken(String(value ?? '')), 'validate')
  });
  ...
  await updateConfig(
    (cfg) => applyAuthProfileConfig(cfg, {
      profileId,
      provider,
      mode: 'token'
    })
...
__name(modelsAuthSetupTokenCommand, 'modelsAuthSetupTokenCommand');
...
__name(modelsAuthPasteTokenCommand, 'modelsAuthPasteTokenCommand');
async function modelsAuthAddCommand(_opts, runtime) {
  ...
  const providerId = provider === 'custom' ? normalizeProviderId(
    String(
      await text({
        message: 'Provider id',
        validate: /* @__PURE__ */ __name((value) => value?.trim() ? void 0 : 'Required', 'validate')
  ...
  const expiresIn = wantsExpiry ? String(
    await text({
...
__name(modelsAuthAddCommand, 'modelsAuthAddCommand');
function resolveProviderMatch(providers, rawProvider) {
  ...
  return providers.find((provider) => normalizeProviderId(provider.id) === normalized) ?? providers.find(
    (provider) => provider.aliases?.some((alias) => normalizeProviderId(alias) === normalized) ?? false
  ) ?? null;
}
__name(resolveProviderMatch, 'resolveProviderMatch');
...
__name(pickAuthMethod, 'pickAuthMethod');
...
__name(mergeConfigPatch, 'mergeConfigPatch');
...
__name(applyDefaultModel, 'applyDefaultModel');
...
__name(credentialMode, 'credentialMode');
...
${issues}`);
  ...
  const result = await chosenMethod.run({
    ...
    runtime,
    isRemote,
    openUrl: /* @__PURE__ */ __name(async (url) => {
      await openUrl(url);
    }, 'openUrl'),
    oauth: {
      createVpsAwareHandlers: /* @__PURE__ */ __name((params) => createVpsAwareOAuthHandlers(params), 'createVpsAwareHandlers')
    }
  });
  ...
  if (result.defaultModel) {
    runtime.log(
      opts.setDefault ? `Default model set to ${result.defaultModel}` : `Default model available: ${result.defaultModel} (use --set-default to apply)`
    );
  }
...
__name(modelsAuthLoginCommand, 'modelsAuthLoginCommand');
export {
