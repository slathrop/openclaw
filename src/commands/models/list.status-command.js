...
async function modelsStatusCommand(opts, runtime) {
  ...
  const providersFromStore = new Set(
    Object.values(store.profiles).map((profile) => profile.provider).filter((p) => Boolean(p))
  );
  const providersFromConfig = new Set(
    Object.keys(cfg.models?.providers ?? {}).map((p) => p.trim()).filter(Boolean)
  );
  ...
  const providerAuth = providers.map((provider) => resolveProviderAuthOverview({ provider, cfg, store, modelsPath })).filter((entry) => {
  ...
  const resolvedCandidates = rawCandidates.map(
    (raw) => resolveModelRefFromString({
      raw: String(raw ?? ''),
      defaultProvider: DEFAULT_PROVIDER,
      aliasIndex
    })?.ref
  ...
  const providersWithOauth = providerAuth.filter(
    (entry) => entry.profiles.oauth > 0 || entry.profiles.token > 0 || entry.env?.value === 'OAuth (env)'
  ).map((entry) => {
  ...
  const label = /* @__PURE__ */ __name((value) => colorize(rich, theme.accent, value.padEnd(14)), 'label');
  const labelWithSource = /* @__PURE__ */ __name((value, source) => label(source ? `${value} (${source})` : value), 'labelWithSource');
  ...
  runtime.log(
    `${labelWithSource('Default', agentId ? agentModelPrimary ? 'agent' : 'defaults' : void 0)}${colorize(
      rich,
      theme.muted,
      ':'
  ...
  runtime.log(
    `${labelWithSource(
      ...
      agentId ? agentFallbacksOverride !== void 0 ? 'agent' : 'defaults' : void 0
  ...
  runtime.log(
    `${labelWithSource('Image model', agentId ? 'defaults' : void 0)}${colorize(
      rich,
      theme.muted,
      ':'
  ...
  runtime.log(
    `${label(`Aliases (${Object.keys(aliases).length || 0})`)}${colorize(rich, theme.muted, ':')} ${colorize(
      ...
      Object.keys(aliases).length ? Object.entries(aliases).map(
        ([alias, target]) => rich ? `${theme.accentDim(alias)} ${theme.muted('->')} ${theme.info(target)}` : `${alias} -> ${target}`
      ).join(', ') : '-'
    )}`
  );
  runtime.log(
  ...
  const formatKey = /* @__PURE__ */ __name((key) => colorize(rich, theme.warn, key), 'formatKey');
  const formatKeyValue = /* @__PURE__ */ __name((key, value) => `${formatKey(key)}=${colorize(rich, theme.info, value)}`, 'formatKeyValue');
  const formatSeparator = /* @__PURE__ */ __name(() => colorize(rich, theme.muted, ' | '), 'formatSeparator');
  ...
  if (oauthProfiles.length === 0) {
    ...
    const usageProviders = Array.from(
      new Set(
        oauthProfiles.map((profile) => resolveUsageProviderId(profile.provider)).filter((provider) => Boolean(provider))
      )
    );
    ...
    const formatStatus = /* @__PURE__ */ __name((status) => {
      if (status === 'ok') {
    ...
    for (const [provider, profiles] of profilesByProvider) {
      ...
      for (const profile of profiles) {
        ...
        const label2 = colorize(rich, theme.accent, labelText);
        ...
        runtime.log(`  - ${label2} ${status}${expiry}`);
      }
    }
  }
  if (probeSummary) {
    ...
    if (probeSummary.results.length === 0) {
      ...
      const statusColor = /* @__PURE__ */ __name((status) => {
        if (status === 'ok') {
...
__name(modelsStatusCommand, 'modelsStatusCommand');
export {
  modelsStatusCommand
};
