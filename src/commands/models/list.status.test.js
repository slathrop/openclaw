...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveAgentModelPrimary: vi.fn().mockReturnValue(void 0),
    resolveAgentModelFallbacksOverride: vi.fn().mockReturnValue(void 0),
    ...
    listProfilesForProvider: vi.fn((s, provider) => {
      return Object.entries(s.profiles).filter(([, cred]) => cred.provider === provider).map(([id]) => id);
    }),
    ...
    getCustomProviderApiKey: vi.fn().mockReturnValue(void 0),
...
describe('modelsStatusCommand auth overview', () => {
  it('includes masked auth sources in JSON output', async () => {
    ...
    expect(
      payload.auth.providersWithOAuth.some((e) => e.startsWith('anthropic'))
    ).toBe(true);
    expect(
      payload.auth.providersWithOAuth.some((e) => e.startsWith('openai-codex'))
    ).toBe(true);
  });
  ...
  it('labels defaults when --agent has no overrides', async () => {
    ...
    const originalPrimary = mocks.resolveAgentModelPrimary.getMockImplementation();
    const originalFallbacks = mocks.resolveAgentModelFallbacksOverride.getMockImplementation();
    mocks.resolveAgentModelPrimary.mockReturnValue(void 0);
    mocks.resolveAgentModelFallbacksOverride.mockReturnValue(void 0);
    try {
