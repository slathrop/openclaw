...
const MODEL_PAD = 42;
const CTX_PAD = 8;
const multiselect = /* @__PURE__ */ __name((params) => clackMultiselect({
  ...params,
...
__name(sortScanResults, 'sortScanResults');
...
__name(sortImageResults, 'sortImageResults');
function buildScanHint(result) {
  const toolLabel = result.tool.ok ? `tool ${formatMs(result.tool.latencyMs)}` : 'tool fail';
  const imageLabel = result.image.skipped ? 'img skip' : result.image.ok ? `img ${formatMs(result.image.latencyMs)}` : 'img fail';
...
__name(buildScanHint, 'buildScanHint');
...
__name(printScanSummary, 'printScanSummary');
...
__name(printScanTable, 'printScanTable');
async function modelsScanCommand(opts, runtime) {
  ...
  const results = await withProgressTotals(
    ...
    async (update) => await scanOpenRouterModels({
      ...
      concurrency,
      probe,
      onProgress: /* @__PURE__ */ __name(({ phase, completed, total }) => {
        if (phase !== 'probe') {
          return;
        }
  ...
  const preselected = preselectPool.slice(0, Math.floor(maxCandidates)).map((entry) => entry.modelRef);
  ...
  const _updated = await updateConfig((cfg2) => {
    const nextModels = { ...cfg2.agents?.defaults?.models };
    ...
    const existingImageModel = cfg2.agents?.defaults?.imageModel;
    ...
    } : cfg2.agents?.defaults?.imageModel;
    const existingModel = cfg2.agents?.defaults?.model;
    const defaults = {
      ...cfg2.agents?.defaults,
      model: {
...
__name(modelsScanCommand, 'modelsScanCommand');
export {
  modelsScanCommand
};
