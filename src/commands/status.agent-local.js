...
async function getAgentLocalStatuses() {
  ...
  for (const agent of agentList.agents) {
    ...
    const bootstrapPath = workspaceDir !== null && workspaceDir !== undefined ? path.join(workspaceDir, 'BOOTSTRAP.md') : null;
    const bootstrapPending = bootstrapPath !== null && bootstrapPath !== undefined ? await fileExists(bootstrapPath) : null;
...
__name(getAgentLocalStatuses, 'getAgentLocalStatuses');
export {
