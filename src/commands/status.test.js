...
const mocks = vi.hoisted(() => ({
  ...
  getWebAuthAgeMs: vi.fn().mockReturnValue(5e3),
...
describe('statusCommand', () => {
  it('prints JSON when requested', async () => {
    ...
    expect(payload.sessions.recent[0].remainingTokens).toBe(5e3);
  ...
  it('prints formatted lines otherwise', async () => {
    ...
    const logs = runtime.log.mock.calls.map((c) => String(c[0]));
    ...
    expect(
      logs.some(
        (l) => l.includes('openclaw status --all') || l.includes('openclaw --profile isolated status --all') || l.includes('openclaw status --all') || l.includes('openclaw --profile isolated status --all')
      )
    ).toBe(true);
  });
  ...
  it('includes sessions across agents in JSON output', async () => {
    ...
    mocks.resolveStorePath.mockImplementation(
      (_store, opts) => opts?.agentId === 'ops' ? '/tmp/ops.json' : '/tmp/main.json'
    );
    ...
    await statusCommand({ json: true }, runtime);
    const payload = JSON.parse(runtime.log.mock.calls.at(-1)?.[0]);
    expect(payload.sessions.count).toBe(2);
    expect(payload.sessions.paths.length).toBe(2);
    expect(
      payload.sessions.recent.some((sess) => sess.key === 'agent:ops:main')
    ).toBe(true);
    if (originalAgents) {
