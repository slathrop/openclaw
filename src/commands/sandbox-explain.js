...
__name(normalizeExplainSessionKey, 'normalizeExplainSessionKey');
...
__name(inferProviderFromSessionKey, 'inferProviderFromSessionKey');
...
__name(resolveActiveChannel, 'resolveActiveChannel');
async function sandboxExplainCommand(opts, runtime) {
  ...
  const resolvedAgentId = normalizeAgentId(
    opts.agent?.trim() ? opts.agent : opts.session?.trim() ? resolveAgentIdFromSessionKey(opts.session) : defaultAgentId
  );
  ...
  const allowTokens = /* @__PURE__ */ __name((values) => (values ?? []).map((v) => String(v).trim()).filter(Boolean), 'allowTokens');
...
`);
  ...
  const rich = isRich();
  const heading = /* @__PURE__ */ __name((value2) => colorize(rich, theme.heading, value2), 'heading');
  const key = /* @__PURE__ */ __name((value2) => colorize(rich, theme.muted, value2), 'key');
  const value = /* @__PURE__ */ __name((val) => colorize(rich, theme.info, val), 'value');
  const ok = /* @__PURE__ */ __name((val) => colorize(rich, theme.success, val), 'ok');
  const warn = /* @__PURE__ */ __name((val) => colorize(rich, theme.warn, val), 'warn');
  const err = /* @__PURE__ */ __name((val) => colorize(rich, theme.error, val), 'err');
  const bool = /* @__PURE__ */ __name((flag) => flag ? ok('true') : err('false'), 'bool');
  const lines = [];
  ...
  for (const key2 of payload.fixIt) {
    lines.push(`  - ${key2}`);
  }
  lines.push('');
...
__name(sandboxExplainCommand, 'sandboxExplainCommand');
export {
