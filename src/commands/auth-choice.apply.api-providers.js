...
// SECURITY: API key credential storage for third-party providers
...
async function applyAuthChoiceApiProviders(params) {
  ...
  if (authChoice === 'apiKey' && params.opts?.tokenProvider && params.opts.tokenProvider !== 'anthropic' && params.opts.tokenProvider !== 'openai') {
    ...
    } else if (params.opts.tokenProvider === 'kimi-code' || params.opts.tokenProvider === 'kimi-coding') {
  ...
  if (authChoice === 'openrouter-api-key') {
    ...
    const existingProfileId = profileOrder.find((profileId2) => Boolean(store.profiles[profileId2]));
  ...
  if (authChoice === 'cloudflare-ai-gateway-api-key') {
    ...
    const ensureAccountGateway = /* @__PURE__ */ __name(async () => {
      if (!accountId) {
        const value = await params.prompter.text({
          message: 'Enter Cloudflare Account ID',
          validate: /* @__PURE__ */ __name((val) => String(val).trim() ? void 0 : 'Account ID is required', 'validate')
        });
      ...
      if (!gatewayId) {
        const value = await params.prompter.text({
          ...
          validate: /* @__PURE__ */ __name((val) => String(val).trim() ? void 0 : 'Gateway ID is required', 'validate')
        });
        gatewayId = String(value).trim();
      }
    }, 'ensureAccountGateway');
    ...
    }
    {
      const applied = await applyDefaultModelChoice({
        ...
        applyDefaultConfig: /* @__PURE__ */ __name((cfg) => applyCloudflareAiGatewayConfig(cfg, {
          accountId: accountId || params.opts?.cloudflareAiGatewayAccountId,
          gatewayId: gatewayId || params.opts?.cloudflareAiGatewayGatewayId
        }), 'applyDefaultConfig'),
        applyProviderConfig: /* @__PURE__ */ __name((cfg) => applyCloudflareAiGatewayProviderConfig(cfg, {
          accountId: accountId || params.opts?.cloudflareAiGatewayAccountId,
          gatewayId: gatewayId || params.opts?.cloudflareAiGatewayGatewayId
        }), 'applyProviderConfig'),
  ...
  if (authChoice === 'zai-api-key') {
    ...
    });
    {
      const applied = await applyDefaultModelChoice({
        ...
        applyProviderConfig: /* @__PURE__ */ __name((config) => ({
          ...config,
          agents: {
            ...config.agents,
            defaults: {
...
__name(applyAuthChoiceApiProviders, 'applyAuthChoiceApiProviders');
export {
