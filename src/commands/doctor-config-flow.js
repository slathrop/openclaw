...
function normalizeIssuePath(path2) {
  return path2.filter((part) => typeof part !== 'symbol');
}
__name(normalizeIssuePath, 'normalizeIssuePath');
function isUnrecognizedKeysIssue(issue) {
  return issue.code === 'unrecognized_keys';
}
__name(isUnrecognizedKeysIssue, 'isUnrecognizedKeysIssue');
...
function resolvePathTarget(root, path2) {
  let current = root;
...
__name(resolvePathTarget, 'resolvePathTarget');
function stripUnknownConfigKeys(config) {
  ...
  for (const issue of parsed.error.issues) {
    ...
    const path2 = normalizeIssuePath(issue.path);
    const target = resolvePathTarget(next, path2);
    ...
    for (const key of issue.keys) {
      ...
      delete record[key];
      removed.push(formatPath([...path2, key]));
    }
  }
  return { config: next, removed };
}
__name(stripUnknownConfigKeys, 'stripUnknownConfigKeys');
...
__name(noteOpencodeProviderOverrides, 'noteOpencodeProviderOverrides');
...
__name(maybeMigrateLegacyConfig, 'maybeMigrateLegacyConfig');
async function loadAndMaybeMigrateDoctorConfig(params) {
  ...
  if (unknown.removed.length > 0) {
    const lines = unknown.removed.map((path2) => `- ${path2}`).join('\n');
...
__name(loadAndMaybeMigrateDoctorConfig, 'loadAndMaybeMigrateDoctorConfig');
export {
