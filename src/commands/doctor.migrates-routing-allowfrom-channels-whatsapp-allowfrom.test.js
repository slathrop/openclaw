...
describe('doctor command', () => {
  it('migrates routing.allowFrom to channels.whatsapp.allowFrom', { timeout: 6e4 }, async () => {
    ...
    expect(written.channels?.whatsapp).toEqual({
      allowFrom: ['+15555550123']
    });
    expect(written.routing).toBeUndefined();
  });
  it('skips legacy gateway services migration', { timeout: 6e4 }, async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: '/tmp/openclaw.json',
      exists: true,
      raw: '{}',
