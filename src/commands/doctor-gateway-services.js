...
__name(detectGatewayRuntime, 'detectGatewayRuntime');
...
__name(findGatewayEntrypoint, 'findGatewayEntrypoint');
function normalizeExecutablePath(value) {
  return path.resolve(value);
}
__name(normalizeExecutablePath, 'normalizeExecutablePath');
...
__name(extractDetailPath, 'extractDetailPath');
async function cleanupLegacyLaunchdService(params) {
  const domain = typeof process.getuid === 'function' ? `gui/${process.getuid()}` : 'gui/501';
  await execFileAsync('launchctl', ['bootout', domain, params.plistPath]).catch(() => void 0);
...
__name(cleanupLegacyLaunchdService, 'cleanupLegacyLaunchdService');
async function maybeRepairGatewayServiceConfig(cfg, mode, runtime, prompter) {
  ...
  const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
    ...
    warn: /* @__PURE__ */ __name((message, title) => note(message, title), 'warn'),
    config: cfg
  });
  const expectedEntrypoint = findGatewayEntrypoint(programArguments);
  const currentEntrypoint = findGatewayEntrypoint(command.programArguments);
  if (expectedEntrypoint && currentEntrypoint && normalizeExecutablePath(expectedEntrypoint) !== normalizeExecutablePath(currentEntrypoint)) {
    audit.issues.push({
  ...
  note(
    audit.issues.map(
      (issue) => issue.detail ? `- ${issue.message} (${issue.detail})` : `- ${issue.message}`
    ).join('\n'),
  ...
  const repair = needsAggressive ? await prompter.confirmAggressive({
...
__name(maybeRepairGatewayServiceConfig, 'maybeRepairGatewayServiceConfig');
...
__name(maybeScanExtraGatewayServices, 'maybeScanExtraGatewayServices');
export {
