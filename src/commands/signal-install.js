...
__name(looksLikeArchive, 'looksLikeArchive');
function pickAsset(assets, platform) {
  const withName = assets.filter(
    (asset) => Boolean(asset.name && asset.browser_download_url)
  );
  const byName = /* @__PURE__ */ __name((pattern) => withName.find((asset) => pattern.test(asset.name.toLowerCase())), 'byName');
  if (platform === 'linux') {
    return byName(/linux-native/) || byName(/linux/) || withName.find((asset) => looksLikeArchive(asset.name.toLowerCase()));
  }
  if (platform === 'darwin') {
    return byName(/macos|osx|darwin/) || withName.find((asset) => looksLikeArchive(asset.name.toLowerCase()));
  }
  if (platform === 'win32') {
...
async function findSignalCliBinary(root) {
  const candidates = [];
  const enqueue = /* @__PURE__ */ __name(async (dir, depth) => {
    if (depth > 3) {
      return;
    }
...
__name(findSignalCliBinary, 'findSignalCliBinary');
...
__name(installSignalCli, 'installSignalCli');
export {
  installSignalCli
};
