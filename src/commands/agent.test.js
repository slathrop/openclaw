...
describe('agentCommand', () => {
    ...
    await withTempHome(async (home) => {
      ...
      const stop = onAgentEvent((evt) => {
        ...
        assistantEvents.push({
          runId: evt.runId,
          text: typeof evt.data?.text === 'string' ? evt.data.text : void 0
        });
      });
  ...
  it('derives session key from --agent when no routing target is provided', async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, 'sessions.json');
      mockConfig(home, store, void 0, void 0, [{ id: 'ops' }]);
  ...
  it('passes through telegram accountId when delivering', async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, 'sessions.json');
      mockConfig(home, store, void 0, { botToken: 't-1' });
      ...
      try {
        ...
        expect(deps.sendMessageTelegram).toHaveBeenCalledWith(
          '123',
          'ok',
          expect.objectContaining({ accountId: void 0, verbose: false })
        );
      } finally {
