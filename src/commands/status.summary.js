...
const classifyKey = /* @__PURE__ */ __name((key, entry) => {
  if (key === 'global') {
    return 'global';
  }
  if (key === 'unknown') {
...
const buildFlags = /* @__PURE__ */ __name((entry) => {
  if (!entry) {
    return [];
  }
  const flags = [];
...
async function getStatusSummary() {
  ...
  const loadStore = /* @__PURE__ */ __name((storePath) => {
  ...
  const buildSessionRows = /* @__PURE__ */ __name((store, opts = {}) => Object.entries(store).filter(([key]) => key !== 'global' && key !== 'unknown').map(([key, entry]) => {
    ...
    const remaining = contextTokens !== null && contextTokens !== undefined ? Math.max(0, contextTokens - total) : null;
  ...
  }).sort((a, b) => (b.updatedAt ?? 0) - (a.updatedAt ?? 0)), 'buildSessionRows');
...
__name(getStatusSummary, 'getStatusSummary');
export {
  getStatusSummary
};
