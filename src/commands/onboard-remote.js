...
__name(ensureWsUrl, 'ensureWsUrl');
async function promptRemoteGatewayConfig(cfg, prompter) {
  ...
  if (wantsDiscover) {
    ...
    const beacons = await discoverGatewayBeacons({ timeoutMs: 2e3, wideAreaDomain });
  ...
  const urlInput = await prompter.text({
    ...
    validate: /* @__PURE__ */ __name((value) => String(value).trim().startsWith('ws://') || String(value).trim().startsWith('wss://') ? void 0 : 'URL must start with ws:// or wss://', 'validate')
  });
...
__name(promptRemoteGatewayConfig, 'promptRemoteGatewayConfig');
export {
