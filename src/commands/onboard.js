...
async function onboardCommand(opts, runtime = defaultRuntime) {
  ...
  const normalizedOpts = normalizedAuthChoice === opts.authChoice && flow === opts.flow ? opts : { ...opts, authChoice: normalizedAuthChoice, flow };
...
__name(onboardCommand, 'onboardCommand');
export {
  onboardCommand
};
