...
// SECURITY: Auth credential diagnostics and health checks
import {
...
__name(maybeRepairAnthropicOAuthProfileId, 'maybeRepairAnthropicOAuthProfileId');
function pruneAuthOrder(order, profileIds) {
  ...
  return { next: Object.keys(next).length > 0 ? next : void 0, changed };
}
__name(pruneAuthOrder, 'pruneAuthOrder');
...
__name(pruneAuthProfiles, 'pruneAuthProfiles');
async function maybeRemoveDeprecatedCliAuthProfiles(cfg, prompter) {
  ...
  await updateAuthProfileStoreWithLock({
    updater: /* @__PURE__ */ __name((nextStore) => {
      let mutated = false;
...
__name(maybeRemoveDeprecatedCliAuthProfiles, 'maybeRemoveDeprecatedCliAuthProfiles');
...
__name(formatAuthIssueHint, 'formatAuthIssueHint');
...
__name(formatAuthIssueLine, 'formatAuthIssueLine');
async function noteAuthProfileHealth(params) {
  ...
  const findIssues = /* @__PURE__ */ __name(() => summary.profiles.filter(
    (profile) => (profile.type === 'oauth' || profile.type === 'token') && (profile.status === 'expired' || profile.status === 'expiring' || profile.status === 'missing')
  ), 'findIssues');
...
__name(noteAuthProfileHealth, 'noteAuthProfileHealth');
export {
