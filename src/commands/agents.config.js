...
__name(listAgentEntries, 'listAgentEntries');
function findAgentEntryIndex(list, agentId) {
  const id = normalizeAgentId(agentId);
  return list.findIndex((entry) => normalizeAgentId(entry.id) === id);
}
__name(findAgentEntryIndex, 'findAgentEntryIndex');
...
__name(parseIdentityMarkdown, 'parseIdentityMarkdown');
...
__name(loadAgentIdentity, 'loadAgentIdentity');
...
__name(buildAgentSummaries, 'buildAgentSummaries');
...
__name(applyAgentConfig, 'applyAgentConfig');
function pruneAgentConfig(cfg, agentId) {
  ...
  const nextAgentsConfig = cfg.agents ? { ...cfg.agents, list: nextAgents } : nextAgents ? { list: nextAgents } : void 0;
...
__name(pruneAgentConfig, 'pruneAgentConfig');
export {
  applyAgentConfig,
  buildAgentSummaries,
  findAgentEntryIndex,
