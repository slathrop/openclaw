...
function createContainer(overrides = {}) {
  return {
    ...
    imageMatch: true,
    running: true,
    createdAtMs: NOW - 36e5,
    lastUsedAtMs: NOW - 6e5,
    ...overrides
  };
}
...
function setupDefaultMocks() {
  ...
  mocks.removeSandboxContainer.mockResolvedValue(void 0);
...
describe('sandboxListCommand', () => {
  ...
  describe('human format output', () => {
    it('should display containers', async () => {
      ...
      await sandboxListCommand({ browser: false, json: false }, runtime);
      expectLogContains(runtime, '\u{1F4E6} Sandbox Containers');
    ...
    it('should display browsers when --browser flag is set', async () => {
      ...
      await sandboxListCommand({ browser: true, json: false }, runtime);
      expectLogContains(runtime, '\u{1F310} Sandbox Browser Containers');
...
describe('sandboxRecreateCommand', () => {
  ...
  describe('validation', () => {
    it('should error if no filter is specified', async () => {
      await sandboxRecreateCommand({ all: false, browser: false, force: false }, runtime);
  ...
  describe('filtering', () => {
    ...
    it('should remove all when --all flag set', async () => {
      ...
      await sandboxRecreateCommand({ all: true, browser: false, force: true }, runtime);
      expect(mocks.removeSandboxContainer).toHaveBeenCalledTimes(2);
    });
    it('should handle browsers when --browser flag set', async () => {
      ...
      await sandboxRecreateCommand({ all: true, browser: true, force: true }, runtime);
  ...
  describe('confirmation flow', () => {
    it('should require confirmation without --force', async () => {
      ...
      mocks.clackConfirm.mockResolvedValue(true);
      await sandboxRecreateCommand({ all: true, browser: false, force: false }, runtime);
    ...
    it('should cancel on clack cancel symbol', async () => {
      mocks.listSandboxContainers.mockResolvedValue([createContainer()]);
      mocks.clackConfirm.mockResolvedValue(/* @__PURE__ */ Symbol.for('clack:cancel'));
      await sandboxRecreateCommand({ all: true, browser: false, force: false }, runtime);
      expect(runtime.log).toHaveBeenCalledWith('Cancelled.');
      expect(mocks.removeSandboxContainer).not.toHaveBeenCalled();
    });
  ...
  describe('execution', () => {
    ...
    it('should handle removal errors and exit with code 1', async () => {
      ...
      mocks.removeSandboxContainer.mockResolvedValueOnce(void 0).mockRejectedValueOnce(new Error('Removal failed'));
      await sandboxRecreateCommand({ all: true, browser: false, force: true }, runtime);
