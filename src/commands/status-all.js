  ...
  await withProgress({ label: 'Scanning status --all\u2026', total: 11 }, async (progress) => {
    ...
    const tailscale = await (async () => {
      try {
        ...
        const ips = self && Array.isArray(self.TailscaleIPs) ? self.TailscaleIPs.filter((v) => typeof v === 'string' && v.trim().length > 0).map((v) => v.trim()) : [];
    ...
    const resolveProbeAuth = /* @__PURE__ */ __name((mode) => {
      const authToken = cfg.gateway?.auth?.token;
      const authPassword = cfg.gateway?.auth?.password;
      const remote = cfg.gateway?.remote;
      const token = mode === 'remote' ? typeof remote?.token === 'string' && remote.token.trim() ? remote.token.trim() : void 0 : process.env.OPENCLAW_GATEWAY_TOKEN?.trim() || (typeof authToken === 'string' && authToken.trim() ? authToken.trim() : void 0);
      const password = process.env.OPENCLAW_GATEWAY_PASSWORD?.trim() || (mode === 'remote' ? typeof remote?.password === 'string' && remote.password.trim() ? remote.password.trim() : void 0 : typeof authPassword === 'string' && authPassword.trim() ? authPassword.trim() : void 0);
    ...
    const readServiceSummary = /* @__PURE__ */ __name(async (service) => {
      try {
    ...
    const skillStatus = defaultWorkspace !== null && defaultWorkspace !== undefined ? (() => {
      try {
    ...
    const dashboard = controlUiEnabled ? resolveControlUiLinks({
      port,
    ...
    const updateLine = (() => {
      if (update.installKind === 'git' && update.git) {
        const parts2 = [];
        parts2.push(update.git.branch ? `git ${update.git.branch}` : 'git');
        if (update.git.upstream) {
          parts2.push(`\u2194 ${update.git.upstream}`);
        }
        if (update.git.dirty) {
          parts2.push('dirty');
        }
        if (update.git.behind !== null && update.git.behind !== undefined && update.git.ahead !== null && update.git.ahead !== undefined) {
          if (update.git.behind === 0 && update.git.ahead === 0) {
            parts2.push('up to date');
          } else if (update.git.behind > 0 && update.git.ahead === 0) {
            parts2.push(`behind ${update.git.behind}`);
          } else if (update.git.behind === 0 && update.git.ahead > 0) {
            parts2.push(`ahead ${update.git.ahead}`);
          } else {
            parts2.push(`diverged (ahead ${update.git.ahead}, behind ${update.git.behind})`);
          }
        }
        if (update.git.fetchOk === false) {
          parts2.push('fetch failed');
        }
        const latest2 = update.registry?.latestVersion;
        if (latest2) {
          const cmp = compareSemverStrings(VERSION, latest2);
          if (cmp === 0) {
            parts2.push(`npm latest ${latest2}`);
          } else if (cmp !== null && cmp !== undefined && cmp < 0) {
            parts2.push(`npm update ${latest2}`);
          } else {
            parts2.push(`npm latest ${latest2} (local newer)`);
          }
        } else if (update.registry?.error) {
          parts2.push('npm latest unknown');
        }
        if (update.deps?.status === 'ok') {
          parts2.push('deps ok');
        }
        if (update.deps?.status === 'stale') {
          parts2.push('deps stale');
        }
        if (update.deps?.status === 'missing') {
          parts2.push('deps missing');
        }
    ...
    const gatewayStatus = gatewayReachable ? `reachable ${formatDuration(gatewayProbe?.connectLatencyMs)}` : gatewayProbe?.error ? `unreachable (${gatewayProbe.error})` : 'unreachable';
    ...
    const aliveAgents = agentStatus.agents.filter(
      (a) => a.lastActiveAgeMs !== null && a.lastActiveAgeMs !== undefined && a.lastActiveAgeMs <= aliveThresholdMs
    ).length;
    const overviewRows = [
      ...
      {
        Item: 'Tailscale',
        Value: tailscaleMode === 'off' ? `off${tailscale.backendState ? ` \xB7 ${tailscale.backendState}` : ''}${tailscale.dnsName ? ` \xB7 ${tailscale.dnsName}` : ''}` : tailscale.dnsName && tailscaleHttpsUrl ? `${tailscaleMode} \xB7 ${tailscale.backendState ?? 'unknown'} \xB7 ${tailscale.dnsName} \xB7 ${tailscaleHttpsUrl}` : `${tailscaleMode} \xB7 ${tailscale.backendState ?? 'unknown'} \xB7 magicdns unknown`
      },
      ...
      daemon ? {
        ...
        Value: !daemon.installed ? `${daemon.label} not installed` : `${daemon.label} ${daemon.installed ? 'installed \xB7 ' : ''}${daemon.loadedText}${daemon.runtime?.status ? ` \xB7 ${daemon.runtime.status}` : ''}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ''}`
      } : { Item: 'Gateway service', Value: 'unknown' },
      nodeService ? {
        Item: 'Node service',
        Value: !nodeService.installed ? `${nodeService.label} not installed` : `${nodeService.label} ${nodeService.installed ? 'installed \xB7 ' : ''}${nodeService.loadedText}${nodeService.runtime?.status ? ` \xB7 ${nodeService.runtime.status}` : ''}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ''}`
...
__name(statusAllCommand, 'statusAllCommand');
export {
  statusAllCommand
};
