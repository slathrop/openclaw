...
__name(formatSupport, 'formatSupport');
...
__name(summarizeDiscordTarget, 'summarizeDiscordTarget');
...
__name(formatDiscordIntents, 'formatDiscordIntents');
  ...
  if (channelId === 'msteams') {
    ...
    if (graph) {
      ...
      if (graph.ok === false) {
        ...
        const formatPermission = /* @__PURE__ */ __name((permission) => {
...
__name(formatProbeLines, 'formatProbeLines');
...
__name(buildDiscordPermissions, 'buildDiscordPermissions');
async function resolveChannelReports(params) {
  ...
  const actions = Array.from(
    /* @__PURE__ */ new Set(['send', 'broadcast', ...listedActions.map((action) => String(action))])
  );
...
__name(resolveChannelReports, 'resolveChannelReports');
  ...
  for (const report of reports) {
    ...
    if (report.channel === 'slack' && report.slackScopes) {
      for (const entry of report.slackScopes) {
        ...
        const label2 = entry.tokenType === 'user' ? 'User scopes' : 'Bot scopes';
        if (entry.result.ok && entry.result.scopes?.length) {
          lines.push(`${label2}${source}: ${entry.result.scopes.join(', ')}`);
        } else if (entry.result.error) {
          lines.push(`${label2}: ${theme.error(entry.result.error)}`);
        }
      }
    }
    if (report.channel === 'discord' && report.channelPermissions) {
      ...
      if (perms.error) {
        ...
        const label2 = perms.channelId ? ` (${perms.channelId})` : '';
        lines.push(`Permissions${label2}: ${list}`);
...
__name(channelsCapabilitiesCommand, 'channelsCapabilitiesCommand');
export {
