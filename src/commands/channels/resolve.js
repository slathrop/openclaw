...
__name(resolvePreferredKind, 'resolvePreferredKind');
function detectAutoKind(input) {
  ...
  if (/^(user|discord|slack|matrix|msteams|teams|zalo|zalouser|googlechat|google-chat|gchat):/i.test(
    trimmed
  )) {
    return 'user';
  }
...
__name(detectAutoKind, 'detectAutoKind');
...
__name(formatResolveResult, 'formatResolveResult');
...
__name(channelsResolveCommand, 'channelsResolveCommand');
export {
