...
__name(listAccountIds, 'listAccountIds');
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
  ...
  if (useWizard && prompter) {
    ...
    const selectedChannel = await prompter.select({
      message: 'Channel',
      options: listChannelPlugins().map((plugin2) => ({
        value: plugin2.id,
        label: plugin2.meta.label
      }))
    });
  ...
  if (useWizard && prompter) {
    await prompter.outro(
      deleteConfig ? `Deleted ${channelLabel(channel)} account "${accountKey}".` : `Disabled ${channelLabel(channel)} account "${accountKey}".`
    );
  } else {
    runtime.log(
      deleteConfig ? `Deleted ${channelLabel(channel)} account "${accountKey}".` : `Disabled ${channelLabel(channel)} account "${accountKey}".`
...
__name(channelsRemoveCommand, 'channelsRemoveCommand');
export {
