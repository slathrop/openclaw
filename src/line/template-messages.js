...
function createButtonTemplate(title, text, actions, options) {
  ...
  const template = {
    ...
    // LINE limit (60 if no thumbnail, 160 with thumbnail)
    actions: actions.slice(0, 4),
    // LINE limit: max 4 actions
...
function createCarouselColumn(params) {
  return {
    ...
    // LINE limit: max 3 actions per column
...
function createYesNoConfirm(question, options) {
  const yesAction = options?.yesData ? postbackAction(options.yesText ?? 'Yes', options.yesData, options.yesText ?? 'Yes') : messageAction(options?.yesText ?? 'Yes');
  const noAction = options?.noData ? postbackAction(options.noText ?? 'No', options.noData, options.noText ?? 'No') : messageAction(options?.noText ?? 'No');
