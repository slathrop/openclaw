...
describe('createQuickReplyItems', () => {
  it('creates quick reply items from labels', () => {
    ...
    expect(quickReply.items[0].action.label).toBe('Option 1');
    expect(quickReply.items[0].action.text).toBe('Option 1');
  });
  ...
  it('truncates labels to 20 characters', () => {
    ...
    expect(quickReply.items[0].action.label).toBe('This is a very long ');
    expect(quickReply.items[0].action.text).toBe(
      'This is a very long option label that exceeds the limit'
    );
  });
  it('creates message actions for each item', () => {
    ...
    expect(quickReply.items[1].action.type).toBe('message');
  });
});
describe('createTextMessageWithQuickReplies', () => {
  ...
  it('preserves text content', () => {
    const longText = 'This is a longer message that asks the user to select from multiple options below.';
  ...
  it('quick replies use label as both label and text', () => {
    ...
    const firstAction = message.quickReply.items[0].action;
