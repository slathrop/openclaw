/**
 * LINE bot message context builder
 * @typedef {object} BotMessageMediaRef
 * @property {string} path
 * @property {string} [mimeType]
 * @typedef {object} BuildLineMessageContextParams
 * @property {import("@line/bot-sdk").WebhookEvent} event
 * @property {import("./types.js").ResolvedLineAccount} account
...
function getSourceInfo(source) {
  const userId = source.type === 'user' ? source.userId : source.type === 'group' ? source.userId : source.type === 'room' ? source.userId : void 0;
  const groupId = source.type === 'group' ? source.groupId : void 0;
  const roomId = source.type === 'room' ? source.roomId : void 0;
...
async function buildLinePostbackContext(params) {
  ...
  if (rawData.includes('line.action=')) {
    const params2 = new URLSearchParams(rawData);
    const action = params2.get('line.action') ?? '';
    const device = params2.get('line.device');
