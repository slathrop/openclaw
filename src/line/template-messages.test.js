...
describe('messageAction', () => {
  it('creates a message action', () => {
    ...
    expect(action.text).toBe('clicked');
  });
...
describe('datetimePickerAction', () => {
  ...
  it('includes min/max/initial when provided', () => {
    ...
    expect(action.initial).toBe('2024-01-01T12:00');
    expect(action.min).toBe('2024-01-01T00:00');
    expect(action.max).toBe('2024-12-31T23:59');
  });
});
describe('createConfirmTemplate', () => {
  it('creates a confirm template', () => {
    ...
    expect(template.template.text).toBe('Are you sure?');
  });
...
describe('createButtonTemplate', () => {
  it('creates a button template', () => {
    ...
    expect(template.template.title).toBe('Title');
    expect(template.template.text).toBe('Description');
  });
  ...
  it('includes thumbnail when provided', () => {
    ...
    expect(template.template.thumbnailImageUrl).toBe(
...
describe('createTemplateCarousel', () => {
  it('creates a carousel template', () => {
    ...
    expect(template.template.columns.length).toBe(2);
  });
...
describe('createImageCarousel', () => {
  ...
  it('limits columns to 10', () => {
    const columns = Array.from(
      { length: 15 },
      (_, i) => createImageCarouselColumn(`https://example.com/${i}.jpg`, messageAction('View'))
    );
    const template = createImageCarousel(columns);
    expect(template.template.columns.length).toBe(10);
  });
