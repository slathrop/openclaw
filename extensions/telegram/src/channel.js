...
const telegramMessageActions = {
  ...
  extractToolSend: (ctx) => getTelegramRuntime().channel.telegram.messageActions.extractToolSend(ctx),
  handleAction: async (ctx) => await getTelegramRuntime().channel.telegram.messageActions.handleAction(ctx)
};
...
const telegramPlugin = {
  ...
  status: {
    ...
    auditAccount: async ({ account, timeoutMs, probe, cfg }) => {
      ...
      const botId = probe?.ok && probe.bot?.id !== null && probe.bot?.id !== undefined ? probe.bot.id : null;
      if (!botId) {
        return {
