...
const zaloDock = {
  ...
  config: {
    ...
    formatAllowFrom: ({ allowFrom }) => allowFrom.map((entry) => String(entry).trim()).filter(Boolean).map((entry) => entry.replace(/^(zalo|zl):/i, '')).map((entry) => entry.toLowerCase())
  },
  groups: {
...
const zaloPlugin = {
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const basePath = useAccountPath ? `channels.zalo.accounts.${resolvedAccountId}.` : 'channels.zalo.';
      return {
