...
function createMSTeamsMessageHandler(deps) {
  ...
  const handleTeamsMessageNow = async (params) => {
    ...
    const storedAllowFrom = await core.channel.pairing.readAllowFromStore('msteams').catch(() => []);
    ...
    const groupAllowFrom = !isDirectMessage && msteamsCfg ? msteamsCfg.groupAllowFrom ?? (msteamsCfg.allowFrom && msteamsCfg.allowFrom.length > 0 ? msteamsCfg.allowFrom : []) : [];
    const effectiveGroupAllowFrom = !isDirectMessage && msteamsCfg ? [...groupAllowFrom.map((v) => String(v)), ...storedAllowFrom] : [];
