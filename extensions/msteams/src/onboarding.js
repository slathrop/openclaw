...
async function promptMSTeamsAllowFrom(params) {
  ...
  while (true) {
    ...
    if (!resolved) {
      ...
      const unique2 = [
        .../* @__PURE__ */ new Set([...existing.map((v) => String(v).trim()).filter(Boolean), ...ids2])
      ];
...
const msteamsOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter }) => {
    ...
    const hasConfigCreds = Boolean(
      cfg.channels?.msteams?.appId?.trim() && cfg.channels?.msteams?.appPassword?.trim() && cfg.channels?.msteams?.tenantId?.trim()
    );
    const canUseEnv = Boolean(
      !hasConfigCreds && process.env.MSTEAMS_APP_ID?.trim() && process.env.MSTEAMS_APP_PASSWORD?.trim() && process.env.MSTEAMS_TENANT_ID?.trim()
    );
    let next = cfg;
    let appId = null;
