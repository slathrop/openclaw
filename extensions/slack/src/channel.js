...
const slackPlugin = {
  ...
  actions: {
    ...
    handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
      const resolveChannelId = () => readStringParam(params, 'channelId') ?? readStringParam(params, 'to', { required: true });
      if (action === 'send') {
      ...
      if (action === 'member-info') {
        ...
        return await getSlackRuntime().channel.slack.handleSlackAction(
          { action: 'memberInfo', userId, accountId: accountId ?? void 0 },
          cfg
        );
      }
      if (action === 'emoji-list') {
        return await getSlackRuntime().channel.slack.handleSlackAction(
          { action: 'emojiList', accountId: accountId ?? void 0 },
          cfg
        );
      }
