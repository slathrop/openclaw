      ...
      this._table = await this._db.createTable(TABLE_NAME, [
        {
          id: '__schema__',
          text: '',
          vector: Array.from({ length: this._vectorDim }).fill(0),
          importance: 0,
          category: 'other',
          createdAt: 0
        }
      ]);
      await this._table.delete('id = "__schema__"');
    }
  }
  async store(entry) {
    ...
    await this._table.add([fullEntry]);
    return fullEntry;
  }
  async search(vector, limit = 5, minScore = 0.5) {
    await this.ensureInitialized();
    const results = await this._table.vectorSearch(vector).limit(limit).toArray();
...
const stdin_default = memoryPlugin;
export {
