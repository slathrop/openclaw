...
const matrixPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg }) => (cfg.channels?.matrix?.dm?.allowFrom ?? []).map((entry) => String(entry)),
  ...
  threading: {
    resolveReplyToMode: ({ cfg }) => cfg.channels?.matrix?.replyToMode ?? 'off',
    buildToolContext: ({ context, hasRepliedRef }) => {
      const currentTarget = context.To;
      return {
        currentChannelId: currentTarget?.trim() || void 0,
        currentThreadTs: context.MessageThreadId !== null && context.MessageThreadId !== undefined ? String(context.MessageThreadId) : context.ReplyToId,
        hasRepliedRef
      };
    }
  },
  ...
  directory: {
    ...
    listPeersLive: async ({ cfg, query, limit }) => listMatrixDirectoryPeersLive({ cfg, query, limit }),
    listGroupsLive: async ({ cfg, query, limit }) => listMatrixDirectoryGroupsLive({ cfg, query, limit })
  },
  resolver: {
