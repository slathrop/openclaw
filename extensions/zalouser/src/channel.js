...
const zalouserDock = {
  ...
  config: {
    ...
    formatAllowFrom: ({ allowFrom }) => allowFrom.map((entry) => String(entry).trim()).filter(Boolean).map((entry) => entry.replace(/^(zalouser|zlu):/i, '')).map((entry) => entry.toLowerCase())
  },
  groups: {
...
const zalouserPlugin = {
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const basePath = useAccountPath ? `channels.zalouser.accounts.${resolvedAccountId}.` : 'channels.zalouser.';
      return {
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      ...
      const result = await runZca(args, { profile: account.profile, timeout: 15e3 });
      if (!result.ok) {
        throw new Error(result.stderr || 'Failed to list peers');
      }
      const parsed = parseJsonOutput(result.stdout);
      const rows = Array.isArray(parsed) ? parsed.map(
        (f) => mapUser({
          id: String(f.userId),
    ...
    listGroups: async ({ cfg, accountId, query, limit }) => {
      ...
      const parsed = parseJsonOutput(result.stdout);
      let rows = Array.isArray(parsed) ? parsed.map(
        (g) => mapGroup({
          id: String(g.groupId),
          name: g.name ?? null,
          raw: g
    ...
    listGroupMembers: async ({ cfg, accountId, groupId, limit }) => {
      ...
      const rows = Array.isArray(parsed) ? parsed.map((m) => {
    ...
    resolveTargets: async ({ cfg, accountId, inputs, kind, runtime }) => {
      ...
      for (const input of inputs) {
        ...
        try {
          ...
          const args = kind === 'user' ? trimmed ? ['friend', 'find', trimmed] : ['friend', 'list', '-j'] : ['group', 'list', '-j'];
          const result = await runZca(args, { profile: account.profile, timeout: 15e3 });
          if (!result.ok) {
          ...
          if (kind === 'user') {
            const parsed = parseJsonOutput(result.stdout) ?? [];
            const matches = Array.isArray(parsed) ? parsed.map((f) => ({
              id: String(f.userId),
              name: f.displayName ?? void 0
            })) : [];
            ...
            const parsed = parseJsonOutput(result.stdout) ?? [];
            const matches = Array.isArray(parsed) ? parsed.map((g) => ({
              id: String(g.groupId),
              name: g.name ?? void 0
            })) : [];
