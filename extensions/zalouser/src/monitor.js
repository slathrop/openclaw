...
async function processMessage(message, account, config, core, runtime, statusSink) {
  ...
  const storeAllowFrom = !isGroup && (dmPolicy !== 'open' || shouldComputeAuth) ? await core.channel.pairing.readAllowFromStore('zalouser').catch(() => []) : [];
...
async function monitorZalouserProvider(options) {
  ...
  try {
    ...
    if (allowFromEntries.length > 0) {
      const result = await runZca(['friend', 'list', '-j'], { profile, timeout: 15e3 });
      if (result.ok) {
        const friends = parseJsonOutput(result.stdout) ?? [];
    ...
    if (groupKeys.length > 0) {
      const result = await runZca(['group', 'list', '-j'], { profile, timeout: 15e3 });
      if (result.ok) {
        const groups = parseJsonOutput(result.stdout) ?? [];
    ...
    proc = startZcaListener(
      ...
      (err) => {
        ...
        if (!stopped && !abortSignal.aborted) {
          ...
          restartTimer = setTimeout(startListener, 5e3);
        } else {
          resolveRunning?.();
        }
      },
