...
async function monitorTlonProvider(opts = {}) {
  ...
  const handleIncomingDM = async (update) => {
    try {
      ...
      const messageId = update.id !== null && update.id !== undefined ? String(update.id) : void 0;
  ...
  const handleIncomingGroupMessage = (channelNest) => async (update) => {
    try {
      ...
      const messageId = rawMessageId !== null && rawMessageId !== undefined ? String(rawMessageId) : void 0;
      if (!processedTracker.mark(messageId)) {
        return;
      }
...
_[Generated by ${formatModelName(modelInfo)}]_`;
  ...
  const subscribedDMs = /* @__PURE__ */ new Set();
