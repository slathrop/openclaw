...
const normalizeAllowEntry = (entry) => entry.replace(/^(feishu|lark):/i, '').trim();
const feishuPlugin = {
  id: 'feishu',
  meta,
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }) => resolveFeishuConfig({ cfg, accountId: accountId ?? void 0 }).allowFrom.map(
      (entry) => String(entry)
    ),
    formatAllowFrom: ({ allowFrom }) => allowFrom.map((entry) => String(entry).trim()).filter(Boolean).map((entry) => entry === '*' ? entry : normalizeAllowEntry(entry)).map((entry) => entry === '*' ? entry : entry.toLowerCase())
  },
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      const resolvedAccountId = accountId ?? account.accountId ?? DEFAULT_ACCOUNT_ID;
      const useAccountPath = Boolean(cfg.channels?.feishu?.accounts?.[resolvedAccountId]);
      const basePath = useAccountPath ? `channels.feishu.accounts.${resolvedAccountId}.` : 'channels.feishu.';
      return {
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const resolved = resolveFeishuConfig({ cfg, accountId: accountId ?? void 0 });
      const normalizedQuery = query?.trim().toLowerCase() ?? '';
      const peers = resolved.allowFrom.map((entry) => String(entry).trim()).filter((entry) => Boolean(entry) && entry !== '*').map((entry) => normalizeAllowEntry(entry)).filter((entry) => normalizedQuery ? entry.toLowerCase().includes(normalizedQuery) : true).slice(0, limit && limit > 0 ? limit : void 0).map((id) => ({ kind: 'user', id }));
      return peers;
    },
    listGroups: async ({ cfg, accountId, query, limit }) => {
      const resolved = resolveFeishuConfig({ cfg, accountId: accountId ?? void 0 });
      const normalizedQuery = query?.trim().toLowerCase() ?? '';
      const groups = Object.keys(resolved.groups ?? {}).filter((id) => normalizedQuery ? id.toLowerCase().includes(normalizedQuery) : true).slice(0, limit && limit > 0 ? limit : void 0).map((id) => ({ kind: 'group', id }));
      return groups;
    }
  },
  status: {
    ...
    probeAccount: async ({ account, timeoutMs }) => probeFeishu(account.config.appId, account.config.appSecret, timeoutMs, account.config.domain),
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      let feishuBotLabel = '';
      try {
        const probe = await probeFeishu(appId, appSecret, 5e3, domain);
