...
function setFeishuDmPolicy(cfg, policy) {
  const allowFrom = policy === 'open' ? addWildcardAllowFrom(cfg.channels?.feishu?.allowFrom) : void 0;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
async function noteFeishuSetup(prompter) {
  await prompter.note(
    [
...
function resolveDomainChoice(domain) {
...
async function promptFeishuAllowFrom(params) {
  ...
  const existingAllowFrom = isDefault ? cfg.channels?.feishu?.allowFrom ?? [] : cfg.channels?.feishu?.accounts?.[accountId]?.allowFrom ?? [];
  const entry = await prompter.text({
    ...
    validate: (value) => {
      ...
      const entries = raw.split(/[\n,;]+/g).map((item) => normalizeAllowEntry(item)).filter(Boolean);
...
function updateFeishuConfig(cfg, accountId, updates) {
  ...
  const accounts = feishu.accounts ? { ...feishu.accounts } : void 0;
  ...
  const existing = resolvedAccounts[accountId] ?? {};
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    ...
    const appId = String(
      await prompter.text({
        ...
        initialValue: resolved.config.appId?.trim() || void 0,
    ...
    const appSecret = String(
      await prompter.text({
        ...
        initialValue: resolved.config.appSecret?.trim() || void 0,
        validate: (value) => String(value ?? '').trim() ? void 0 : 'Required'
      })
    ).trim();
