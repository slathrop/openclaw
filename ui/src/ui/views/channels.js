...
function renderChannels(props) {
  ...
  const whatsapp = channels?.whatsapp ?? void 0;
  const telegram = channels?.telegram ?? void 0;
  ...
  return html`
    <section class="grid grid-cols-2">
      ${orderedChannels.map(
        (channel) => renderChannel(channel.key, props, {
          whatsapp,
          telegram,
          discord,
          googlechat,
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      ${props.lastError ? html`<div class="callout danger" style="margin-top: 12px;">
            ${props.lastError}
          </div>` : nothing}
...
function renderGenericChannelCard(key, props, channelAccounts) {
  ...
  const configured = typeof status?.configured === 'boolean' ? status.configured : void 0;
  const running = typeof status?.running === 'boolean' ? status.running : void 0;
  const connected = typeof status?.connected === 'boolean' ? status.connected : void 0;
  const lastError = typeof status?.lastError === 'string' ? status.lastError : void 0;
          ...
          ` : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                ...
                <span>${configured === null || configured === undefined ? 'n/a' : configured ? 'Yes' : 'No'}</span>
              </div>
              <div>
                <span class="label">Running</span>
                <span>${running === null || running === undefined ? 'n/a' : running ? 'Yes' : 'No'}</span>
              </div>
              <div>
                <span class="label">Connected</span>
                <span>${connected === null || connected === undefined ? 'n/a' : connected ? 'Yes' : 'No'}</span>
              </div>
            </div>
          `}

      ${lastError ? html`<div class="callout danger" style="margin-top: 12px;">
            ${lastError}
          </div>` : nothing}
...
const RECENT_ACTIVITY_THRESHOLD_MS = 10 * 60 * 1e3;
