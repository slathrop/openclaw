...
function resolveAgentEmoji(agent, agentIdentity) {
...
function agentBadgeText(agentId, defaultId) {
...
function buildAgentContext(agent, configForm, agentFilesList, defaultId, agentIdentity) {
  ...
  const modelLabel = config.entry?.model ? resolveModelLabel(config.entry?.model) : resolveModelLabel(config.defaults?.model);
  const identityName = agentIdentity?.name?.trim() || agent.identity?.name?.trim() || agent.name?.trim() || config.entry?.name || agent.id;
...
function normalizeModelValue(label) {
...
function resolveModelPrimary(model) {
  ...
  if (typeof model === 'object' && model) {
    const record = model;
    const candidate = typeof record.primary === 'string' ? record.primary : typeof record.model === 'string' ? record.model : typeof record.id === 'string' ? record.id : typeof record.value === 'string' ? record.value : null;
...
function resolveModelFallbacks(model) {
  ...
  if (typeof model === 'object' && model) {
    const record = model;
    const fallbacks = Array.isArray(record.fallbacks) ? record.fallbacks : Array.isArray(record.fallback) ? record.fallback : null;
    return fallbacks ? fallbacks.filter((entry) => typeof entry === 'string') : null;
  }
  return null;
}
function parseFallbackList(value) {
  return value.split(',').map((entry) => entry.trim()).filter(Boolean);
}
function resolveConfiguredModels(configForm) {
  ...
  for (const [modelId, modelRaw] of Object.entries(models)) {
    ...
    const alias = modelRaw && typeof modelRaw === 'object' && 'alias' in modelRaw ? typeof modelRaw.alias === 'string' ? modelRaw.alias?.trim() : void 0 : void 0;
    const label = alias && alias !== trimmed ? `${alias} (${trimmed})` : trimmed;
    options.push({ value: trimmed, label });
  }
  return options;
}
function buildModelOptions(configForm, current) {
...
function compilePatterns(patterns) {
  ...
  return expandToolGroups(patterns).map(compilePattern).filter((pattern) => {
...
function isAllowedByPolicy(name, policy) {
  if (!policy) {
    return true;
  }
    ...
    <div class="agents-layout">
      ...
      <section class="agents-main">
        ${!selectedAgent ? html`
              ...
              ${props.activePanel === 'overview' ? renderAgentOverview({
                agent: selectedAgent,
                defaultId,
              ...
              ${props.activePanel === 'files' ? renderAgentFiles({
              ...
              ${props.activePanel === 'tools' ? renderAgentTools({
                agentId: selectedAgent.id,
                configForm: props.configForm,
                configLoading: props.configLoading,
                configSaving: props.configSaving,
              ...
              ${props.activePanel === 'skills' ? renderAgentSkills({
                agentId: selectedAgent.id,
              ...
              ${props.activePanel === 'channels' ? renderAgentChannels({
                agent: selectedAgent,
                defaultId,
                configForm: props.configForm,
                agentFilesList: props.agentFilesList,
              ...
              ${props.activePanel === 'cron' ? renderAgentCron({
                agent: selectedAgent,
                defaultId,
                configForm: props.configForm,
                agentFilesList: props.agentFilesList,
...
function renderAgentHeader(agent, defaultId, agentIdentity) {
  const badge = agentBadgeText(agent.id, defaultId);
...
function renderAgentTabs(active, onSelect) {
  const tabs = [
        ...
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          <label class="field" style="min-width: 260px; flex: 1;">
            ...
            <select
              ...
              @change=${(e) => onModelChange(agent.id, e.target.value || null)}
            >
              ${isDefault ? nothing : html`
                      <option value="">
                        ${defaultPrimary ? `Inherit default (${defaultPrimary})` : 'Inherit default'}
                      </option>
                    `}
              ${buildModelOptions(configForm, effectivePrimary ?? void 0)}
            </select>
          </label>
          <label class="field" style="min-width: 260px; flex: 1;">
            ...
            <input
              ...
              @input=${(e) => onModelFallbacksChange(
                agent.id,
                parseFallbackList(e.target.value)
              )}
            />
          </label>
        </div>
...
function renderAgentContextCard(context, subtitle) {
  return html`
    <section class="card">
...
function resolveChannelLabel(snapshot, id) {
...
function resolveChannelEntries(snapshot) {
  if (!snapshot) {
    return [];
  }
...
function resolveChannelConfigValue(configForm, channelId) {
  if (!configForm) {
    return null;
  }
...
function formatChannelExtraValue(raw) {
...
function resolveChannelExtras(configForm, channelId) {
...
function summarizeChannelAccounts(accounts) {
  let connected = 0;
  let configured = 0;
  let enabled = 0;
  for (const account of accounts) {
    const probeOk = account.probe && typeof account.probe === 'object' && 'ok' in account.probe ? Boolean(account.probe.ok) : false;
              ...
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${files.length === 0 ? html`
                        ...
                        ` : files.map(
                    (file) => renderAgentFileRow(file, active, () => params.onSelectFile(file.name))
                  )}
                </div>
                  ...
                  ${!activeEntry ? html`
                        ...
                        ` : html`
                          ...
                          <label class="field" style="margin-top: 12px;">
                            ...
                            <textarea
                              .value=${draft}
                              @input=${(e) => params.onFileDraftChange(
                                activeEntry.name,
                                e.target.value
                              )}
                            ></textarea>
...
function renderAgentFileRow(file, active, onSelect) {
  const status = file.missing ? 'Missing' : `${formatBytes(file.size)} \xB7 ${formatAgo(file.updatedAtMs ?? null)}`;
  return html`
    <button
      type="button"
...
function renderAgentTools(params) {
  ...
  const basePolicy = hasAgentAllow ? { allow: agentTools.allow ?? [], deny: agentTools.deny ?? [] } : resolveToolProfilePolicy(profile) ?? void 0;
  const toolIds = TOOL_SECTIONS.flatMap((section) => section.tools.map((tool) => tool.id));
  const resolveAllowed = (toolId) => {
  ...
  const updateTool = (toolId, nextEnabled) => {
                    ...
                    <div class="agent-tool-row">
                      ...
                      <label class="cfg-toggle">
                        <input
                          ...
                          .checked=${allowed}
                          ?disabled=${!editable}
                          @change=${(e) => updateTool(tool.id, e.target.checked)}
                        />
...
function groupSkills(skills) {
  ...
  const ordered = SKILL_SOURCE_GROUPS.map((group) => groups.get(group.id)).filter(
    (group) => Boolean(group && group.skills.length > 0)
  );
...
function renderAgentSkills(params) {
  ...
  const config = resolveAgentConfig(params.configForm, params.agentId);
  const allowlist = Array.isArray(config.entry?.skills) ? config.entry?.skills : void 0;
  const allowSet = new Set((allowlist ?? []).map((name) => name.trim()).filter(Boolean));
  const usingAllowlist = allowlist !== void 0;
  const reportReady = Boolean(params.report && params.activeAgentId === params.agentId);
  const rawSkills = reportReady ? params.report?.skills ?? [] : [];
  const filter = params.filter.trim().toLowerCase();
  const filtered = filter ? rawSkills.filter(
    (skill) => [skill.name, skill.description, skill.source].join(' ').toLowerCase().includes(filter)
  ) : rawSkills;
    ...
    <section class="card">
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="flex: 1;">
          ...
          <input
            .value=${params.filter}
            @input=${(e) => params.onFilterChange(e.target.value)}
      ...
      ${filtered.length === 0 ? html`
            ...
            ` : html`
              <div class="agent-skills-groups" style="margin-top: 16px;">
                ${groups.map(
                  (group) => renderAgentSkillGroup(group, {
...
function renderAgentSkillGroup(group, params) {
  ...
  return html`
    <details class="agent-skills-group" ?open=${!collapsedByDefault}>
      ...
      <div class="list skills-grid">
        ${group.skills.map(
          (skill) => renderAgentSkillRow(skill, {
            agentId: params.agentId,
...
function renderAgentSkillRow(skill, params) {
      ...
      <div class="list-meta">
        <label class="cfg-toggle">
          <input
            ...
            @change=${(e) => params.onToggle(params.agentId, skill.name, e.target.checked)}
          />
          <span class="cfg-toggle__track"></span>
        </label>
      </div>
