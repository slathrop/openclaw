...
function renderConfig(props) {
  const validity = props.valid === null || props.valid === undefined ? 'unknown' : props.valid ? 'valid' : 'invalid';
  const analysis = analyzeConfigSchema(props.schema);
  const formUnsafe = analysis.schema ? analysis.unsupportedPaths.length > 0 : false;
  const schemaProps = analysis.schema?.properties ?? {};
  const availableSections = SECTIONS.filter((s) => s.key in schemaProps);
  const knownKeys = new Set(SECTIONS.map((s) => s.key));
  const extraSections = Object.keys(schemaProps).filter((k) => !knownKeys.has(k)).map((k) => ({ key: k, label: k.charAt(0).toUpperCase() + k.slice(1) }));
  const allSections = [...availableSections, ...extraSections];
  const activeSectionSchema = props.activeSection && analysis.schema && schemaType(analysis.schema) === 'object' ? analysis.schema.properties?.[props.activeSection] : void 0;
  const activeSectionMeta = props.activeSection ? resolveSectionMeta(props.activeSection, activeSectionSchema) : null;
  const subsections = props.activeSection ? resolveSubsections({
  ...
  const effectiveSubsection = props.searchQuery ? null : isAllSubsection ? null : props.activeSubsection ?? subsections[0]?.key ?? null;
  ...
  const canApply = props.connected && !props.applying && !props.updating && hasChanges && (props.formMode === 'raw' ? true : canSaveForm);
    ...
    <div class="config-layout">
      ...
      <main class="config-main">
        ...
        ${hasChanges && props.formMode === 'form' ? html`
            ...
            ` : nothing}
        ${activeSectionMeta && props.formMode === 'form' ? html`
              <div class="config-section-hero">
                ...
                <div class="config-section-hero__text">
                  ...
                  ${activeSectionMeta.description ? html`<div class="config-section-hero__desc">
        ...
        ${props.issues.length > 0 ? html`<div class="callout danger" style="margin-top: 12px;">
