...
function renderInstances(props) {
  return html`
    <section class="card">
      ...
      ${props.statusMessage ? html`<div class="callout" style="margin-top: 12px;">
            ${props.statusMessage}
          </div>` : nothing}
...
function renderEntry(entry) {
  const lastInput = entry.lastInputSeconds !== null && entry.lastInputSeconds !== undefined ? `${entry.lastInputSeconds}s ago` : 'n/a';
  const mode = entry.mode ?? 'unknown';
  const roles = Array.isArray(entry.roles) ? entry.roles.filter(Boolean) : [];
  const scopes = Array.isArray(entry.scopes) ? entry.scopes.filter(Boolean) : [];
  const scopesLabel = scopes.length > 0 ? scopes.length > 3 ? `${scopes.length} scopes` : `scopes: ${scopes.join(', ')}` : null;
  return html`
    <div class="list-item">
      <div class="list-main">
