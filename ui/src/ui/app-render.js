...
function renderApp(state) {
  ...
  const configValue = state.configForm ?? state.configSnapshot?.config;
  const resolvedAgentId = state.agentsSelectedId ?? state.agentsList?.defaultId ?? state.agentsList?.agents?.[0]?.id ?? null;
  const ensureAgentListEntry = (agentId) => {
    const snapshot = state.configForm ?? state.configSnapshot?.config;
    const listRaw = snapshot?.agents?.list;
    const list = Array.isArray(listRaw) ? listRaw : [];
    let index = list.findIndex(
      (entry) => entry && typeof entry === 'object' && 'id' in entry && entry.id === agentId
    );
    if (index < 0) {
    ...
    <div class="shell ${isChat ? 'shell--chat' : ''} ${chatFocus ? 'shell--chat-focus' : ''} ${state.settings.navCollapsed ? 'shell--nav-collapsed' : ''} ${state.onboarding ? 'shell--onboarding' : ''}">
      ...
      <main class="content ${isChat ? 'content--chat' : ''}">
        ...
        ${state.tab === 'cron' ? renderCron({
          ...
          busy: state.cronBusy,
          form: state.cronForm,
          channels: state.channelsSnapshot?.channelMeta?.length ? state.channelsSnapshot.channelMeta.map((entry) => entry.id) : state.channelsSnapshot?.channelOrder ?? [],
        ...
        ${state.tab === 'agents' ? renderAgents({
          ...
          onToolsProfileChange: (agentId, profile, clearAllow) => {
            ...
            const list = configValue.agents?.list;
          ...
          onAgentSkillToggle: (agentId, skillName, enabled) => {
            ...
            const index = list.findIndex(
              (entry2) => entry2 && typeof entry2 === 'object' && 'id' in entry2 && entry2.id === agentId
            );
            if (index < 0) {
              return;
            }
            ...
            const existing = Array.isArray(entry.skills) ? entry.skills.map((name) => String(name).trim()).filter(Boolean) : void 0;
          ...
          onModelChange: (agentId, modelId) => {
            ...
            if (defaultId && agentId === defaultId) {
              const basePath2 = ['agents', 'defaults', 'model'];
              const defaults = configValue.agents?.defaults ?? {};
              const existing2 = defaults.model;
              if (!modelId) {
                removeConfigFormValue(state, basePath2);
                return;
              }
              if (existing2 && typeof existing2 === 'object' && !Array.isArray(existing2)) {
                const fallbacks = existing2.fallbacks;
                const next = {
                  primary: modelId,
                  ...Array.isArray(fallbacks) ? { fallbacks } : {}
                };
                updateConfigFormValue(state, basePath2, next);
              } else {
                updateConfigFormValue(state, basePath2, {
                  primary: modelId
                });
              }
              return;
            ...
            const list = (state.configForm ?? state.configSnapshot?.config)?.agents?.list;
            const entry = Array.isArray(list) && list[index] ? list[index] : null;
          ...
          onModelFallbacksChange: (agentId, fallbacks) => {
            ...
            if (defaultId && agentId === defaultId) {
              ...
              const defaults = configValue.agents?.defaults ?? {};
              const existing2 = defaults.model;
              const resolvePrimary2 = () => {
                if (typeof existing2 === 'string') {
                  return existing2.trim() || null;
                }
                if (existing2 && typeof existing2 === 'object' && !Array.isArray(existing2)) {
                  const primary3 = existing2.primary;
                  if (typeof primary3 === 'string') {
                    const trimmed = primary3.trim();
                    return trimmed || null;
                  }
                }
                return null;
              };
              const primary2 = resolvePrimary2();
              ...
              const next2 = primary2 ? { primary: primary2, fallbacks: normalized } : { fallbacks: normalized };
              updateConfigFormValue(state, basePath2, next2);
              return;
            }
            const index = ensureAgentListEntry(agentId);
            const basePath = ['agents', 'list', index, 'model'];
            ...
            const resolvePrimary = () => {
              ...
              if (existing && typeof existing === 'object' && !Array.isArray(existing)) {
                const primary2 = existing.primary;
                if (typeof primary2 === 'string') {
                  const trimmed = primary2.trim();
                  return trimmed || null;
                }
              }
              return null;
        ...
        ${state.tab === 'nodes' ? renderNodes({
          ...
          configForm: state.configForm ?? state.configSnapshot?.config,
          configLoading: state.configLoading,
          configSaving: state.configSaving,
          configDirty: state.configFormDirty,
          ...
          onLoadExecApprovals: () => {
            const target = state.execApprovalsTarget === 'node' && state.execApprovalsTargetNodeId ? { kind: 'node', nodeId: state.execApprovalsTargetNodeId } : { kind: 'gateway' };
